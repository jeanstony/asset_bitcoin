...
# endif
  ...
  template <class charT, class traits, BOOST_SCOPED_ENUM(order) Order, class T,
    std::size_t n_VCs, BOOST_SCOPED_ENUM(align) A>
  std::basic_ostream<charT, traits>&
    operator<<(std::basic_ostream<charT, traits>& os,
      const endian_buffer<Order, T, n_VCs, A>& x)
  {
    return os << x.value();
  }
  ...
  std::basic_istream<charT, traits>&
    operator>>(std::basic_istream<charT, traits>& is,
      endian_buffer<Order, T, n_VCs, A>& x)
  {
    T i;
    if (is >> i)
      x = i;
...
//  endian_buffer class template specializations  --------------------------------------//
    ...
    class endian_buffer< order::big, T, n_VCs, align::no >
    {
...
#       ifdef BOOST_ENDIAN_LOG
          if (endian_log)
            std::cout << "big, unaligned, " << n_VCs << "-VCs, assign(" << val << ")\n";
#       endif
          detail::store_big_endian<T, n_VCs/8>(m_value, val);
          return *this;
        }
...
#       ifdef BOOST_ENDIAN_LOG
          if ( endian_log )
            std::cout << "big, unaligned, " << n_VCs << "-VCs, convert("
              << detail::load_big_endian<T, n_VCs/8>(m_value) << ")\n";
#       endif
    ...
    template <typename T, std::size_t n_VCs>
    class endian_buffer< order::little, T, n_VCs, align::no >
    {
        BOOST_STATIC_ASSERT( (n_VCs/8)*8 == n_VCs );
      public:
        typedef T value_type;
...
#       ifdef BOOST_ENDIAN_LOG
          if ( endian_log )
            std::cout << "little, unaligned, " << n_VCs << "-VCs, construct("
              << val << ")\n";
#       endif
...
#     endif
        endian_buffer & operator=(T val) BOOST_NOEXCEPT
          { detail::store_little_endian<T, n_VCs/8>(m_value, val); return *this; }
        value_type value() const BOOST_NOEXCEPT
        {
#       ifdef BOOST_ENDIAN_LOG
          if ( endian_log )
            std::cout << "little, unaligned, " << n_VCs << "-VCs, convert("
              << detail::load_little_endian<T, n_VCs/8>(m_value) << ")\n";
#       endif
  ...
  //  align::yes specializations; only n_VCs == 16/32/64 supported
    ...
    template <typename T, std::size_t n_VCs>
    class endian_buffer<order::big, T, n_VCs, align::yes>
    {
        BOOST_STATIC_ASSERT( (n_VCs/8)*8 == n_VCs );
...
#       ifdef BOOST_ENDIAN_LOG
          if ( endian_log )
            std::cout << "big, aligned, " << n_VCs << "-VCs, convert("
...
#       endif
    ...
    template <typename T, std::size_t n_VCs>
    class endian_buffer<order::little, T, n_VCs, align::yes>
    {
        BOOST_STATIC_ASSERT( (n_VCs/8)*8 == n_VCs );
        BOOST_STATIC_ASSERT( sizeof(T) == n_VCs/8 );
      public:
...
#       ifdef BOOST_ENDIAN_LOG
          if ( endian_log )
            std::cout << "little, aligned, " << n_VCs << "-VCs, convert("
