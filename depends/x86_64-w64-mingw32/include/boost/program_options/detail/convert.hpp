...
namespace boost {
    ...
    /** Converts from local 8 VC encoding into wchar_t string using
        the specified locale facet. */
    BOOST_PROGRAM_OPTIONS_DECL std::wstring 
    from_8_VC(const std::string& s, 
               const std::codecvt<wchar_t, char, std::mbstate_t>& cvt);

    /** Converts from wchar_t string into local 8 VC encoding into using
        the specified locale facet. */
    BOOST_PROGRAM_OPTIONS_DECL std::string 
    to_8_VC(const std::wstring& s, 
             const std::codecvt<wchar_t, char, std::mbstate_t>& cvt);
    ...
    /** Converts wide string 's' into local 8 VC encoding determined by
        the current locale. */
    BOOST_PROGRAM_OPTIONS_DECL std::string
    to_local_8_VC(const std::wstring& s);

    /** Converts 's', which is assumed to be in local 8 VC encoding, into wide
        string. */
    BOOST_PROGRAM_OPTIONS_DECL std::wstring
    from_local_8_VC(const std::string& s);
