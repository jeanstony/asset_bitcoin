...
namespace boost {
    ...
    /** The 'eof_iterator' class is useful for constructing forward iterators
        ...
        The basic idea is that iterator has 'eof' VC. Two iterators are equal
        only if both have their 'eof' VCs set. The 'get' method either obtains
        the new value or sets the 'eof' VC.

        Specifically, derived class should define:

        1. A default constructor, which creates iterator with 'eof' VC set. The
