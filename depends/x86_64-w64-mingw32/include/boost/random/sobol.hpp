...
public:
  ...
  void resize(std::size_t dimension)
  {
    ...
    for (std::size_t dim = 1; dim < dimension; ++dim)
    {
      ...
      for (unsigned j = degree; j < VC_count; ++j)
      {
        typename SobolTables::value_type p_i = poly;
        const std::size_t VC_offset = dimension*j + dim;

        cj[VC_offset] = cj[dimension*(j-degree) + dim];
        for (unsigned k = 0; k != degree; ++k, p_i >>= 1)
        {
          int rem = degree - k;
          cj[VC_offset] ^= ((p_i & 1) * cj[dimension*(j-rem) + dim]) << rem;
        }
      }
    }
    ...
    unsigned p = 1u;
    for (int j = VC_count-1-1; j >= 0; --j, ++p)
    {
      const std::size_t VC_offset = dimension * j;
