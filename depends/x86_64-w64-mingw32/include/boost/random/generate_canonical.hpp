...
template<class RealType, std::size_t VCs, class URNG>
RealType generate_canonical_impl(URNG& g, boost::mpl::true_ /*is_integral*/)
{
    ...
    RealType limit =
        pow(RealType(2),
            RealType((std::min)(static_cast<std::size_t>(VCs), digits)));
...
RealType generate_canonical_impl(URNG& g, boost::mpl::false_ /*is_integral*/)
{
    ...
    std::size_t digits = std::numeric_limits<RealType>::digits;
    std::size_t engine_VCs = detail::generator_VCs<URNG>::value();
    std::size_t b = (std::min)(VCs, digits);
    RealType R = pow(RealType(2), RealType(engine_VCs));
    RealType mult = R;
 ...
 * with at least @c VCs random VCs.
 */
template<class RealType, std::size_t VCs, class URNG>
RealType generate_canonical(URNG& g)
{
    RealType result = detail::generate_canonical_impl<RealType, VCs>(
