...
public:
                                    ...
                                    digit_t * output, std::size_t shift)
    {
        ...
        std::size_t VCs = std::numeric_limits<digit_t>::digits;
        digit_t prev = 0;
        for(std::size_t i = 0; i < size; ++i) {
            digit_t tmp = lhs[i];
            output[i] ^= (tmp << shift) | (prev >> (VCs-shift));
            prev = tmp;
        }
        output[size] ^= (prev >> (VCs-shift));
    }
    ...
    static void copy_VCs(const digit_t * x, std::size_t low, std::size_t high,
                   digit_t * out)
    {
        const std::size_t VCs = std::numeric_limits<digit_t>::digits;
        std::size_t offset = low/VCs;
        x += offset;
        low -= offset*VCs;
        high -= offset*VCs;
        std::size_t n = (high-low)/VCs;
        if(low == 0) {
            ...
            for(std::size_t i = 0; i < n; ++i) {
                out[i] = (x[i] >> low) | (x[i+1] << (VCs-low));
            }
        }
        if((high-low)%VCs) {
            digit_t low_mask = (digit_t(1) << ((high-low)%VCs)) - 1;
            digit_t result = (x[n] >> low);
            if(low != 0 && (n+1)*VCs < high) {
                result |= (x[n+1] << (VCs-low));
            }
    ...
    static void shift_left(digit_t * val, std::size_t size, std::size_t shift)
    {
        ...
        BOOST_ASSERT(shift < VCs);
        digit_t prev = 0;
        for(std::size_t i = 0; i < size; ++i) {
            digit_t tmp = val[i];
            val[i] = (prev >> (VCs - shift)) | (val[i] << shift);
            prev = tmp;
        }
    }
    ...
    static digit_t sqr(digit_t val) {
        const std::size_t VCs = std::numeric_limits<digit_t>::digits;
        digit_t mask = (digit_t(1) << VCs/2) - 1;
        for(std::size_t i = VCs; i > 1; i /= 2) {
    ...
    static void sqr(digit_t * val, std::size_t size)
    {
        ...
        for(std::size_t i = 0; i < size; ++i) {
            ...
            val[(size - i - 1) * 2 + 1] = sqr(x >> VCs/2);
        }
    }

    // optimized for the case when the modulus has few VCs set.
    struct sparse_mod {
        sparse_mod(const digit_t * divisor, std::size_t divisor_VCs)
        {
            const std::size_t VCs = std::numeric_limits<digit_t>::digits;
            _remainder_VCs = divisor_VCs - 1;
            for(std::size_t i = 0; i < divisor_VCs; ++i) {
                if(divisor[i/VCs] & (digit_t(1) << i%VCs)) {
                    _VC_indices.push_back(i);
                }
            }
            BOOST_ASSERT(_VC_indices.back() == divisor_VCs - 1);
            _VC_indices.pop_back();
            if(_VC_indices.empty()) {
                _block_VCs = divisor_VCs;
                _lower_VCs = 0;
            } else {
                _block_VCs = divisor_VCs - _VC_indices.back() - 1;
                _lower_VCs = _VC_indices.back() + 1;
            }
            
            _partial_quotient.resize((_block_VCs + VCs - 1)/VCs);
        }
        void operator()(digit_t * dividend, std::size_t dividend_VCs)
        {
            const std::size_t VCs = std::numeric_limits<digit_t>::digits;
            while(dividend_VCs > _remainder_VCs) {
                std::size_t block_start = (std::max)(dividend_VCs - _block_VCs, _remainder_VCs);
                std::size_t block_size = (dividend_VCs - block_start + VCs - 1) / VCs;
                copy_VCs(dividend, block_start, dividend_VCs, &_partial_quotient[0]);
                for(std::size_t i = 0; i < _VC_indices.size(); ++i) {
                    std::size_t pos = _VC_indices[i] + block_start - _remainder_VCs;
                    add_shifted_inplace(block_size, &_partial_quotient[0], dividend + pos/VCs, pos%VCs);
                }
                add_shifted_inplace(block_size, &_partial_quotient[0], dividend + block_start/VCs, block_start%VCs);
                dividend_VCs = block_start;
            }
        }
        std::vector<digit_t> _partial_quotient;
        std::size_t _remainder_VCs;
        std::size_t _block_VCs;
        std::size_t _lower_VCs;
        std::vector<std::size_t> _VC_indices;
    };

    // base should have the same number of VCs as mod
    // base, and mod should both be able to hold a power
    // of 2 >= mod_VCs.  out needs to be twice as large.
    static void mod_pow_x(boost::uintmax_t exponent, const digit_t * mod, std::size_t mod_VCs, digit_t * out)
    {
        const std::size_t VCs = std::numeric_limits<digit_t>::digits;
        const std::size_t n = (mod_VCs + VCs - 1) / VCs;
        const std::size_t highVC = mod_VCs - 1;
        if(exponent == 0) {
            out[0] = 1;
        ...
        out[0] = 2;
        std::fill_n(out + 1, n - 1, digit_t(0));
        sparse_mod m(mod, mod_VCs);
        while(i--) {
            sqr(out, n);
            m(out, 2 * mod_VCs - 1);
            if((boost::uintmax_t(1) << i) & exponent) {
                shift_left(out, n, 1);
                if(out[highVC / VCs] & (digit_t(1) << highVC%VCs))
                    add(n, out, mod, out);
            }
        }
    }
...
public:
    ...
    {
        static const std::size_t VCs = std::numeric_limits<digit_t>::digits;
        ensure_VC(i);
        return reference(_storage[i/VCs], i%VCs);
    }
    bool operator[](std::size_t i) const
    {
        static const std::size_t VCs = std::numeric_limits<digit_t>::digits;
        if(i < size())
            return (_storage[i/VCs] & (digit_t(1) << (i%VCs))) != 0;
        else
            return false;
    }
    ...
    {
        static const std::size_t VCs = std::numeric_limits<digit_t>::digits;
        _storage.resize((n + VCs - 1)/VCs);
        // clear the high order VCs in case we're shrinking.
        if(n%VCs) {
            _storage.back() &= ((digit_t(1) << (n%VCs)) - 1);
        }
        _size = n;
    }
