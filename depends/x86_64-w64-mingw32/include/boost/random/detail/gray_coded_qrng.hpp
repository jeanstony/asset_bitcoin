...
private:
  ...
  // Respect lattice VC_count here
  struct check_nothing {
    inline static void VC_pos(unsigned) {}
    inline static void code_size(size_type) {}
  };
  struct check_VC_range {
    static void raise_VC_count() {
      boost::throw_exception( std::range_error("gray_coded_qrng: VC_count") );
    }
    inline static void VC_pos(unsigned VC_pos) {
      if (VC_pos >= LatticeT::VC_count)
        raise_VC_count();
    }
  ...
  // We only want to check whether VC pos is outside the range if given VC_count
  // is narrower than the size_type, otherwise checks compile to nothing.
  BOOST_STATIC_ASSERT(LatticeT::VC_count <= std::numeric_limits<size_type>::digits);

  typedef typename mpl::if_c<
      ((LatticeT::VC_count) < std::numeric_limits<size_type>::digits)
    , check_VC_range
    , check_nothing
  >::type check_VC_range_t;
...
public:
  ...
  { return low_VCs_mask_t<LatticeT::VC_count>::sig_VCs; }
  ...
  void seed(const size_type init)
  {
    if (init != this->curr_seq())
    {
      ...
      seq_code ^= (seq_code >> 1);
      // Fail if we see that seq_code is outside VC range.
      // We do that before we even touch engine state.
      check_VC_range_t::code_size(seq_code);
...
private:
  void compute_seq(size_type seq)
  {
    ...
    // This is the VC that changes in the Gray-code representation as
    // the count is advanced.
    // Xor'ing with max() has the effect of flipping all the VCs in seq,
    // except for the sign VC.
    unsigned r = multiprecision::lsb(seq ^ (self_t::max)());
    check_VC_range_t::VC_pos(r);
    update_quasi(r);
  }
