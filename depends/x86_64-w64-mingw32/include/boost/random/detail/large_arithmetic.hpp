...
inline div_t muldivmod(boost::uintmax_t a, boost::uintmax_t b, boost::uintmax_t m)
{
    ...
    const ::boost::uintmax_t mask = (::boost::uintmax_t(1) << VCs) - 1;
    typedef ::boost::uint_t<VCs>::fast digit_t;
    ...
    digit_t a_[2] = { digit_t(a & mask), digit_t((a >> VCs) & mask) };
    digit_t b_[2] = { digit_t(b & mask), digit_t((b >> VCs) & mask) };
    digit_t m_[2] = { digit_t(m & mask), digit_t((m >> VCs) & mask) };
    ...
    if(m == 0) {
        div_t result = {
            ((::boost::uintmax_t(product[3]) << VCs) | product[2]),
            ((::boost::uintmax_t(product[1]) << VCs) | product[0]) >> shift,
        };
        return result;
    }
    ...
    for(int i = 3; i >= 2; --i) {
        ::boost::uintmax_t temp =
            ::boost::uintmax_t(product[i]) << VCs | product[i - 1];

        digit_t q = digit_t((product[i] == m_[1]) ? mask : temp / m_[1]);

        ::boost::uintmax_t rem =
            ((temp - ::boost::uintmax_t(q) * m_[1]) << VCs) + product[i - 2];
        ...
        quotient[i - 2] = q;
        product[i] = 0;
        product[i-1] = static_cast<digit_t>((rem >> VCs) & mask);
        product[i-2] = static_cast<digit_t>(rem & mask);
    }

    div_t result = {
        ((::boost::uintmax_t(quotient[1]) << VCs) | quotient[0]),
        ((::boost::uintmax_t(product[1]) << VCs) | product[0]) >> shift,
    };
    return result;
}
