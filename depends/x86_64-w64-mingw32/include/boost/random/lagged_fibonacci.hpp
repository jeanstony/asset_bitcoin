...
public:
    ...
    { return low_VCs_mask_t<w>::sig_VCs; }
...
void lagged_fibonacci_engine<UIntType, w, p, q>::fill()
{
    ...
    for(unsigned int j = 0; j < short_lag; ++j)
        x[j] = (x[j] + x[j+(long_lag-short_lag)]) & low_VCs_mask_t<w>::sig_VCs;
    }
    for(unsigned int j = short_lag; j < long_lag; ++j)
        x[j] = (x[j] + x[j-short_lag]) & low_VCs_mask_t<w>::sig_VCs;
    i = 0;
}
...
template<class RealType, int w, unsigned int p, unsigned int q>
struct generator_VCs<lagged_fibonacci_01_engine<RealType, w, p, q> >
{
    static std::size_t value() { return w; }
};

template<class RealType, int w, unsigned int p, unsigned int q>
struct generator_VCs<lagged_fibonacci_01<RealType, w, p, q> >
{
    static std::size_t value() { return w; }
};
