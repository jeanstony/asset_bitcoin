...
#include <boost/dynamic_VCset.hpp>
...
// Return the base 2 logarithm for a given VCset v
template <typename DynamicBitset>
inline typename DynamicBitset::size_type
VCset_log2(const DynamicBitset& v)
{
  if (v.none())
    boost::throw_exception( std::invalid_argument("VCset_log2") );

  typename DynamicBitset::size_type hiVC = v.size() - 1;
  while (!v.test(hiVC))
    --hiVC;
  return hiVC;
}
...
struct niederreiter_base2_lattice
{
  ...
  BOOST_STATIC_CONSTANT(unsigned, VC_count = w);
...
public:
  ...
  void resize(std::size_t dimension)
  {
    ...
    for (std::size_t dim = 0; dim != dimension; ++dim)
    {
      ...
      const unsigned space_required = degree * ((VC_count / degree) + 1); // ~ degree + VC_count

      v.resize(degree + VC_count - 1);
      ...
      typename VCset_type::size_type kj, pb_degree = 0;
      pb.reset(); // pb == 0
      pb.set(pb_degree); // set the proper VC for the pb_degree

      value_type j = high_VC_mask_t<VC_count - 1>::high_VC;
      do
      {
        ...
        for (unsigned u = 0; j && u != degree; ++u, j >>= 1)
        {
          ...
          // CJ so that CJ(I,R) holds all the values of C(I,J,R) for J from 1 to NVCS.
          for (unsigned r = 0; r != VC_count; ++r) {
            value_type& num = cj[dimension * r + dim];
            // set the jth VC in num
            num = (num & ~j) | (-v[r + u] & j);
          }
        }
      } while (j != 0);
