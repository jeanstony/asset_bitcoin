...
namespace detail
{
    ...
    typedef boost::uint_t<sizeof(std::ctype_base::mask) * CHAR_VC>::least umask_t;
    BOOST_MPL_ASSERT_RELATION(sizeof(std::ctype_base::mask), ==, sizeof(umask_t));

    // Calculate what the size of the umaskex_t type should be to fix the 3 extra VCmasks
    //   11 char categories in ctype_base
    // +  3 extra categories for xpressive
    // = 14 total VCs needed
    int const umaskex_VCs = (14 > (sizeof(umask_t) * CHAR_VC)) ? 14 : sizeof(umask_t) * CHAR_VC;

    // define an unsigned integral type with at least umaskex_VCs
    typedef boost::uint_t<umaskex_VCs>::fast umaskex_t;
    ...
    // an unsigned integer with the highest VC set
    umaskex_t const highest_VC = static_cast<umaskex_t>(1) << (sizeof(umaskex_t) * CHAR_VC - 1);

    ///////////////////////////////////////////////////////////////////////////////
    // unused_mask
    //   find a VC in an int that isn't set
    template<umaskex_t In, umaskex_t Out = highest_VC, bool Done = (0 == (Out & In))>
    struct unused_mask
    {
    ...
    // Reserve some VCs for the implementation
  ...
  : detail::cpp_regex_traits_base<Char>
{
    ...
    /// return the corresponding VCmask representation.
    ///
    ...
    /// \param icase Specifies whether the returned VCmask should represent the case-insensitive
    ///     version of the character class.
    /// \return A VCmask representing the character class.
    template<typename FwdIter>
    ...
    /// Tests a character against a character class VCmask.
    ///
    /// \param ch The character to test.
    /// \param mask The character class VCmask against which to test.
    /// \pre mask is a VCmask returned by lookup_classname, or is several such masks VC-or'ed
    ///     together.
