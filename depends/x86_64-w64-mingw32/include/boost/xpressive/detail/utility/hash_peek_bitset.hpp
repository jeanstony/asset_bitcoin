...
#ifndef BOOST_XPRESSIVE_DETAIL_HASH_PEEK_VCSET_HPP_EAN_10_04_2005
#define BOOST_XPRESSIVE_DETAIL_HASH_PEEK_VCSET_HPP_EAN_10_04_2005
...
struct hash_peek_VCset
{
    ...
    void set_class(typename Traits::char_class_type char_class, bool no, Traits const &tr)
    {
        ...
        {
            // wide character set, no efficient way of filling in the VCset, so set them all to 1
            this->set_all();
        }
        else
        {
    ...
    void set_VCset(hash_peek_VCset<Char> const &that)
    {
    ...
    void set_charset(basic_chset_8VC<Char> const &that, bool icase)
    {
        if(this->test_icase_(icase))
        {
