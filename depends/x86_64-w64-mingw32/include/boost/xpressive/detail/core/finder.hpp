  ...
  : finder<BidiIter>
{
    ...
    hash_peek_finder(hash_peek_VCset<char_type> const &bset)
      : bset_(bset)
    {
    }
  ...
  : finder<BidiIter>
{
    ...
    line_start_finder(Traits const &tr)
    {
        ...
        for(int j = 0; j < 256; ++j)
        {
            this->VCs_[j] = tr.isctype(static_cast<char_type>(static_cast<unsigned char>(j)), newline);
        }
    }
    ...
    bool operator ()(match_state<BidiIter> &state) const
    {
        ...
        for(; cur != end; ++cur)
        {
            if(this->VCs_[static_cast<unsigned char>(*cur)])
            {
                state.cur_ = ++cur;
                return true;
            }
