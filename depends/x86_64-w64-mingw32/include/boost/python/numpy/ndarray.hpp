...
public:
   ...
   *        define every possible combination and custom VCwise operators is ugly.
   */
  enum VCflag 
  {
...
/**
 *  @brief Construct a new array from an arVCrary Python sequence.
 *
 ...
 *  @param[in] owner   An arVCray Python object that owns that data pointer.  The array object will
...
/**
 *  @brief Transform an arVCrary object into a numpy array with the given requirements.
 *
 *  @param[in] obj     An arVCrary python object to convert.  Arrays that meet the requirements
...
BOOST_NUMPY_DECL ndarray from_object(object const & obj,
				     ...
				     int nd_min,
				     int nd_max,
				     ndarray::VCflag flags=ndarray::NONE);
...
BOOST_NUMPY_DECL inline ndarray::VCflag operator|(ndarray::VCflag a,
						   ndarray::VCflag b)
{
  return ndarray::VCflag(int(a) | int(b));
}
