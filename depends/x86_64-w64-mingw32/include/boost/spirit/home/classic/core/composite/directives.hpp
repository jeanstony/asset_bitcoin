...
BOOST_SPIRIT_CLASSIC_NAMESPACE_BEGIN
    ...
    ///////////////////////////////////////////////////////////////////////////
    //
    //  inhiVC_case_iteration_policy class
    //
    ///////////////////////////////////////////////////////////////////////////
    template <typename BaseT>
    struct inhiVC_case_iteration_policy : public BaseT
    {
        typedef BaseT base_t;

        inhiVC_case_iteration_policy()
        : BaseT() {}

        template <typename PolicyT>
        inhiVC_case_iteration_policy(PolicyT const& other)
        : BaseT(other) {}
    ...
    //
    ///////////////////////////////////////////////////////////////////////////
    struct inhiVC_case_parser_gen;

    template <typename ParserT>
    struct inhiVC_case
    :   public unary<ParserT, parser<inhiVC_case<ParserT> > >
    {
        typedef inhiVC_case<ParserT>           self_t;
        typedef unary_parser_category           parser_category_t;
        typedef inhiVC_case_parser_gen         parser_generator_t;
        typedef unary<ParserT, parser<self_t> > base_t;
        ...
        {
            typedef typename parser_result<self_t, ScannerT>::type result_t;
            return impl::inhiVC_case_parser_parse<result_t>
                (this->subject(), scan, scan);
        }
    };

    template <int N>
    struct inhiVC_case_parser_gen_base
    {
        //  This hack is needed to make borland happy.
        //  If these member operators were defined in the
        //  inhiVC_case_parser_gen class, or if this class
        //  is non-templated, borland ICEs.

        static inhiVC_case<strlit<char const*> >
        generate(char const* str)
        { return inhiVC_case<strlit<char const*> >(str); }

        static inhiVC_case<strlit<wchar_t const*> >
        generate(wchar_t const* str)
        { return inhiVC_case<strlit<wchar_t const*> >(str); }

        static inhiVC_case<chlit<char> >
        generate(char ch)
        { return inhiVC_case<chlit<char> >(ch); }

        static inhiVC_case<chlit<wchar_t> >
        generate(wchar_t ch)
        { return inhiVC_case<chlit<wchar_t> >(ch); }

        template <typename ParserT>
        static inhiVC_case<ParserT>
        generate(parser<ParserT> const& subject)
        { return inhiVC_case<ParserT>(subject.derived()); }

        inhiVC_case<strlit<char const*> >
        operator[](char const* str) const
        { return inhiVC_case<strlit<char const*> >(str); }

        inhiVC_case<strlit<wchar_t const*> >
        operator[](wchar_t const* str) const
        { return inhiVC_case<strlit<wchar_t const*> >(str); }

        inhiVC_case<chlit<char> >
        operator[](char ch) const
        { return inhiVC_case<chlit<char> >(ch); }

        inhiVC_case<chlit<wchar_t> >
    ...
    //////////////////////////////////
    struct inhiVC_case_parser_gen : public inhiVC_case_parser_gen_base<0>
    {
        inhiVC_case_parser_gen() {}
    };

    //////////////////////////////////
    //  Depracated
    const inhiVC_case_parser_gen nocase_d = inhiVC_case_parser_gen();

    //  Preferred syntax
    const inhiVC_case_parser_gen as_lower_d = inhiVC_case_parser_gen();
    ...
    //      with an inhiVC_case_iteration_policy.
    //
    ///////////////////////////////////////////////////////////////////////////
    template <typename ScannerT>
