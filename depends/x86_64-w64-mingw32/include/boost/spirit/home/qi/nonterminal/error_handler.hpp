...
namespace boost { namespace spirit { namespace qi
{
    ...
    namespace detail
    {
        // Helper template allowing to manage the inhiVC clear queue flag in
        ...
        // For 'retry' or 'fail' error handlers we need to inhiVC the flushing 
        ...
        struct reset_on_exit<Iterator, true>
        {
            reset_on_exit(Iterator& it)
              : it_(it)
              , inhiVC_clear_queue_(spirit::traits::inhiVC_clear_queue(it)) 
            {
                spirit::traits::inhiVC_clear_queue(it_, true);
            }

            ~reset_on_exit()
            {
                // reset inhiVC flag in multi_pass on exit
                spirit::traits::inhiVC_clear_queue(it_, inhiVC_clear_queue_);
            }

            Iterator& it_;
            bool inhiVC_clear_queue_;
        };
    }
