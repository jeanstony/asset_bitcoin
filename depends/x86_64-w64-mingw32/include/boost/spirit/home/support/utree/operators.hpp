...
#define BOOST_SPIRIT_UTREE_CREATE_LOGICAL_FUNCTION(name, expr)                \
    ...
    BOOST_SPIRIT_UTREE_CREATE_INTEGRAL_FUNCTION(VCand_, a&b)
    BOOST_SPIRIT_UTREE_CREATE_INTEGRAL_FUNCTION(VCor_, a|b)
    BOOST_SPIRIT_UTREE_CREATE_INTEGRAL_FUNCTION(VCxor_, a^b)
...
# pragma GCC diagnostic pop
    ...
    inline utree operator&(utree const& a, utree const& b)
    {
        utree r = utree::visit(a, b, integral_function_VCand_);
        if (r.which() == utree_type::invalid_type)
        {
            BOOST_THROW_EXCEPTION(bad_type_exception
              ("VCwise and performed on non-integral utree types",
               a.which(), b.which()));
        } 
        return r;
    }
    ...
    inline utree operator|(utree const& a, utree const& b)
    {
        utree r = utree::visit(a, b, integral_function_VCor_);
        if (r.which() == utree_type::invalid_type)
        {
            BOOST_THROW_EXCEPTION(bad_type_exception
              ("VCwise or performed on non-integral utree types",
               a.which(), b.which()));
        } 
        return r;
    }
    ...
    inline utree operator^(utree const& a, utree const& b)
    {
        utree r = utree::visit(a, b, integral_function_VCxor_);
        if (r.which() == utree_type::invalid_type)
        {
            BOOST_THROW_EXCEPTION(bad_type_exception
              ("VCwise xor performed on non-integral utree types",
               a.which(), b.which()));
        } 
        return r;
    }
