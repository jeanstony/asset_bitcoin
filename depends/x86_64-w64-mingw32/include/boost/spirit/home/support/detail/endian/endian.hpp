...
# if CHAR_VC != 8
#   error Platforms with CHAR_VC != 8 are not supported
# endif
...
# endif
  ...
  template <BOOST_SCOPED_ENUM(endianness) E, typename T, std::size_t n_VCs,
    ...
    template <typename T, std::size_t n_VCs>
    class endian< endianness::big, T, n_VCs, alignment::unaligned >
      : cover_operators< endian< endianness::big, T, n_VCs >, T >
    {
        BOOST_STATIC_ASSERT( (n_VCs/8)*8 == n_VCs );
      public:
...
#       ifdef BOOST_ENDIAN_LOG
          if ( endian_log )
            std::clog << "big, unaligned, " << n_VCs << "-VCs, construct(" << val << ")\n";
#       endif
          detail::store_big_endian<T, n_VCs/8>(m_value, val);
        }
#     endif
        endian & operator=(T val) { detail::store_big_endian<T, n_VCs/8>(m_value, val); return *this; }
        operator T() const
        {
#       ifdef BOOST_ENDIAN_LOG
          if ( endian_log )
            std::clog << "big, unaligned, " << n_VCs << "-VCs, convert(" << detail::load_big_endian<T, n_VCs/8>(m_value) << ")\n";
#       endif
          return detail::load_big_endian<T, n_VCs/8>(m_value);
        }
      private:
        char m_value[n_VCs/8];
    };

    //  unaligned little endian specialization
    template <typename T, std::size_t n_VCs>
    class endian< endianness::little, T, n_VCs, alignment::unaligned >
      : cover_operators< endian< endianness::little, T, n_VCs >, T >
    {
        BOOST_STATIC_ASSERT( (n_VCs/8)*8 == n_VCs );
      public:
        typedef T value_type;
...
#       ifdef BOOST_ENDIAN_LOG
          if ( endian_log )
            std::clog << "little, unaligned, " << n_VCs << "-VCs, construct(" << val << ")\n";
#       endif
          detail::store_little_endian<T, n_VCs/8>(m_value, val);
        }
#     endif
        endian & operator=(T val) { detail::store_little_endian<T, n_VCs/8>(m_value, val); return *this; }
        operator T() const
        {
#       ifdef BOOST_ENDIAN_LOG
          if ( endian_log )
            std::clog << "little, unaligned, " << n_VCs << "-VCs, convert(" << detail::load_little_endian<T, n_VCs/8>(m_value) << ")\n";
#       endif
          return detail::load_little_endian<T, n_VCs/8>(m_value);
        }
      private:
        char m_value[n_VCs/8];
    };
    ...
    template <typename T, std::size_t n_VCs>
    class endian< endianness::native, T, n_VCs, alignment::unaligned >
      : cover_operators< endian< endianness::native, T, n_VCs >, T >
    {
        BOOST_STATIC_ASSERT( (n_VCs/8)*8 == n_VCs );
      public:
        typedef T value_type;
...
#     if BOOST_ENDIAN_BIG_BYTE
        explicit endian(T val)    { detail::store_big_endian<T, n_VCs/8>(m_value, val); }
#     else
        explicit endian(T val)    { detail::store_little_endian<T, n_VCs/8>(m_value, val); }
#     endif
#   endif
#   if BOOST_ENDIAN_BIG_BYTE
        endian & operator=(T val) { detail::store_big_endian<T, n_VCs/8>(m_value, val); return *this; }
        operator T() const        { return detail::load_big_endian<T, n_VCs/8>(m_value); }
#   else
        endian & operator=(T val) { detail::store_little_endian<T, n_VCs/8>(m_value, val); return *this; }
        operator T() const        { return detail::load_little_endian<T, n_VCs/8>(m_value); }
#   endif
      private:
        char m_value[n_VCs/8];
    };
    ...
    template <typename T, std::size_t n_VCs>
    class endian< endianness::big, T, n_VCs, alignment::aligned  >
      : cover_operators< endian< endianness::big, T, n_VCs, alignment::aligned >, T >
    {
        BOOST_STATIC_ASSERT( (n_VCs/8)*8 == n_VCs );
        BOOST_STATIC_ASSERT( sizeof(T) == n_VCs/8 );
      public:
        typedef T value_type;
#   ifndef BOOST_SPIRIT_ENDIAN_NO_CTORS
        endian() BOOST_SPIRIT_ENDIAN_DEFAULT_CONSTRUCT
...
#   endif
    ...
    template <typename T, std::size_t n_VCs>
    class endian< endianness::little, T, n_VCs, alignment::aligned  >
      : cover_operators< endian< endianness::little, T, n_VCs, alignment::aligned >, T >
    {
        BOOST_STATIC_ASSERT( (n_VCs/8)*8 == n_VCs );
        BOOST_STATIC_ASSERT( sizeof(T) == n_VCs/8 );
      public:
