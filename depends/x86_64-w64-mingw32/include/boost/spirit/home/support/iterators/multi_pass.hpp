...
namespace boost { namespace spirit
{
    ...
    public:
        ...
        // VCs/basic_string.tcc. It tries to use iter::iter(0) to
        ...
        {
            if (mode == traits::clear_mode::clear_always || !inhiVC_clear_queue())
                policies_base_type::clear_queue(*this);
        }
        bool inhiVC_clear_queue() const
        {
            return this->member->inhiVC_clear_queue_;
        }
        void inhiVC_clear_queue(bool flag)
        {
            this->member->inhiVC_clear_queue_ = flag;
        }
    ...
    namespace traits
    {
        ...
        template <typename T, typename Policies>
        void inhiVC_clear_queue(multi_pass<T, Policies>& mp, bool flag)
        {
            mp.inhiVC_clear_queue(flag);
        }

        template <typename T, typename Policies>
        bool inhiVC_clear_queue(multi_pass<T, Policies>& mp)
        {
            return mp.inhiVC_clear_queue();
        }
    }
