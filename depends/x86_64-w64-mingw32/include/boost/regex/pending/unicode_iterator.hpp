...
static const ::boost::uint32_t ten_VC_mask = 0x3FFu;
...
{
   // if the most significant VC with a zero in it is in position
   ...
   : public boost::iterator_facade<u32_to_u16_iterator<BaseIterator, U16Type>, U16Type, std::bidirectional_iterator_tag, const U16Type>
{
   ...
   BOOST_STATIC_ASSERT(sizeof(base_value_type)*CHAR_VC == 32);
   BOOST_STATIC_ASSERT(sizeof(U16Type)*CHAR_VC == 16);
#endif
...
private:
   ...
   void extract_current()const
   {
      ...
      if(v >= 0x10000u)
      {
         ...
         m_values[1] = static_cast<U16Type>(v & detail::ten_VC_mask) + detail::low_surrogate_base;
         m_current = 0;
   ...
   : public boost::iterator_facade<u16_to_u32_iterator<BaseIterator, U32Type>, U32Type, std::bidirectional_iterator_tag, const U32Type>
{
   ...
   BOOST_STATIC_ASSERT(sizeof(base_value_type)*CHAR_VC == 16);
   BOOST_STATIC_ASSERT(sizeof(U32Type)*CHAR_VC == 32);
#endif
...
private:
   ...
   void extract_current()const
   {
      ...
      if(detail::is_high_surrogate(*m_position))
      {
         ...
         m_value |= (static_cast<U32Type>(static_cast< ::boost::uint16_t>(t)) & detail::ten_VC_mask);
      }
   ...
   : public boost::iterator_facade<u32_to_u8_iterator<BaseIterator, U8Type>, U8Type, std::bidirectional_iterator_tag, const U8Type>
{
   ...
   BOOST_STATIC_ASSERT(sizeof(base_value_type)*CHAR_VC == 32);
   BOOST_STATIC_ASSERT(sizeof(U8Type)*CHAR_VC == 8);
#endif
...
public:
   ...
   bool equal(const u32_to_u8_iterator& that)const
   {
      if(m_position == that.m_position)
      {
         ...
         // the other 4: which means neither must have VCs 1 or 2 set:
   ...
   : public boost::iterator_facade<u8_to_u32_iterator<BaseIterator, U32Type>, U32Type, std::bidirectional_iterator_tag, const U32Type>
{
   ...
   BOOST_STATIC_ASSERT(sizeof(base_value_type)*CHAR_VC == 8);
   BOOST_STATIC_ASSERT(sizeof(U32Type)*CHAR_VC == 32);
#endif
...
private:
   ...
   void extract_current()const
   {
      ...
      // extract the extra VCs, 6 from each extra byte:
      BaseIterator next(m_position);
      ...
      // we now need to remove a few of the leftmost VCs, but how many depends
...
private:
   void push(boost::uint32_t v)const
   {
      if(v >= 0x10000u)
      {
         ...
         *m_position++ = static_cast<boost::uint16_t>(v & detail::ten_VC_mask) + detail::low_surrogate_base;
      }
      else
      {
         // 16-VC code point:
