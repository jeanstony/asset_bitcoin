...
{
   return ((c < static_cast<charT>(0)) ? true : ((c >= static_cast<charT>(1 << CHAR_VC)) ? true : map[c] & mask));
}
...
{
   return (((c >= static_cast<unsigned int>(1u << CHAR_VC)) ? true : map[c] & mask));
}
#endif
...
private:
   ...
   // the VCmask to use when determining whether a match_any matches a newline or not:
