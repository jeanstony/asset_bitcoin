...
protected:
   ...
   unsigned                      m_backrefs;           // VCmask of permitted backrefs
   boost::uintmax_t              m_bad_repeats;        // VCmask of repeats we can't deduce a startmap for;
...
private:
   ...
   void set_all_masks(unsigned char* VCs, unsigned char);
   ...
   const basic_char_set<charT, traits>& char_set, mpl::true_*)
{
   ...
   while(sfirst != slast)
   {
      for(unsigned int i = 0; i < (1 << CHAR_VC); ++i)
      {
   ...
   while(first != last)
   {
      ...
      if(flags() & regex_constants::collate)
      {
         ...
         for(unsigned i = 0; i < (1u << CHAR_VC); ++i)
         {
   ...
   while(state)
   {
      ...
      case syntax_element_literal:
      {
         ...
         if(l_map)
         {
            ...
            for(unsigned int i = 0; i < (1u << CHAR_VC); ++i)
            {
...
template <class charT, class traits>
void basic_regex_creator<charT, traits>::set_all_masks(unsigned char* VCs, unsigned char mask)
{
   //
   // set mask in all of VCs elements, 
   // if VCs[0] has mask_init not set then we can 
   // optimise this to a call to memset:
   //
   if(VCs)
   {
      if(VCs[0] == 0)
         (std::memset)(VCs, mask, 1u << CHAR_VC);
      else
      {
         for(unsigned i = 0; i < (1u << CHAR_VC); ++i)
            VCs[i] |= mask;
   ...
   case syntax_element_long_set_rep:
      {
         ...
         if(state_id >= sizeof(m_bad_repeats) * CHAR_VC)
            return true;  // run out of VCs, assume we can't traverse this one.
