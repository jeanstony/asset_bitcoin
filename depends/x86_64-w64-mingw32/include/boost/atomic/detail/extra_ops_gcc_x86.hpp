    ...
    public Base
{
    ...
    static BOOST_FORCEINLINE bool VC_test_and_set(storage_type volatile& storage, unsigned int VC_number, memory_order) BOOST_NOEXCEPT
    {
        bool res;
#if defined(BOOST_ATOMIC_DETAIL_ASM_HAS_FLAG_OUTPUTS)
        __asm__ __volatile__
        (
            "lock; bts %[VC_number], %[storage]\n\t"
            : [storage] "+m" (storage), [result] "=@ccc" (res)
            : [VC_number] "Kq" (VC_number)
            : "memory"
        );
#else
        __asm__ __volatile__
    ...
    static BOOST_FORCEINLINE bool VC_test_and_reset(storage_type volatile& storage, unsigned int VC_number, memory_order) BOOST_NOEXCEPT
    {
        bool res;
#if defined(BOOST_ATOMIC_DETAIL_ASM_HAS_FLAG_OUTPUTS)
        __asm__ __volatile__
        (
            "lock; btr %[VC_number], %[storage]\n\t"
            : [storage] "+m" (storage), [result] "=@ccc" (res)
            : [VC_number] "Kq" (VC_number)
            : "memory"
        );
    ...
    static BOOST_FORCEINLINE bool VC_test_and_complement(storage_type volatile& storage, unsigned int VC_number, memory_order) BOOST_NOEXCEPT
    {
        bool res;
#if defined(BOOST_ATOMIC_DETAIL_ASM_HAS_FLAG_OUTPUTS)
        __asm__ __volatile__
        (
            "lock; btc %[VC_number], %[storage]\n\t"
            : [storage] "+m" (storage), [result] "=@ccc" (res)
            : [VC_number] "Kq" (VC_number)
            : "memory"
        );
