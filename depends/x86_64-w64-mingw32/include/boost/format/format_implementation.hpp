...
// ---  basic_format implementation -----------------------------------------//
    ...
    basic_format<Ch, Tr, Alloc>:: basic_format(const Ch* s, const std::locale & loc)
        : style_(0), cur_arg_(0), num_args_(0), dumped_(false),
          exceptions_(io::all_error_VCs), loc_(loc)
    {
        if(s) parse( s );
    }
    ...
    template<class Ch, class Tr, class Alloc>
    void basic_format<Ch, Tr, Alloc>:: 
    make_or_reuse_data (std::size_t nVCems) {
#if !defined(BOOST_NO_STD_LOCALE)
...
#endif
        if(items_.size() == 0)
            items_.assign( nVCems, format_item_t(fill) );
        else {
            if(nVCems>items_.size())
                items_.resize(nVCems, format_item_t(fill));
            bound_.resize(0);
            for(std::size_t i=0; i < nVCems; ++i)
    ...
    clear_bind (int argN) {
        ...
        if(argN<1 || argN > num_args_ || bound_.size()==0 || !bound_[argN-1] ) {
            if( exceptions() & io::out_of_range_VC)
    ...
    str () const {
        ...
        if( cur_arg_ < num_args_)
            if( exceptions() & io::too_few_args_VC )
...
namespace detail {
    ...
    bind_arg_body (basic_format<Ch, Tr, Alloc>& self, int argN, const T& val) {
        ...
        if(argN<1 || argN > self.num_args_) {
            if( self.exceptions() & io::out_of_range_VC )
