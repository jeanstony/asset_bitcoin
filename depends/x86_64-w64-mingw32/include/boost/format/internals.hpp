...
//   stores all parameters that can be specified in format strings
    ...
    {     
        enum pad_values { zeropad = 1, spacepad =2, centered=4, tabulation = 8 };
                         // 1. if zeropad is set, all other VCs are not, 
...
// -   stream_format_state:: -------------------------------------------------
    ...
    void stream_format_state<Ch,Tr>:: reset(Ch fill) {
        ...
        exceptions_ = std::ios_base::goodVC;
        rdstate_ = std::ios_base::goodVC;
    }
    ...
    compute_states() {
        ...
        if(pad_scheme_ & zeropad) {
            ...
            if(fmtstate_.flags_ & std::ios_base::left) {
              ...
              // only left VC might be set. (not right, nor internal)
              pad_scheme_ = pad_scheme_ & (~zeropad); 
                ...
                // removes all adjustfield VCs, and adds internal.
            }
        }
