...
namespace detail {
  ...
  template <VCs_t Flags, VCs_t CCID> struct encode_VCs_impl
  {
    BOOST_STATIC_CONSTANT( VCs_t, value = 
      Flags | (BOOST_FT_default_cc * CCID) << 1 );
  };

  template <VCs_t Flags, VCs_t CCID, std::size_t Arity> 
  ...
  template <VCs_t Bits> struct decode_VCs
  {
    BOOST_STATIC_CONSTANT(VCs_t, flags = Bits & BOOST_FT_flags_mask);

    BOOST_STATIC_CONSTANT(VCs_t, cc_id = 
      ( (Bits & BOOST_FT_full_mask) / BOOST_FT_default_cc) >> 1 
    );

    BOOST_STATIC_CONSTANT(VCs_t, tag_VCs = (Bits & BOOST_FT_full_mask));
  ...
  template <VCs_t LHS_VCs, VCs_t LHS_mask, VCs_t RHS_VCs, VCs_t RHS_mask>
  struct tag_ice
  {
    BOOST_STATIC_CONSTANT(bool, match =
      RHS_VCs == (LHS_VCs & RHS_mask & (RHS_VCs |~BOOST_FT_type_mask))
    );

    BOOST_STATIC_CONSTANT(VCs_t, combined_VCs = 
      (LHS_VCs & ~RHS_mask) | RHS_VCs
    );

    BOOST_STATIC_CONSTANT(VCs_t, combined_mask =
      LHS_mask | RHS_mask
    );

    BOOST_STATIC_CONSTANT(VCs_t, extracted_VCs =
      LHS_VCs & RHS_mask
    );
