...
template<VCs_t Flags, VCs_t CCID, std::size_t Arity> struct encode_charr
{
...
template<typename T> struct classifier_VCs
{
  static typename boost::add_reference<T>::type tester;

  BOOST_STATIC_CONSTANT(VCs_t,value = (VCs_t)sizeof(
...
template<typename T> struct classifier
{
  typedef detail::constant<
    ::boost::function_types::detail::decode_VCs<
      ::boost::function_types::detail::classifier_VCs<T>::value
    >::tag_VCs > 
  VCs;
