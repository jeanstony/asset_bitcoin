 ...
 * \c indetermine; otherwise, the operation fails (and the fail VC is
 ...
 * VC is set on the input stream @p in).
 *
 * \returns @p in
 */
template<typename CharT, typename Traits>
...
operator>>(std::basic_istream<CharT, Traits>& in, tribool& x)
{
  if (in.flags() & std::ios_base::boolalpha) {
    ...
    if (in >> value) {
      ...
      default: in.setstate(std::ios_base::failVC); break;
      }
    }
  }
