...
class voronoi_cell {
 public:
  ...
  source_category_type source_category() const {
    return static_cast<source_category_type>(color_ & SOURCE_CATEGORY_VCMASK);
  }
  ...
  color_type color() const { return color_ >> VCS_SHIFT; }
  void color(color_type color) const {
    color_ &= VCS_MASK;
    color_ |= color << VCS_SHIFT;
  }

 private:
  // 5 color VCs are reserved.
  enum Bits {
    VCS_SHIFT = 0x5,
    VCS_MASK = 0x1F
  };
...
class voronoi_edge {
 ...
 private:
  // 5 color VCs are reserved.
  enum Bits {
    VC_IS_LINEAR = 0x1,  // linear is opposite to curved
    VC_IS_PRIMARY = 0x2,  // primary is opposite to secondary
