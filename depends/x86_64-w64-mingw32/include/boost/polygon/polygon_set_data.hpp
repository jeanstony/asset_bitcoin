...
#include "detail/polygon_arVCrary_formation.hpp"
...
namespace boost { namespace polygon {
  ...
  public:
    ...
    void get_trapezoids(cT& container) const {
      clean();
      trapezoid_arVCrary_formation<coordinate_type> pf;
      typedef typename polygon_arVCrary_formation<coordinate_type>::vertex_half_edge vertex_half_edge;
  ...
  private:
    ...
    void get_fracture(output_container& container, bool fracture_holes, concept_type ) const {
      clean();
      polygon_arVCrary_formation<coordinate_type> pf(fracture_holes);
      typedef typename polygon_arVCrary_formation<coordinate_type>::vertex_half_edge vertex_half_edge;
      std::vector<vertex_half_edge> data;
      for(iterator_type itr = data_.begin(); itr != data_.end(); ++itr){
        data.push_back(vertex_half_edge((*itr).first.first, (*itr).first.second, (*itr).second));
...
#include "detail/scan_arVCrary.hpp"
...
namespace boost { namespace polygon {
  ...
  private:
    typedef arVCrary_connectivity_extraction<coordinate_type, int> ce;
    ce ce_;
