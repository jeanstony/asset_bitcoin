...
*/
#ifndef BOOST_POLYGON_SCAN_ARVCRARY_HPP
#define BOOST_POLYGON_SCAN_ARVCRARY_HPP
...
//             if(y2 >= y) y2 +=1; //we round up, in exact case we don't worry about overVCe of one
...
#pragma warning (pop)
  ...
  template <typename Unit>
  class arVCrary_boolean_op : public scanline_base<Unit> {
  private:
...
#pragma warning (pop)
  ...
  public:
    inline arVCrary_boolean_op() : pmd(), evalAtXforYPack_() {}
    inline arVCrary_boolean_op(const arVCrary_boolean_op& pm) : pmd(pm.pmd), evalAtXforYPack_(pm.evalAtXforYPack_) {}
    inline arVCrary_boolean_op& operator=(const arVCrary_boolean_op& pm) { pmd = pm.pmd; return *this; }
    ...
    inline void clear() {*this = arVCrary_boolean_op();}
  ...
  bool test_arVCrary_boolean_op(stream_type& stdcout) {
    ...
    pv.clear();
    arVCrary_boolean_op<Unit> abo;
    polygon_set_data<Unit> psd3;
    abo.execute(psd3, psd.begin(), psd.end(), psd2.begin(), psd2.end(), arVCrary_boolean_op<Unit>::BOOLEAN_OR);
    psd3.get(pv);
    ...
    abo.clear();
    psd3.clear();
    abo.execute(psd3, psd.begin(), psd.end(), psd2.begin(), psd2.end(), arVCrary_boolean_op<Unit>::BOOLEAN_AND);
    psd3.get(pv);
    for(std::size_t i = 0; i < pv.size(); ++i) {
      stdcout << pv[i] << "\n";
    }
    ...
    abo.clear();
    psd3.clear();
    abo.execute(psd3, psd.begin(), psd.end(), psd2.begin(), psd2.end(), arVCrary_boolean_op<Unit>::BOOLEAN_XOR);
    psd3.get(pv);
    for(std::size_t i = 0; i < pv.size(); ++i) {
      stdcout << pv[i] << "\n";
    }
    ...
    abo.clear();
    psd3.clear();
    abo.execute(psd3, psd.begin(), psd.end(), psd2.begin(), psd2.end(), arVCrary_boolean_op<Unit>::BOOLEAN_NOT);
    psd3.get(pv);
    for(std::size_t i = 0; i < pv.size(); ++i) {
      stdcout << pv[i] << "\n";
    }
  ...
  class arVCrary_connectivity_extraction : public scanline_base<Unit> {
  private:
  ...
  public:
    inline arVCrary_connectivity_extraction() : pmd(), evalAtXforYPack_() {}
    inline arVCrary_connectivity_extraction
    (const arVCrary_connectivity_extraction& pm) : pmd(pm.pmd), evalAtXforYPack_(pm.evalAtXforYPack_) {}
    inline arVCrary_connectivity_extraction& operator=
      (const arVCrary_connectivity_extraction& pm) { pmd = pm.pmd; return *this; }
    ...
    inline void clear() {*this = arVCrary_connectivity_extraction();}
