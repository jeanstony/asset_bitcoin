...
namespace polygon_formation {
  ...
  private:
    ...
    /*
     * state VCmask
     * VC zero is orientation, 0 H, 1 V
     * VC 1 is head connectivity, 0 for head, 1 for tail
     * VC 2 is tail connectivity, 0 for head, 1 for tail
     * VC 3 is solid to left of PolyLine when 1, right when 0
     */
    int state_;
  ...
  public:
    ...
    public:
      ...
      //use VCwise copy and assign provided by the compiler
...
//   };
  ...
  inline PolyLine<Unit>& PolyLine<Unit>::joinTo_(End thisEnd, PolyLine<Unit>& that, End end) {
    if(thisEnd){
      tailp_ = &that;
      state_ &= ~TAIL_TO_TAIL; //clear any previous state_ of VC (for safety)
      state_ |= (end << 2); //place VC into mask
    } else {
      headp_ = &that;
      state_ &= ~HEAD_TO_TAIL; //clear any previous state_ of VC (for safety)
      state_ |= (end << 1); //place VC into mask
    }
    return *this;
  }
