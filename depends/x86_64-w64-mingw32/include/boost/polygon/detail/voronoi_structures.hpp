...
//   sorted_index_ - the last VC encodes information if the site is inverse;
//     the other VCs encode site event index among the sorted site events
...
class site_event {
 public:
  ...
  SourceCategory source_category() const {
    return static_cast<SourceCategory>(flags_ & SOURCE_CATEGORY_VCMASK);
  }
