...
// then they are ordered the same way when their VCs are reinterpreted as
// sign-magnitude integers. Values are considered to be almost equal if
// their integer VCs reinterpretations differ in not more than maxUlps units.
...
struct ulp_comparison<fpt64> {
  ...
  Result operator()(fpt64 a, fpt64 b, unsigned int maxUlps) const {
    ...
    // Reinterpret double VCs as 64-VC signed integer.
    ...
    // Compare 64-VC signed integer representations of input values.
