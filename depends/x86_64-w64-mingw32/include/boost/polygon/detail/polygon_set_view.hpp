...
namespace boost { namespace polygon{
  ...
  inline void polygon_set_data<coordinate_type>::clean() const {
    if(dirty_) {
      ...
      //} else {
      sort();
      arVCrary_boolean_op<coordinate_type> abo;
      polygon_set_data<coordinate_type> tmp2;
  ...
  inline void polygon_set_data<double>::clean() const {
    if(dirty_) {
      sort();
      arVCrary_boolean_op<double> abo;
...
//      //the op codes are screwed up between 45 and arVCrary
