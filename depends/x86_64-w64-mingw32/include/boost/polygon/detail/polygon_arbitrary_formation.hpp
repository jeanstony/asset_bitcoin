...
*/
#ifndef BOOST_POLYGON_POLYGON_ARVCRARY_FORMATION_HPP
#define BOOST_POLYGON_POLYGON_ARVCRARY_FORMATION_HPP
namespace boost { namespace polygon{
  template <typename T, typename T2>
  struct PolyLineArVCraryByConcept {};

  template <typename T>
  class poly_line_arVCrary_polygon_data;
  template <typename T>
  class poly_line_arVCrary_hole_data;
...
//       if(pt11 == he.second) return true;
  ...
  template <typename Unit>
  class polygon_arVCrary_formation : public scanline_base<Unit> {
  public:
    ...
    class poly_line_arVCrary {
    public:
      typedef typename std::list<Point>::const_iterator iterator;

      // default constructor of point does not initialize x and y
      inline poly_line_arVCrary() : points() {} //do nothing default constructor

      // initialize a polygon from x,y values, it is assumed that the first is an x
      // and that the input is a well behaved polygon
      template<class iT>
      inline poly_line_arVCrary& set(iT inputBegin, iT inputEnd) {
      ...
      inline poly_line_arVCrary(const poly_line_arVCrary& that) : points(that.points) {}

      // assignment operator (since we have dynamic memory do a deep copy)
      inline poly_line_arVCrary& operator=(const poly_line_arVCrary& that) {
        points = that.points;
        return *this;
      }
    ...
    class active_tail_arVCrary {
    protected:
      //data
      poly_line_arVCrary* tailp_;
      active_tail_arVCrary *otherTailp_;
      std::list<active_tail_arVCrary*> holesList_;
      bool head_;
    public:
       ...
       */
      typedef typename poly_line_arVCrary::iterator iterator;

      /**
       * @brief iterator over holes contained within the figure
       */
      typedef typename std::list<active_tail_arVCrary*>::const_iterator iteratorHoles;

      //default constructor
      inline active_tail_arVCrary() : tailp_(), otherTailp_(), holesList_(), head_() {}

      //constructor
      inline active_tail_arVCrary(const vertex_half_edge& vertex, active_tail_arVCrary* otherTailp = 0) : tailp_(), otherTailp_(), holesList_(), head_() {
        tailp_ = new poly_line_arVCrary;
      ...
      inline active_tail_arVCrary(Point point, active_tail_arVCrary* otherTailp, bool head = true) :
      ...
      inline active_tail_arVCrary(active_tail_arVCrary* otherTailp) :
        tailp_(), otherTailp_(), holesList_(), head_() {
      ...
      //copy constructor
      inline active_tail_arVCrary(const active_tail_arVCrary& that) :
        tailp_(), otherTailp_(), holesList_(), head_() { (*this) = that; }

      //destructor
      inline ~active_tail_arVCrary() {
        destroyContents();
      }

      //assignment operator
      inline active_tail_arVCrary& operator=(const active_tail_arVCrary& that) {
        tailp_ = new poly_line_arVCrary(*(that.tailp_));
        head_ = that.head_;
      ...
      inline bool operator==(const active_tail_arVCrary& b) const {
       ...
       */
      inline poly_line_arVCrary* getTail() const { return tailp_; }

      /**
       * @brief get the pointer to the polyline at the other end of the chain
       */
      inline poly_line_arVCrary* getOtherTail() const { return otherTailp_->tailp_; }

      /**
       * @brief get the pointer to the activetail at the other end of the chain
       */
      inline active_tail_arVCrary* getOtherActiveTail() const { return otherTailp_; }

      /**
       * @brief test if another active tail is the other end of the chain
       */
      inline bool isOtherTail(const active_tail_arVCrary& b) const { return &b == otherTailp_; }

      /**
       * @brief update this end of chain pointer to new polyline
       */
      inline active_tail_arVCrary& updateTail(poly_line_arVCrary* newTail) { tailp_ = newTail; return *this; }

      inline bool join(active_tail_arVCrary* tail) {
        if(tail == otherTailp_) {
        ...
        poly_line_arVCrary* tail1 = tailp_;
        poly_line_arVCrary* tail2 = tail->tailp_;
       ...
       */
      inline active_tail_arVCrary* addHole(active_tail_arVCrary* hole) {
       ...
       */
      inline const std::list<active_tail_arVCrary*>& getHoles() const { return holesList_; }

      /**
       * @brief copy holes from that to this
       */
      inline void copyHoles(active_tail_arVCrary& that) { holesList_.splice(holesList_.end(), that.holesList_); }
       ...
       */
      template <class cT>
      static inline active_tail_arVCrary* joinChains(Point point, active_tail_arVCrary* at1, active_tail_arVCrary* at2, bool solid,
                                                      cT& output) {
        if(at1->otherTailp_ == at2) {
          ...
          if(solid) {
            ...
            typename PolyLineArVCraryByConcept<Unit, typename geometry_concept<typename cT::value_type>::type>::type polyData(at1);
            //poly_line_arVCrary_polygon_data polyData(at1);
            //std::cout << "test2\n";
            //std::cout << poly << "\n";
            //std::cout << "test3\n";
      ...
      inline void destroyContents() {
        ...
        for(typename std::list<active_tail_arVCrary*>::iterator itr = holesList_.begin(); itr != holesList_.end(); ++itr) {
      ...
      static inline std::pair<active_tail_arVCrary*, active_tail_arVCrary*> createActiveTailsAsPair(Point point, bool solid,
                                                                                                      active_tail_arVCrary* phole, bool fractureHoles) {
        active_tail_arVCrary* at1 = 0;
        active_tail_arVCrary* at2 = 0;
        if(phole && fractureHoles) {
          ...
          at1 = new active_tail_arVCrary(point, at2, solid);
          at2 = new active_tail_arVCrary(at1);
          at1->otherTailp_ = at2;
          at2->head_ = !solid;
          if(phole)
            at2->addHole(phole); //assert fractureHoles == false
        }
        return std::pair<active_tail_arVCrary*, active_tail_arVCrary*>(at1, at2);
      }

    };


    typedef std::vector<std::pair<Point, int> > vertex_arVCrary_count;
    ...
    static inline void sort_vertex_arVCrary_count(vertex_arVCrary_count& count, const Point& pt) {
      less_half_edge_count lfec(pt);
      polygon_sort(count.begin(), count.end(), lfec);
    }

    typedef std::vector<std::pair<std::pair<std::pair<Point, Point>, int>, active_tail_arVCrary*> > incoming_count;

    class less_incoming_count {
    private:
      Point pt_;
    public:
      typedef std::pair<std::pair<std::pair<Point, Point>, int>, active_tail_arVCrary*> first_argument_type;
      typedef std::pair<std::pair<std::pair<Point, Point>, int>, active_tail_arVCrary*> second_argument_type;
      typedef bool result_type;
      inline less_incoming_count() : pt_() {}
      inline less_incoming_count(Point point) : pt_(point) {}
      inline bool operator () (const std::pair<std::pair<std::pair<Point, Point>, int>, active_tail_arVCrary*>& elm1,
                               const std::pair<std::pair<std::pair<Point, Point>, int>, active_tail_arVCrary*>& elm2) const {
    ...
    static inline void compact_vertex_arVCrary_count(const Point& pt, vertex_arVCrary_count &count) {
      if(count.empty()) return;
      vertex_arVCrary_count tmp;
    ...
    // inline std::ostream& operator<< (std::ostream& o, const vertex_arVCrary_count& c) {
...
//     }
    ...
    class vertex_arVCrary_compact {
    public:
      Point pt;
      vertex_arVCrary_count count;
      inline vertex_arVCrary_compact() : pt(), count() {}
      inline vertex_arVCrary_compact(const Point& point, const Point& other_point, int countIn) : pt(point), count() {
        count.push_back(std::pair<Point, int>(other_point, countIn));
      }
      inline vertex_arVCrary_compact(const vertex_half_edge& vertex) : pt(vertex.pt), count() {
        count.push_back(std::pair<Point, int>(vertex.other_pt, vertex.count));
      }
      inline vertex_arVCrary_compact(const vertex_arVCrary_compact& vertex) : pt(vertex.pt), count(vertex.count) {}
      inline vertex_arVCrary_compact& operator=(const vertex_arVCrary_compact& vertex){
        pt = vertex.pt; count = vertex.count; return *this; }
      inline bool operator==(const vertex_arVCrary_compact& vertex) const {
        return pt == vertex.pt && count == vertex.count; }
      inline bool operator!=(const vertex_arVCrary_compact& vertex) const { return !((*this) == vertex); }
      inline bool operator<(const vertex_arVCrary_compact& vertex) const {
        if(pt.get(HORIZONTAL) < vertex.pt.get(HORIZONTAL)) return true;
        if(pt.get(HORIZONTAL) == vertex.pt.get(HORIZONTAL)) {
      ...
      inline bool operator>(const vertex_arVCrary_compact& vertex) const { return vertex < (*this); }
      inline bool operator<=(const vertex_arVCrary_compact& vertex) const { return !((*this) > vertex); }
      inline bool operator>=(const vertex_arVCrary_compact& vertex) const { return !((*this) < vertex); }
...
//     inline std::ostream& operator<< (std::ostream& o, const vertex_arVCrary_compact& c) {
//       o << c.pt << ", " << c.count;
//       return o;
//     }
  ...
  protected:
    //definitions
    typedef std::map<vertex_half_edge, active_tail_arVCrary*, less_vertex_half_edge> scanline_data;
    ...
    int fractureHoles_;
  public:
    inline polygon_arVCrary_formation() :
    ...
    inline polygon_arVCrary_formation(bool fractureHoles) :
      scanData_(), x_((std::numeric_limits<Unit>::min)()), justBefore_(false), fractureHoles_(fractureHoles) {
      less_vertex_half_edge lessElm(&x_, &justBefore_);
      scanData_ = scanline_data(lessElm);
    }
    inline polygon_arVCrary_formation(const polygon_arVCrary_formation& that) :
      scanData_(), x_((std::numeric_limits<Unit>::min)()), justBefore_(false), fractureHoles_(0) { (*this) = that; }
    inline polygon_arVCrary_formation& operator=(const polygon_arVCrary_formation& that) {
      x_ = that.x_;
      justBefore_ = that.justBefore_;
  ...
  protected:
    ...
    inline std::pair<std::pair<Point, int>, active_tail_arVCrary*> processPoint_(cT& output, cT2& elements, Point point,
                                                                                  incoming_count& counts_from_scanline, vertex_arVCrary_count& incoming_count) {
      //std::cout << "\nAT POINT: " <<  point << "\n";
      //join any closing solid corners
      std::vector<int> counts;
      std::vector<int> incoming;
      std::vector<active_tail_arVCrary*> tails;
      ...
      active_tail_arVCrary* returnValue = 0;
            ...
            for(int j = i + 1; j < c_size; ++j) {
              ...
              if(counts[j]) {
                if(counts[j] == 1) {
                  ...
                  active_tail_arVCrary::joinChains(point, tails[i], tails[j], true, output);
                  counts[i] = 0;
                  counts[j] = 0;
                  tails[i] = 0;
                  tails[j] = 0;
            ...
            for(int j = i + 1; j < i_size; ++j) {
              ...
              if(incoming[j]) {
                ...
                if(incoming[j] == -1) {
                  ...
                  std::pair<active_tail_arVCrary*, active_tail_arVCrary*> tailPair =
                    active_tail_arVCrary::createActiveTailsAsPair(point, true, 0, fractureHoles_ != 0);
                  ...
                  if(j == i_size_less_1 && incoming_count[j].first.get(HORIZONTAL) == point.get(HORIZONTAL)) {
                    ...
                    elements.push_back(std::pair<vertex_half_edge,
                                       active_tail_arVCrary*>(vertex_half_edge(point,
          ...
          if(counts[i] != 0) {
            for(int j = i+1; j < c_size; ++j) {
              if(counts[j] != 0) {
                ...
                returnValue = active_tail_arVCrary::joinChains(point, tails[i], tails[j], false, output);
          ...
          if(incoming[i] != 0) {
            for(int j = i+1; j < i_size; ++j) {
              if(incoming[j] != 0) {
                ...
                std::pair<active_tail_arVCrary*, active_tail_arVCrary*> tailPair =
                  active_tail_arVCrary::createActiveTailsAsPair(point, false, holep, fractureHoles_ != 0);
                if(j == i_size_less_1 && incoming_count[j].first.get(HORIZONTAL) == point.get(HORIZONTAL)) {
      ...
      return std::pair<std::pair<Point, int>, active_tail_arVCrary*>(returnCount, returnValue);
    }

    static inline void print(const vertex_arVCrary_count& count) {
    ...
    inline iT processEvent_(cT& output, iT inputBegin, iT inputEnd) {
      ...
      active_tail_arVCrary* verticalTail = 0;
      std::pair<Point, int> verticalCount(Point(0, 0), 0);
      iT currentIter = inputBegin;
      std::vector<iterator> elementIters;
      std::vector<std::pair<vertex_half_edge, active_tail_arVCrary*> > elements;
      while(currentIter != inputEnd && currentIter->pt.get(HORIZONTAL) == x_) {
        ...
        while(iter != scanData_.end() &&
          ...
          counts_from_scanline.push_back(std::pair<std::pair<std::pair<Point, Point>, int>, active_tail_arVCrary*>
        ...
        vertex_arVCrary_count incoming;
        ...
        std::pair<std::pair<Point, int>, active_tail_arVCrary*> result = processPoint_(output, elements, Point(x_, currentY), counts_from_scanline, incoming);
        ...
        if(verticalTail && !(verticalCount.second)) {
          ...
          if(currentIter == inputEnd ||
            ...
            if(fractureHoles_) {
              ...
              active_tail_arVCrary* at = iter->second;
      ...
      //std::cout << "inserting\n";
      for(typename std::vector<std::pair<vertex_half_edge, active_tail_arVCrary*> >::iterator iter = elements.begin();
  ...
  public: //test functions
    ...
    static inline bool testPolygonArVCraryFormationRect(stream_type& stdcout) {
      stdcout << "testing polygon formation\n";
      polygon_arVCrary_formation pf(true);
    ...
    template <typename stream_type>
    static inline bool testPolygonArVCraryFormationP1(stream_type& stdcout) {
    ...
    template <typename stream_type>
    static inline bool testPolygonArVCraryFormationP2(stream_type& stdcout) {
    ...
    template <typename stream_type>
    static inline bool testPolygonArVCraryFormationPolys(stream_type& stdcout) {
      stdcout << "testing polygon formation polys\n";
      polygon_arVCrary_formation pf(false);
      std::vector<polygon_with_holes_data<Unit> > polys;
      polygon_arVCrary_formation pf2(true);
    ...
    template <typename stream_type>
    static inline bool testPolygonArVCraryFormationSelfTouch1(stream_type& stdcout) {
    ...
    template <typename stream_type>
    static inline bool testPolygonArVCraryFormationSelfTouch2(stream_type& stdcout) {
    ...
    template <typename stream_type>
    static inline bool testPolygonArVCraryFormationSelfTouch3(stream_type& stdcout) {
    ...
    template <typename stream_type>
    static inline bool testPolygonArVCraryFormationColinear(stream_type& stdcout) {
  ...
  class poly_line_arVCrary_hole_data {
  private:
    typedef typename polygon_arVCrary_formation<Unit>::active_tail_arVCrary active_tail_arVCrary;
    active_tail_arVCrary* p_;
  public:
    typedef point_data<Unit> Point;
    typedef Point point_type;
    typedef Unit coordinate_type;
    typedef typename active_tail_arVCrary::iterator iterator_type;
    //typedef iterator_points_to_compact<iterator_type, Point> compact_iterator_type;

    typedef iterator_type iterator;
    inline poly_line_arVCrary_hole_data() : p_(0) {}
    inline poly_line_arVCrary_hole_data(active_tail_arVCrary* p) : p_(p) {}
  ...
  public:
    ...
    private:
      typedef poly_line_arVCrary_hole_data<Unit> holeType;
      mutable holeType hole_;
      typename active_tail_arVCrary::iteratorHoles itr_;
    ...
    public:
      ...
      inline iterator_holes_type(typename active_tail_arVCrary::iteratorHoles itr) : hole_(), itr_(itr) {}
    ...
    typedef poly_line_arVCrary_hole_data<Unit> hole_type;

    inline poly_line_arVCrary_polygon_data() : p_(0) {}
    inline poly_line_arVCrary_polygon_data(active_tail_arVCrary* p) : p_(p) {}
    //use default copy and assign
    ...
    inline active_tail_arVCrary* yield() { return p_; }
  ...
  template <typename Unit>
  class trapezoid_arVCrary_formation : public polygon_arVCrary_formation<Unit> {
  private:
    typedef typename scanline_base<Unit>::Point Point;
    typedef typename scanline_base<Unit>::half_edge half_edge;
    typedef typename scanline_base<Unit>::vertex_half_edge vertex_half_edge;
    ...
    typedef typename polygon_arVCrary_formation<Unit>::poly_line_arVCrary poly_line_arVCrary;

    typedef typename polygon_arVCrary_formation<Unit>::active_tail_arVCrary active_tail_arVCrary;

    typedef std::vector<std::pair<Point, int> > vertex_arVCrary_count;

    typedef typename polygon_arVCrary_formation<Unit>::less_half_edge_count less_half_edge_count;

    typedef std::vector<std::pair<std::pair<std::pair<Point, Point>, int>, active_tail_arVCrary*> > incoming_count;

    typedef typename polygon_arVCrary_formation<Unit>::less_incoming_count less_incoming_count;

    typedef typename polygon_arVCrary_formation<Unit>::vertex_arVCrary_compact vertex_arVCrary_compact;
  ...
  private:
    ...
    //data
  public:
    inline trapezoid_arVCrary_formation() : polygon_arVCrary_formation<Unit>() {}
    inline trapezoid_arVCrary_formation(const trapezoid_arVCrary_formation& that) : polygon_arVCrary_formation<Unit>(that) {}
    inline trapezoid_arVCrary_formation& operator=(const trapezoid_arVCrary_formation& that) {
      * static_cast<polygon_arVCrary_formation<Unit>*>(this) = * static_cast<polygon_arVCrary_formation<Unit>*>(&that);
      return *this;
    }
    ...
    void scan(cT& output, iT inputBegin, iT inputEnd) {
      ...
      while(inputBegin != inputEnd) {
        //std::cout << "2\n";
        polygon_arVCrary_formation<Unit>::x_ = (*inputBegin).pt.get(HORIZONTAL);
        //std::cout << "SCAN FORMATION " << x_ << "\n";
        //std::cout << "x_ = " << x_ << "\n";
        //std::cout << "scan line size: " << scanData_.size() << "\n";
        inputBegin = processEvent_(output, inputBegin, inputEnd);
  ...
  private:
    //functions
    inline void getVerticalPair_(std::pair<active_tail_arVCrary*,
                                 active_tail_arVCrary*>& verticalPair,
                                 iterator previter) {
      active_tail_arVCrary* iterTail = (*previter).second;
      Point prevPoint(polygon_arVCrary_formation<Unit>::x_,
                      convert_high_precision_type<Unit>(previter->first.evalAtX(polygon_arVCrary_formation<Unit>::x_)));
      iterTail->pushPoint(prevPoint);
      std::pair<active_tail_arVCrary*, active_tail_arVCrary*> tailPair =
        active_tail_arVCrary::createActiveTailsAsPair(prevPoint, true, 0, false);
    ...
    template <class cT, class cT2>
    inline std::pair<std::pair<Point, int>, active_tail_arVCrary*>
    processPoint_(cT& output, cT2& elements,
                  std::pair<active_tail_arVCrary*, active_tail_arVCrary*>& verticalPair,
                  iterator previter, Point point, incoming_count& counts_from_scanline,
                  vertex_arVCrary_count& incoming_count) {
      //std::cout << "\nAT POINT: " <<  point << "\n";
      //join any closing solid corners
      std::vector<int> counts;
      std::vector<int> incoming;
      ...
      active_tail_arVCrary* returnValue = 0;
      std::pair<active_tail_arVCrary*, active_tail_arVCrary*> verticalPairOut;
            ...
            for(int j = i + 1; j < i_size; ++j) {
              ...
              if(incoming[j]) {
                ...
                if(incoming[j] == -1) {
                  ...
                  //std::cout << "creating active tail pair\n";
                  std::pair<active_tail_arVCrary*, active_tail_arVCrary*> tailPair =
                    active_tail_arVCrary::createActiveTailsAsPair(point, true, 0, polygon_arVCrary_formation<Unit>::fractureHoles_ != 0);
                  //tailPair.first->print();
                  //tailPair.second->print();
                  if(j == i_size_less_1 && incoming_count[j].first.get(HORIZONTAL) == point.get(HORIZONTAL)) {
                    //vertical active tail becomes return value
                ...
                if(incoming[j] != 0) {
                  if(incoming[j] == 1) {
                    ...
                    if(j == i_size_less_1 && incoming_count[j].first.get(HORIZONTAL) == point.get(HORIZONTAL)) {
                      ...
                      std::pair<active_tail_arVCrary*, active_tail_arVCrary*> tailPair =
                        active_tail_arVCrary::createActiveTailsAsPair(point, true, 0, false);
                ...
                if(incoming[j] != 0) {
                  if(incoming[j] == -1) {
                    ...
                    if(i == c_size_less_1 &&
                      ...
                      active_tail_arVCrary::joinChains(point, tails[i], verticalPair.first, true, output);
            ...
            for(int j = i+1; j < c_size; ++j) {
              if(counts[j] != 0) {
                ...
                if(j == c_size_less_1 &&
                  ...
                  active_tail_arVCrary::joinChains(point, tails[j], verticalPair.first, true, output);
            ...
            for(int j = i+1; j < i_size; ++j) {
              if(incoming[j] != 0) {
                ...
                if(j == i_size_less_1 &&
                  ...
                  std::pair<active_tail_arVCrary*, active_tail_arVCrary*> tailPair =
                  active_tail_arVCrary::createActiveTailsAsPair(point, false, 0, false);
                  elements.push_back(std::pair<vertex_half_edge,
                                     active_tail_arVCrary*>(vertex_half_edge(point,
                                                                              incoming_count[j].first, incoming[j]), tailPair.second));
                  verticalPairOut.second = tailPair.first;
    ...
    inline iT processEvent_(cT& output, iT inputBegin, iT inputEnd) {
      //typedef typename high_precision_type<Unit>::type high_precision;
      //std::cout << "processEvent_\n";
      polygon_arVCrary_formation<Unit>::justBefore_ = true;
      //collect up all elements from the tree that are at the y
      //values of events in the input queue
      //create vector of new elements to add into tree
      active_tail_arVCrary* verticalTail = 0;
      ...
      std::vector<iterator> elementIters;
      std::vector<std::pair<vertex_half_edge, active_tail_arVCrary*> > elements;
      while(currentIter != inputEnd && currentIter->pt.get(HORIZONTAL) == polygon_arVCrary_formation<Unit>::x_) {
        //std::cout << "loop\n";
        Unit currentY = (*currentIter).pt.get(VERTICAL);
        //std::cout << "current Y " << currentY << "\n";
        //std::cout << "scanline size " << scanData_.size() << "\n";
        ...
        if(previter != polygon_arVCrary_formation<Unit>::scanData_.end() &&
             previter->first.evalAtX(polygon_arVCrary_formation<Unit>::x_) >= currentY &&
             previter != polygon_arVCrary_formation<Unit>::scanData_.begin())
           --previter;
        while(iter != polygon_arVCrary_formation<Unit>::scanData_.end() &&
              ((iter->first.pt.x() == polygon_arVCrary_formation<Unit>::x_ && iter->first.pt.y() == currentY) ||
               (iter->first.other_pt.x() == polygon_arVCrary_formation<Unit>::x_ && iter->first.other_pt.y() == currentY))) {
               //iter->first.evalAtX(polygon_arVCrary_formation<Unit>::x_) == (high_precision)currentY) {
          //std::cout << "loop2\n";
          elementIters.push_back(iter);
          counts_from_scanline.push_back(std::pair<std::pair<std::pair<Point, Point>, int>, active_tail_arVCrary*>
                                         (std::pair<std::pair<Point, Point>, int>(std::pair<Point, Point>(iter->first.pt,
        ...
        Point currentPoint(polygon_arVCrary_formation<Unit>::x_, currentY);
        //std::cout << "counts_from_scanline size " << counts_from_scanline.size() << "\n";
        ...
        do {
          ...
          ++currentIter;
                ...
                currentIter->pt.get(HORIZONTAL) == polygon_arVCrary_formation<Unit>::x_);
        //print(incoming);
        this->sort_vertex_arVCrary_count(incoming, currentPoint);
        //std::cout << currentPoint.get(HORIZONTAL) << "," << currentPoint.get(VERTICAL) << "\n";
        //print(incoming);
        //std::cout << "incoming counts from input size " << incoming.size() << "\n";
        //compact_vertex_arVCrary_count(currentPoint, incoming);
        ...
        if(!incoming.empty() && incoming.back().first.get(HORIZONTAL) == polygon_arVCrary_formation<Unit>::x_) {
          //std::cout << "inverted vertical event\n";
          incoming.back().second *= -1;
        }
        //std::cout << "calling processPoint_\n";
           std::pair<std::pair<Point, int>, active_tail_arVCrary*> result = processPoint_(output, elements, verticalPair, previter, Point(polygon_arVCrary_formation<Unit>::x_, currentY), counts_from_scanline, incoming);
        verticalCount = result.first;
        verticalTail = result.second;
        if(verticalPair.first != 0 && iter != polygon_arVCrary_formation<Unit>::scanData_.end() &&
           (currentIter == inputEnd || currentIter->pt.x() != polygon_arVCrary_formation<Unit>::x_ ||
            currentIter->pt.y() > (*iter).first.evalAtX(polygon_arVCrary_formation<Unit>::x_))) {
          //splice vertical pair into edge above
          active_tail_arVCrary* tailabove = (*iter).second;
          Point point(polygon_arVCrary_formation<Unit>::x_,
                      convert_high_precision_type<Unit>((*iter).first.evalAtX(polygon_arVCrary_formation<Unit>::x_)));
          verticalPair.second->pushPoint(point);
          active_tail_arVCrary::joinChains(point, tailabove, verticalPair.first, true, output);
      ...
      for(typename std::vector<iterator>::iterator iter = elementIters.begin();
          iter != elementIters.end(); ++iter) {
        //std::cout << "erasing loop\n";
        polygon_arVCrary_formation<Unit>::scanData_.erase(*iter);
      }
      //switch comparison tie breaking policy
      polygon_arVCrary_formation<Unit>::justBefore_ = false;
      //add new elements into tree
      //std::cout << "inserting\n";
      for(typename std::vector<std::pair<vertex_half_edge, active_tail_arVCrary*> >::iterator iter = elements.begin();
          iter != elements.end(); ++iter) {
        //std::cout << "inserting loop\n";
        polygon_arVCrary_formation<Unit>::scanData_.insert(polygon_arVCrary_formation<Unit>::scanData_.end(), *iter);
      }
      //std::cout << "end processEvent\n";
      return currentIter;
    }
  public:
    template <typename stream_type>
    static inline bool testTrapezoidArVCraryFormationRect(stream_type& stdcout) {
      stdcout << "testing trapezoid formation\n";
      trapezoid_arVCrary_formation pf;
      std::vector<polygon_data<Unit> > polys;
      std::vector<vertex_half_edge> data;
      data.push_back(vertex_half_edge(Point(0, 0), Point(10, 0), 1));
      data.push_back(vertex_half_edge(Point(0, 0), Point(0, 10), 1));
    ...
    template <typename stream_type>
    static inline bool testTrapezoidArVCraryFormationP1(stream_type& stdcout) {
    ...
    template <typename stream_type>
    static inline bool testTrapezoidArVCraryFormationP2(stream_type& stdcout) {
    ...
    template <typename stream_type>
    static inline bool testTrapezoidArVCraryFormationPolys(stream_type& stdcout) {
      stdcout << "testing trapezoid formation polys\n";
      trapezoid_arVCrary_formation pf;
      std::vector<polygon_with_holes_data<Unit> > polys;
      //trapezoid_arVCrary_formation pf2(true);
      //std::vector<polygon_with_holes_data<Unit> > polys2;
      std::vector<vertex_half_edge> data;
      data.push_back(vertex_half_edge(Point(0, 0), Point(100, 1), 1));
      data.push_back(vertex_half_edge(Point(0, 0), Point(1, 100), -1));
    ...
    template <typename stream_type>
    static inline bool testTrapezoidArVCraryFormationSelfTouch1(stream_type& stdcout) {
  ...
  template <typename T>
  struct PolyLineArVCraryByConcept<T, polygon_with_holes_concept> { typedef poly_line_arVCrary_polygon_data<T> type; };
  template <typename T>
  struct PolyLineArVCraryByConcept<T, polygon_concept> { typedef poly_line_arVCrary_hole_data<T> type; };

  template <typename T>
  struct geometry_concept<poly_line_arVCrary_polygon_data<T> > { typedef polygon_45_with_holes_concept type; };
  template <typename T>
  struct geometry_concept<poly_line_arVCrary_hole_data<T> > { typedef polygon_45_concept type; };
}
}
#endif
