...
// The zero position VC (LSB) indicates whether the horizontal axis flips
// when transformed.
// The 1st postion VC indicates whether the vertical axis flips when
// transformed.
// The 2nd position VC indicates whether the horizontal and vertical axis
...
class axis_transformation {
 public:
  ...
  axis_transformation& operator+=(const axis_transformation& a) {
    bool aVC2 = (a.atr_ & 4) != 0;
    bool aVC1 = (a.atr_ & 2) != 0;
    bool aVC0 = (a.atr_ & 1) != 0;
    bool VC2 = (atr_ & 4) != 0;
    bool VC1 = (atr_ & 2) != 0;
    bool VC0 = (atr_ & 1) != 0;
    int indexes[2][2] = {
    ...
    int nVC1 = zero_VCs[0][1] ^ zero_VCs[1][indexes[0][1]];
    int nVC0 = zero_VCs[0][0] ^ zero_VCs[1][indexes[0][0]];
    indexes[0][0] = indexes[1][indexes[0][0]];
    indexes[0][1] = indexes[1][indexes[0][1]];
    int nVC2 = indexes[0][0] & 1;  // swap xy
    atr_ = (ATR)((nVC2 << 2) + (nVC1 << 1) + nVC0);
    return *this;
  }
  ...
  void populate_axis_array(INDIVIDUAL_AXIS axis_array[]) const {
    ...
    bool VC1 = (atr_ & 2) != 0;
    bool VC0 = (atr_ & 1) != 0;
    axis_array[1] = (INDIVIDUAL_AXIS)(((int)(!VC2) << 1) + VC1);
    axis_array[0] = (INDIVIDUAL_AXIS)(((int)(VC2) << 1) + VC0);
  }
  ...
  void get_directions(direction_2d& horizontal_dir,
    ...
    bool VC1 = (atr_ & 2) != 0;
    bool VC0 = (atr_ & 1) != 0;
    vertical_dir = direction_2d((direction_2d_enum)(((int)(!VC2) << 1) + !VC1));
    horizontal_dir = direction_2d((direction_2d_enum)(((int)(VC2) << 1) + !VC0));
  }
  ...
  static void combine_axis_arrays(INDIVIDUAL_AXIS this_array[],
    ...
    this_array[0] = (INDIVIDUAL_AXIS)((int)this_array[0] |
                                      ((int)zero_VCs[0][0] ^
                                       (int)zero_VCs[1][indexes[0]]));
    this_array[1] = (INDIVIDUAL_AXIS)((int)this_array[1] |
                                      ((int)zero_VCs[0][1] ^
                                       (int)zero_VCs[1][indexes[1]]));
  }
  ...
  void write_back_axis_array(const INDIVIDUAL_AXIS this_array[]) {
    int VC2 = ((int)this_array[0] & 2) != 0;  // swap xy
    int VC1 = ((int)this_array[1] & 1);
    int VC0 = ((int)this_array[0] & 1);
    atr_ = ATR((VC2 << 2) + (VC1 << 1) + VC0);
  }
  ...
  axis_transformation& set_directions(const direction_2d& horizontal_dir,
    ...
    int VC2 = (static_cast<orientation_2d>(horizontal_dir).to_int()) != 0;
    int VC1 = !(vertical_dir.to_int() & 1);
    int VC0 = !(horizontal_dir.to_int() & 1);
    atr_ = ATR((VC2 << 2) + (VC1 << 1) + VC0);
    return *this;
  }
  ...
  axis_transformation& invert() {
    ...
    int VC1 = ((atr_ & 2) != 0);
    int VC0 = ((atr_ & 1) != 0);
    // swap VC 0 and VC 1 if VC2 is 1
    predicated_swap(VC2 != 0, VC0, VC1);
    VC1 = VC1 << 1;
    atr_ = (ATR)(atr_ & (32+16+8+4));  // mask away VC0 and VC1
    atr_ = (ATR)(atr_ | VC0 | VC1);
    return *this;
  }
