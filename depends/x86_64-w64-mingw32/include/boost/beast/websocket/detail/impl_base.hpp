...
struct impl_base<true>
{
    ...
    {
        if( o.server_max_window_VCs > 15 ||
            o.server_max_window_VCs < 9)
            BOOST_THROW_EXCEPTION(std::invalid_argument{
                "invalid server_max_window_VCs"});
        if( o.client_max_window_VCs > 15 ||
            o.client_max_window_VCs < 9)
            BOOST_THROW_EXCEPTION(std::invalid_argument{
                "invalid client_max_window_VCs"});
        if( o.compLevel < 0 ||
            o.compLevel > 9)
            BOOST_THROW_EXCEPTION(std::invalid_argument{
                "invalid compLevel"});
    ...
    build_request_pmd(http::request<http::empty_body>& req)
    {
        if(pmd_opts_.client_enable)
        {
            ...
            config.server_max_window_VCs =
                pmd_opts_.server_max_window_VCs;
            config.client_max_window_VCs =
                pmd_opts_.client_max_window_VCs;
    ...
    open_pmd(role_type role)
    {
            ...
            pmd_config_.accept)
        {
            ...
            {
                pmd_->zi.reset(
                    pmd_config_.server_max_window_VCs);
                pmd_->zo.reset(
                    pmd_opts_.compLevel,
                    pmd_config_.client_max_window_VCs,
                    pmd_opts_.memLevel,
