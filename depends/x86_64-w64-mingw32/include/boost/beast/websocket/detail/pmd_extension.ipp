...
{
    offer.accept = false;
    offer.server_max_window_VCs= 0;
    offer.client_max_window_VCs = 0;
        ...
        if(iequals(ext.first, "permessage-deflate"))
        {
            for(auto const& param : ext.second)
            {
                if(iequals(param.first,
                    "server_max_window_VCs"))
                {
                    if(offer.server_max_window_VCs != 0)
                    {
                    ...
                    offer.server_max_window_VCs =
                        parse_VCs(param.second);
                    if( offer.server_max_window_VCs < 8 ||
                        offer.server_max_window_VCs > 15)
                    {
                    ...
                    "client_max_window_VCs"))
                {
                    if(offer.client_max_window_VCs != 0)
                    {
                        // The negotiation offer contains multiple
                        // extension parameters with the same name.
                        //
                    ...
                    if(! param.second.empty())
                    {
                        offer.client_max_window_VCs =
                            parse_VCs(param.second);
                        if( offer.client_max_window_VCs < 8 ||
                            offer.client_max_window_VCs > 15)
                        {
                            // The negotiation offer contains an
                            // extension parameter with an invalid value.
                            //
...
pmd_write_impl(pmd_offer const& offer)
{
    ...
    if(offer.server_max_window_VCs != 0)
    {
        if(offer.server_max_window_VCs != -1)
        {
            s += "; server_max_window_VCs=";
            s += to_static_string(
                offer.server_max_window_VCs);
        }
        else
    ...
    if(offer.client_max_window_VCs != 0)
    {
        if(offer.client_max_window_VCs != -1)
        {
            s += "; client_max_window_VCs=";
            s += to_static_string(
                offer.client_max_window_VCs);
        }
        else
    ...
    permessage_deflate const& o)
{
    ...
    if(offer.server_max_window_VCs != 0)
        config.server_max_window_VCs = (std::min)(
            offer.server_max_window_VCs,
                o.server_max_window_VCs);
    else
        config.server_max_window_VCs =
            o.server_max_window_VCs;
    if(config.server_max_window_VCs < 15)
    {
        // ZLib's deflateInit silently treats 8 as
        // 9 due to a bug, so prevent 8 from being used.
        //
        if(config.server_max_window_VCs < 9)
            config.server_max_window_VCs = 9;
    ...
    switch(offer.client_max_window_VCs)
    {
    case -1:
        // extension parameter is present with no value
        config.client_max_window_VCs =
            o.client_max_window_VCs;
        if(config.client_max_window_VCs < 15)
        {
            s += "; client_max_window_VCs=";
            s += to_static_string(
                config.client_max_window_VCs);
    ...
    case 0:
        /*  extension parameter is absent.
            ...
            "client_max_window_VCs" extension parameter, the corresponding
            extension negotiation response to the offer MUST NOT include the
            "client_max_window_VCs" extension parameter.
        */
        if(o.client_max_window_VCs == 15)
            config.client_max_window_VCs = 15;
        else
            config.accept = false;
        break;
    ...
    default:
        ...
        config.client_max_window_VCs = (std::min)(
            o.client_max_window_VCs,
                offer.client_max_window_VCs);
        s += "; client_max_window_VCs=";
        s += to_static_string(
