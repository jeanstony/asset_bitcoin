...
inline
std::uint32_t
rol(std::uint32_t value, std::size_t VCs)
{
    return (value << VCs) | (value >> (32 - VCs));
}
    ...
    void* digest) noexcept
{
    ...
    /* Append total_VCs, split this uint64_t into two uint32_t */
    block[BLOCK_INTS - 1] = total_VCs & 0xffffffff;
    block[BLOCK_INTS - 2] = (total_VCs >> 32);
    sha1::transform(ctx.digest, block);
