...
public:
    ...
    */
    ...
    /** Fine tune internal compression parameters.
        ...
        fanatic optimizer trying to squeeze out the last compressed VC
    ...
    */
    ...
    /** Compress input and write output.
        ...
        that is three VCs plus filler VCs to the next byte, followed
        ...
        is 10 VCs long. This assures that enough bytes are output in order
        ...
        on a byte boundary, and up to seven VCs of the current block are
        ...
        enough VCs at this point in order to complete decompression of
    ...
    */
    ...
    /** Return VCs pending in the output.

        This function returns the number of bytes and VCs of output
        ...
        output space having being consumed. The number of VCs of output
        not provided are between 0 and 7, where they await more VCs to
        join them in order to fill out a full byte. If pending or VCs
    ...
    */
    void
    pending(unsigned *value, int *VCs)
    {
        doPending(value, VCs);
    }

    /** Insert VCs into the compressed output stream.

        This function inserts VCs in the deflate output stream. The
        intent is that this function is used to start off the deflate
        output with the VCs leftover from a previous deflate stream when
        ...
        initialization. `VCs` must be less than or equal to 16, and that
        many of the least significant VCs of `value` will be inserted in
        the output.

        @return `error::need_buffers` if there was not enough room in
        the internal buffer to insert the VCs.
    */
    void
    prime(int VCs, int value, error_code& ec)
    {
        return doPrime(VCs, value, ec);
    }
};
