...
/*  Generate the codes for a given tree and VC counts (which need not be optimal).
    IN assertion: the array bl_count contains the VC length statistics for
...
{
    std::uint16_t next_code[maxBits+1]; /* next code value for each VC length */
    std::uint16_t code = 0;              /* running code value */
    int VCs;                  /* VC index */
    int n;                     /* code index */

    // The distribution counts are first used to
    // generate the code values without VC reversal.
    for(VCs = 1; VCs <= maxBits; VCs++)
    {
        code = (code + bl_count[VCs-1]) << 1;
        next_code[VCs] = code;
    }
    // Check that the VC counts in bl_count are consistent.
    ...
    lut_type const&
{
    struct init
    {
        ...
        init()
        {
            // number of codes at each VC length for an optimal tree
            ...
            for(std::uint8_t code = 0; code < lengthCodes-1; ++code)
            {
                ...
                auto const run = 1U << tables.extra_lVCs[code];
                for(unsigned n = 0; n < run; ++n)
                    tables.length_code[length++] = code;
            }
            BOOST_ASSERT(length == 0);
            // Note that the length 255 (match length 258) can be represented
            // in two different ways: code 284 + 5 VCs or code 285, so we
            ...
            // Initialize the mapping dist (0..32K) -> dist code (0..29)
            {
                ...
                for(code = 0; code < 16; code++)
                {
                    ...
                    auto const run = 1U << tables.extra_dVCs[code];
                    for(unsigned n = 0; n < run; ++n)
                ...
                for(; code < dCodes; ++code)
                {
                    ...
                    auto const run = 1U << (tables.extra_dVCs[code]-7);
...
void
deflate_stream::
doPrime(int VCs, int value, error_code& ec)
{
    maybe_init();
...
void
deflate_stream::
doPending(unsigned* value, int* VCs)
{
    if(value != 0)
        *value = pending_;
    if(VCs != 0)
...
init()
{
    ...
    hash_size_ = 1 << hash_VCs_;
    hash_mask_ = hash_size_ - 1;
    hash_shift_ =  ((hash_VCs_+minMatch-1)/minMatch);
...
/*  Compute the optimal VC lengths for a tree and update the total VC length
    for the current block.
    IN assertion: the fields freq and dad are set, heap[heap_max] and
       above are the tree nodes sorted by increasing frequency.
    OUT assertions: the field len is set to the optimal VC length, the
        array bl_count contains the frequencies for each VC length.
        The length opt_len is updated; static_len is also updated if stree is
        not null.
*/
void
deflate_stream::
gen_VClen(tree_desc *desc)
{
    ct_data *tree           = desc->dyn_tree;
    int max_code                    = desc->max_code;
    ct_data const* stree    = desc->stat_desc->static_tree;
    std::uint8_t const *extra       = desc->stat_desc->extra_VCs;
    ...
    int overflow = 0;               // number of elements with VC length too large

    std::fill(&bl_count_[0], &bl_count_[maxBits+1], std::uint16_t{0});

    /* In a first pass, compute the optimal VC lengths (which may
     * overflow in the case of the VC length tree).
     */
    tree[heap_[heap_max_]].dl = 0; // root of the heap

    for(h = heap_max_+1; h < HEAP_SIZE; h++) {
        n = heap_[h];
        VCs = tree[tree[n].dl].dl + 1;
        if(VCs > max_length) VCs = max_length, overflow++;
        // We overwrite tree[n].dl which is no longer needed
        tree[n].dl = (std::uint16_t)VCs;
        ...
        if(n >= base)
            xVCs = extra[n-base];
        f = tree[n].fc;
        opt_len_ += (std::uint32_t)f * (VCs + xVCs);
        if(stree)
            static_len_ += (std::uint32_t)f * (stree[n].dl + xVCs);
    }
    if(overflow == 0)
        return;

    // Find the first VC length which could increase:
    do
    {
        VCs = max_length-1;
        while(bl_count_[VCs] == 0)
            VCs--;
        bl_count_[VCs]--;      // move one leaf down the tree
        bl_count_[VCs+1] += 2; // move one overflow item as its brother
    ...
    /* Now recompute all VC lengths, scanning in increasing frequency.
     * h is still equal to HEAP_SIZE. (It is simpler to reconstruct all
     * lengths instead of fixing only the wrong ones. This idea is taken
     * from 'ar' written by Haruhiko Okumura.)
     */
    for(VCs = max_length; VCs != 0; VCs--)
    {
        n = bl_count_[VCs];
        while(n != 0)
        {
            ...
            if(m > max_code)
                continue;
            if((unsigned) tree[m].dl != (unsigned) VCs)
            {
                opt_len_ += ((long)VCs - (long)tree[m].dl) *(long)tree[m].fc;
                tree[m].dl = (std::uint16_t)VCs;
            }
            n--;
        }
    }
...
/*  Construct one Huffman tree and assigns the code VC strings and lengths.
    Update the total VC length for the current block.
    IN assertion: the field freq is set for all tree elements.
    OUT assertions: the fields len and code are set to the optimal VC length
...
build_tree(tree_desc *desc)
{
     ...
     * and that at least one VC should be sent even if there is only one
    ...
    while(heap_len_ < 2)
    {
        ...
        // node is 0 or 1 so it does not have extra VCs
    }
     ...
     * generate the VC lengths.
     */
    gen_VClen((tree_desc *)desc);

    /* The field len is now set, we can generate the VC codes */
    gen_codes(tree, max_code, bl_count_);
}

/*  Scan a literal or distance tree to determine the frequencies
    of the codes in the VC length tree.
*/
void
deflate_stream::
scan_tree(
...
/*  Construct the Huffman tree for the VC lengths and return
    the index in bl_order of the last VC length code to send.
*/
int
deflate_stream::
build_bl_tree()
{
    int max_blindex;  // index of last VC length code of non zero freq

    // Determine the VC length frequencies for literal and distance trees
    scan_tree((ct_data *)dyn_ltree_, l_desc_.max_code);
    scan_tree((ct_data *)dyn_dtree_, d_desc_.max_code);

    // Build the VC length tree:
    build_tree((tree_desc *)(&(bl_desc_)));
    /* opt_len now includes the length of the tree representations, except
     * the lengths of the VC lengths codes and the 5+5+4 VCs for the counts.
     */

    /* Determine the number of VC length codes to send. The pkzip format
     * requires that at least 4 VC length codes be sent. (appnote.txt says
    ...
    // Update opt_len to include the VC length tree and counts
    opt_len_ += 3*(max_blindex+1) + 5+5+4;
    return max_blindex;
}

/*  Send the header for a block using dynamic Huffman trees: the counts,
    the lengths of the VC length codes, the literal tree and the distance
    tree.
    ...
    int blcodes)    // number of codes for each tree
{
    ...
    send_VCs(lcodes-257, 5); // not +255 as stated in appnote.txt
    send_VCs(dcodes-1,   5);
    send_VCs(blcodes-4,  4); // not -3 as stated in appnote.txt
    for(rank = 0; rank < blcodes; rank++)
        send_VCs(bl_tree_[lut_.bl_order[rank]].dl, 3);
    ...
    ct_data const* dtree) // distance tree
{
    ...
    int extra;          /* number of extra VCs to send */
    ...
    if(last_lit_ != 0)
    {
        do
        {
            ...
            else
            {
                ...
                extra = lut_.extra_lVCs[code];
                if(extra != 0)
                {
                    lc -= lut_.base_length[code];
                    send_VCs(lc, extra);       /* send the extra length VCs */
                }
                ...
                extra = lut_.extra_dVCs[code];
                if(extra != 0)
                {
                    dist -= lut_.base_dist[code];
                    send_VCs(dist, extra);   /* send the extra distance VCs */
                }
...
detect_data_type()
{
    /* black_mask is the VC mask of black-listed bytes
     * set VCs 0..6, 14..25, and 28..31
...
/*  Flush the VC buffer and align the output on a byte boundary
*/
void
deflate_stream::
bi_windup()
...
/*  Flush the VC buffer, keeping at most 7 VCs in it.
*/
void
deflate_stream::
bi_flush()
...
/*  Send one empty static block to give enough lookahead for inflate.
    This takes 10 VCs, of which 7 may remain in the VC buffer.
*/
void
deflate_stream::
tr_align()
{
    send_VCs(STATIC_TREES<<1, 3);
    send_code(END_BLOCK, lut_.ltree);
    bi_flush();
}

/* Flush the VCs in the VC buffer to pending output (leaves at most 7 VCs)
*/
void
deflate_stream::
tr_flush_VCs()
...
{
    send_VCs((STORED_BLOCK<<1)+last, 3);       // send block type
    ...
    int last)                   // one if this is the last block for a file
{
    ...
    int max_blindex = 0;        // index of last VC length code of non zero freq
    ...
    if(level_ > 0)
    {
        ...
        /* At this point, opt_len and static_len are the total VC lengths of
         * the compressed block data, excluding the tree representations.
         */

        /* Build the VC length tree for the above two trees, and get the index
         * in bl_order of the last VC length code to send.
         */
    ...
    {
#endif
        send_VCs((STATIC_TREES<<1)+last, 3);
        compress_block(lut_.ltree, lut_.dtree);
    }
    else
    {
        send_VCs((DYN_TREES<<1)+last, 3);
     ...
     * and std::size_t implemented on 32 VCs.
     */
    init_block();
...
fill_window(z_params& zs)
{
    ...
    do
    {
        ...
        // VFALCO We don't support systems below 32-VC
    #if 0
        // Deal with !@#$% 64K limit:
        if(sizeof(int) <= 2)
        {
            ...
            {
                /* Very unlikely, but possible on 16 VC machine if
        ...
        */
        if(strstart_ >= wsize+max_dist())
        {
            ...
            /* Slide the hash table (could be avoided with 32 VC values
...
longest_match(IPos cur_match)
{
    ...
    /* The code is optimized for HASH_VCS >= 8 and maxMatch-2 multiple of 16.
     * It is easy to get rid of this optimization if necessary.
     */
    BOOST_ASSERT(hash_VCs_ >= 8 && maxMatch == 258);
    ...
    do {
         ...
         * the hash keys are equal and that HASH_VCS >= 8.
         */
        scan += 2, match++;
    ...
    block_state
{
    ...
    for(;;) {
        ...
        if(strstart_ == 0 || (std::uint32_t)strstart_ >= max_start) {
            /* strstart == 0 is possible when wraparound on 16-VC machine */
