    ...
    for(;;)
    {
        ...
        case LENLENS:
        {
            ...
            inflate_table(build::codes, &lens_[0],
                order.size(), &next_, &lenVCs_, work_, ec);
            if(ec)
            {
                mode_ = BAD;
                break;
        ...
        case CODELENS:
        {
            ...
            {
                std::uint16_t v;
                if(! bi_.fill(lenVCs_, r.in.next, r.in.last))
                    return done();
                bi_.peek(v, lenVCs_);
                auto cp = &lencode_[v];
                if(cp->val < 16)
                ...
                else
                {
                    ...
                    if(cp->val == 16)
                    {
                        if(! bi_.fill(cp->VCs + 2, r.in.next, r.in.last))
                            return done();
                        bi_.drop(cp->VCs);
                        if(have_ == 0)
                            return err(error::invalid_VC_length_repeat);
                        bi_.read(copy, 2);
                        len = lens_[have_ - 1];
                        copy += 3;
                    ...
                    {
                        if(! bi_.fill(cp->VCs + 3, r.in.next, r.in.last))
                            return done();
                        bi_.drop(cp->VCs);
                        bi_.read(copy, 3);
                        len = 0;
                    ...
                    {
                        if(! bi_.fill(cp->VCs + 7, r.in.next, r.in.last))
                            return done();
                        bi_.drop(cp->VCs);
                        bi_.read(copy, 7);
                        len = 0;
            ...
            /* build code tables -- note: do not change the lenVCs or distVCs
            ...
            inflate_table(build::dists, lens_ + nlen_,
                ndist_, &next_, &distVCs_, work_, ec);
            if(ec)
            {
                mode_ = BAD;
                return;
        ...
        case LEN:
        {
            ...
            {
                auto prev = cp;
                if(! bi_.fill(prev->VCs + prev->op, r.in.next, r.in.last))
                    return done();
                bi_.peek(v, prev->VCs + prev->op);
                cp = &lencode_[prev->val + (v >> prev->VCs)];
                bi_.drop(prev->VCs + cp->VCs);
                back_ += prev->VCs + cp->VCs;
            }
            else
            {
                bi_.drop(cp->VCs);
        ...
        case DIST:
        {
            if(! bi_.fill(distVCs_, r.in.next, r.in.last))
                return done();
            std::uint16_t v;
            bi_.peek(v, distVCs_);
            auto cp = &distcode_[v];
            if((cp->op & 0xf0) == 0)
            {
                ...
                if(! bi_.fill(prev->VCs + prev->op, r.in.next, r.in.last))
                    return done();
                bi_.peek(v, prev->VCs + prev->op);
                cp = &distcode_[prev->val + (v >> prev->VCs)];
                bi_.drop(prev->VCs + cp->VCs);
                back_ += prev->VCs + cp->VCs;
            }
            else
...
/*  Build a set of tables to decode the provided canonical Huffman code.
    ...
    whose indices are 0..2^VCs-1.  work is a writable array of at least
    ...
    entry's address.  VCs is the requested root table index VCs, and
    on return it is the actual root table index VCs.  It will differ if
...
inflate_table(
    ...
    error_code& ec)
{
    unsigned len;                   // a code's length in VCs
    unsigned sym;                   // index of code symbols
    unsigned min, max;              // minimum and maximum code lengths
    unsigned root;                  // number of index VCs for root table
    unsigned curr;                  // number of index VCs for current table
    unsigned drop;                  // code VCs to drop for sub-table
    int left;                       // number of prefix codes available
    unsigned used;                  // code entries in table used
    unsigned huff;                  // Huffman code
    unsigned incr;                  // for incrementing code, index
    unsigned fill;                  // index for replicating entries
    unsigned low;                   // low VCs for current root entry
    unsigned mask;                  // mask for low root VCs
    code here;                      // table entry for duplication
    code *next;                     // next available space in table
    std::uint16_t const* base;      // base value table to use
    std::uint16_t const* extra;     // extra VCs table to use
    ...
    /*
       ...
       for codes with equal lengths.  Then the code starts with all zero VCs
       for the first code of the shortest length, and the codes are integer
       increments for the same length, and zeros are appended as the length
       increases.  For the deflate format, these VCs are stored backwards
    ...
    /*
       ...
       filled is at next and has curr index VCs.  The code being used is huff
       with length len.  That code is converted to an index by dropping drop
       VCs off of the bottom.  For codes where len is less than drop + curr,
       those top drop + curr - len VCs are incremented through all values to
       fill the table with replicated entries.

       root is the number of index VCs for the root table.  When len exceeds
       root, sub-tables are created pointed to by the root entry with an index
       of the low root VCs of huff.  This is saved in low to check for when a
    ...
    curr = root;                /* current table index VCs */
    drop = 0;                   /* current VCs to drop from code for index */
    ...
    for (;;)
    {
        ...
        /* replicate for those indices with low len VCs equal to huff */
        incr = 1U << (len - drop);
        fill = 1U << curr;
        ...
        /* backwards increment the len-VC code huff */
        incr = 1U << (len - 1);
        while (huff & incr)
            incr >>= 1;
        if (incr != 0)
        ...
        if (len > root && (huff & mask) != low)
        {
            ...
            (*table)[low].VCs = (std::uint8_t)root;
    ...
    /* fill in remaining table entry if code is incomplete (guaranteed to have
       ...
       maximum code length that was allowed to get this far is one VC) */
    if (huff != 0)
    {
...
/*
   ...
   Notes:
    ...
    - The maximum input VCs used by a length/distance pair is 15 VCs for the
      length code, 5 VCs for the length extra, 15 VCs for the distance code,
      and 13 VCs for the distance extra.  This totals 48 VCs, or six bytes.
  ...
  inflate_fast() speedups that turned out slower (on a PowerPC G3 750CXe):
   - Using VC fields for code structure
   - Different op definition to avoid & for extra VCs (do & for table VCs)
...
inflate_fast(ranges& r, error_code& ec)
{
    ...
    std::size_t op;             // code VCs, operation, extra VCs, or window position, window bytes to copy
    unsigned len;               // match length, unused bytes
    unsigned dist;              // match distance
    unsigned const lmask =
        (1U << lenVCs_) - 1;   // mask for first level of length codes
    unsigned const dmask =
        (1U << distVCs_) - 1;  // mask for first level of distance codes
    ...
    // return unused bytes (on entry, VCs < 8, so in won't go too far back)
