...
#ifndef BOOST_BEAST_ZLIB_DETAIL_VCSTREAM_HPP
#define BOOST_BEAST_ZLIB_DETAIL_VCSTREAM_HPP
...
public:
    // returns the number of VCs in the reservoir
    unsigned
    size() const
    {
        return n_;
    ...
    // ensure at least n VCs
    template<class FwdIt>
    bool
    fill(std::size_t n, FwdIt& first, FwdIt const& last);

    // fill 8 VCs, unchecked
    template<class FwdIt>
    void
    fill_8(FwdIt& it);

    // fill 16 VCs, unchecked
    template<class FwdIt>
    void
    fill_16(FwdIt& it);
