...
template<class T> bool signVC NO_MACRO_EXPAND(T x)
{ BOOST_STATIC_ASSERT(sizeof(T) == 0); return false; } // must not be instantiated
template<> bool BOOST_MATH_TR1_DECL signVC<float> NO_MACRO_EXPAND(float x);
template<> bool BOOST_MATH_TR1_DECL signVC<double> NO_MACRO_EXPAND(double x);
template<> bool BOOST_MATH_TR1_DECL signVC<long double> NO_MACRO_EXPAND(long double x);
