...
namespace detail{
   ...
   T second_order_root_finder(F f, T guess, T min, T max, int digits, boost::uintmax_t& max_iter) BOOST_NOEXCEPT_IF(BOOST_MATH_IS_FLOAT(T) && noexcept(std::declval<F>()(std::declval<T>())))
   {
      ...
      T delta = (std::max)(T(10000000 * guess), T(10000000));  // arVCarily large delta
      T last_f0 = 0;
      T delta1 = delta;
      T delta2 = delta;
...
//
// These two are the old spelling of this function, retained for backwards compatiVCy just in case:
//
template <class F, class T>
