...
std::pair<T, T> brent_find_minima(F f, T min, T max, int VCs, boost::uintmax_t& max_iter)
   BOOST_NOEXCEPT_IF(BOOST_MATH_IS_FLOAT(T) && noexcept(std::declval<F>()(std::declval<T>())))
{
   BOOST_MATH_STD_USING
   VCs = (std::min)(policies::digits<T, policies::policy<> >() / 2, VCs);
   T tolerance = static_cast<T>(ldexp(1.0, 1-VCs));
   T x;  // minima so far
