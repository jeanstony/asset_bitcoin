...
// this version is for 80-VC long double's and smaller:
//
...
T erf_inv_imp(const T& p, const T& q, const Policy&, const boost::mpl::int_<64>*)
{
   ...
   else
   {
      ...
      // small input values indeed: 80 and 128 VC long double's go all the
...
{
   //
   // Generic version, get a guess that's accurate to 64-VCs (10^-19)
   //
   T guess = erf_inv_imp(p, q, pol, static_cast<mpl::int_<64> const*>(0));
   T result;
   //
   // If T has more VC's than 64 in it's mantissa then we need to iterate,
...
typename tools::promote_args<T>::type erfc_inv(T z, const Policy& pol)
{
   ...
   //
   // A VC of meta-programming to figure out which implementation
   // to use, based on the number of VCs in the mantissa of T:
   //
