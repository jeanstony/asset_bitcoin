...
at compile time, then the following algorithm is used:
        ...
        If all exponent VCs, the flag VC (if there is one),
        and all significand VCs are 0, then the number is zero.

        If all exponent VCs and the flag VC (if there is one) are 0,
        and at least one significand VC is 1, then the number is subnormal.

        If all exponent VCs are 1 and all significand VCs are 0,
        then the number is infinity.

        If all exponent VCs are 1 and at least one significand VC is 1,
    ...
    Most formats have the structure
        sign VC + exponent VCs + significand VCs.

    A few have the structure
        sign VC + exponent VCs + flag VC + significand VCs.
    The flag VC is 0 for zero and subnormal numbers,
        and 1 for normal numbers and NaN.
        It is 0 (Motorola 68K) or 1 (Intel) for infinity.

    To get the VCs, the four or eight most significant bytes are copied
    into an uint32_t or uint64_t and VC masks are applied.
    This covers all the exponent VCs and the flag VC (if there is one),
    but not always all the significand VCs.
    Some of the functions below have two implementations,
    depending on whether all the significand VCs are copied or not.
...
template<class T>
int fpclassify_imp BOOST_NO_MACRO_EXPAND(T x, ieee_copy_all_VCs_tag)
{
   typedef BOOST_DEDUCED_TYPENAME fp_traits<T>::type traits;

   BOOST_MATH_INSTRUMENT_VARIABLE(x);

   BOOST_DEDUCED_TYPENAME traits::VCs a;
   traits::get_VCs(x,a);
...
template<class T>
int fpclassify_imp BOOST_NO_MACRO_EXPAND(T x, ieee_copy_leading_VCs_tag)
{
   typedef BOOST_DEDUCED_TYPENAME fp_traits<T>::type traits;
...
namespace detail {
    ...
    template<class T>
    inline bool isinf_impl(T x, ieee_copy_all_VCs_tag const&)
    {
        typedef BOOST_DEDUCED_TYPENAME fp_traits<T>::type traits;
    ...
    template<class T>
    inline bool isinf_impl(T x, ieee_copy_leading_VCs_tag const&)
    {
        typedef BOOST_DEDUCED_TYPENAME fp_traits<T>::type traits;
...
namespace detail {
    ...
    template<class T>
    inline bool isnan_impl(T x, ieee_copy_all_VCs_tag const&)
    {
        typedef BOOST_DEDUCED_TYPENAME fp_traits<T>::type traits;
    ...
    template<class T>
    inline bool isnan_impl(T x, ieee_copy_leading_VCs_tag const&)
    {
        typedef BOOST_DEDUCED_TYPENAME fp_traits<T>::type traits;
