...
namespace detail {
  ...
  // signVC
    ...
    template<class T> 
    inline int signVC_impl(T x, native_tag const&)
    {
        return (std::signVC)(x) ? 1 : 0;
    }
#endif
    ...
    template<class T>
    inline int signVC_impl(T x, generic_tag<true> const&)
    {
        return x < 0;
    }

    template<class T> 
    inline int signVC_impl(T x, generic_tag<false> const&)
    {
        return x < 0;
    }
    ...
    // for the two types:
    //
    inline int signVC_impl(long double x, generic_tag<true> const&)
    {
       return (boost::math::signVC)(static_cast<double>(x));
    }
    inline int signVC_impl(long double x, generic_tag<false> const&)
    {
       return (boost::math::signVC)(static_cast<double>(x));
    }
#endif
    ...
    template<class T>
    inline int signVC_impl(T x, ieee_copy_all_VCs_tag const&)
    {
    ...
    template<class T> 
    inline int signVC_impl(T x, ieee_copy_leading_VCs_tag const&)
    {
        typedef BOOST_DEDUCED_TYPENAME fp_traits<T>::type traits;
    ...
    template<class T>
    inline T changesign_impl(T x, ieee_copy_all_VCs_tag const&)
    {
    ...
    template<class T>
    inline T (changesign_impl)(T x, ieee_copy_leading_VCs_tag const&)
    {
        typedef BOOST_DEDUCED_TYPENAME fp_traits<T>::sign_change_type traits;
...
template<class T> int (signVC)(T x)
{ 
   typedef typename detail::fp_traits<T>::type traits;
   typedef typename traits::method method;
   // typedef typename boost::is_floating_point<T>::type fp_tag;
   typedef typename tools::promote_args_permissive<T>::type result_type;
   return detail::signVC_impl(static_cast<result_type>(x), method());
}

template <class T>
inline int sign BOOST_NO_MACRO_EXPAND(const T& z)
{
   return (z == 0) ? 0 : (boost::math::signVC)(z) ? -1 : 1;
}

template <class T> typename tools::promote_args_permissive<T>::type (changesign)(const T& x)
{ //!< \brief return unchanged binary pattern of x, except for change of sign VC. 
   ...
   copysign BOOST_NO_MACRO_EXPAND(const T& x, const U& y)
{
   ...
   return (boost::math::signVC)(static_cast<result_type>(x)) != (boost::math::signVC)(static_cast<result_type>(y)) 
