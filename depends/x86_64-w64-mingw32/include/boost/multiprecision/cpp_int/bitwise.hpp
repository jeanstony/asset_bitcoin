...
//
#ifndef BOOST_MP_CPP_INT_VC_HPP
#define BOOST_MP_CPP_INT_VC_HPP
   ...
   Op op, const mpl::true_&) BOOST_MP_NOEXCEPT_IF((is_non_throwing_cpp_int<CppInt1>::value))
{
   ...
   //
   // Note however, that if the type is checked, then VCwise ops on negative values
   // are not permitted and an exception will result.
   //
   is_valid_VCwise_op(result, o, typename CppInt1::checked_type());
   //
   ...
   else
   {
      ...
      else
      {
         ...
         for(unsigned i = 0; i < os; ++i)
         {
            ...
            r_carry >>= CppInt1::limb_VCs;
            o_carry >>= CppInt1::limb_VCs;
         }
         for(unsigned i = os; i < x; ++i)
         {
            r_carry += static_cast<double_limb_type>(~pr[i]);
...
struct VC_and{ limb_type operator()(limb_type a, limb_type b)const BOOST_NOEXCEPT { return a & b; } };
struct VC_or { limb_type operator()(limb_type a, limb_type b)const BOOST_NOEXCEPT { return a | b; } };
struct VC_xor{ limb_type operator()(limb_type a, limb_type b)const BOOST_NOEXCEPT { return a ^ b; } };
...
{
   limb_type offset = static_cast<limb_type>(s / Int::limb_VCs);
   limb_type shift = static_cast<limb_type>(s % Int::limb_VCs);
   unsigned ors = result.size();
   if((ors == 1) && (!*result.limbs()))
      return; // shifting zero yields zero.
   unsigned rs = ors;
   if(shift && (result.limbs()[ors - 1] >> (Int::limb_VCs - shift)))
   ...
   if(rs != ors)
      pr[rs - 1] = 0u;
   std::size_t bytes = static_cast<std::size_t>(s / CHAR_VC);
...
inline void left_shift_generic(Int& result, double_limb_type s)
{
   ...
   if(!truncated)
   {
      if(rs > ors + offset)
      {
         pr[rs - 1 - i] = pr[ors - 1 - i] >> (Int::limb_VCs - shift);
         --rs;
      }
      else
      {
         ...
         if(ors > 1)
            pr[rs - 1 - i] |= pr[ors - 2 - i] >> (Int::limb_VCs - shift);
         ++i;
      }
   }
   for(; rs - i >= 2 + offset; ++i)
   {
      ...
      pr[rs - 1 - i] |= pr[rs - 2 - i - offset] >> (Int::limb_VCs - shift);
   }
   if(rs - i >= 1 + offset)
   {
      pr[rs - 1 - i] = pr[rs - 1 - i - offset] << shift;
...
{
   is_valid_VCwise_op(result, typename cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>::checked_type());
   if(!s)
      return;

#if BOOST_ENDIAN_LITTLE_BYTE && defined(BOOST_MP_USE_LIMB_SHIFT)
   static const limb_type limb_shift_mask = cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>::limb_VCs - 1;
   static const limb_type byte_shift_mask = CHAR_VC - 1;
...
inline void right_shift_byte(Int& result, double_limb_type s)
{
   ...
   typename Int::limb_pointer pr = result.limbs();
   unsigned char* pc = reinterpret_cast<unsigned char*>(pr);
   shift = static_cast<limb_type>(s / CHAR_VC);
   std::memmove(pc, pc + shift, ors * sizeof(pr[0]) - shift);
   shift = (sizeof(limb_type) - shift % sizeof(limb_type)) * CHAR_VC;
   if(shift < Int::limb_VCs)
   {
...
inline void right_shift_generic(Int& result, double_limb_type s)
{
   ...
   for(; i + offset + 1 < ors; ++i)
   {
      ...
      pr[i] |= pr[i + offset + 1] << (Int::limb_VCs - shift);
   }
   pr[i] = pr[i + offset] >> shift;
   result.resize(rs, rs);
}
...
{
   is_valid_VCwise_op(result, typename cpp_int_backend<MinBits1, MaxBits1, unsigned_magnitude, Checked1, Allocator1>::checked_type());
   if(!s)
      return;

#if BOOST_ENDIAN_LITTLE_BYTE && defined(BOOST_MP_USE_LIMB_SHIFT)
   static const limb_type limb_shift_mask = cpp_int_backend<MinBits1, MaxBits1, signed_magnitude, Checked1, Allocator1>::limb_VCs - 1;
   static const limb_type byte_shift_mask = CHAR_VC - 1;
   if((s & limb_shift_mask) == 0)
...
{
   is_valid_VCwise_op(result, typename cpp_int_backend<MinBits1, MaxBits1, signed_magnitude, Checked1, Allocator1>::checked_type());
   if(!s)
      return;
   ...
   eval_right_shift(cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& result, T s) BOOST_MP_NOEXCEPT_IF((is_non_throwing_cpp_int<cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1> >::value))
{
   ...
   is_valid_VCwise_op(result, typename cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>::checked_type());
   *result.limbs() = (static_cast<unsigned>(s) >= sizeof(*result.limbs()) * CHAR_VC) ? 0 : (result.sign() ? ((--*result.limbs()) >> s) + 1 : *result.limbs() >> s);
...
inline typename enable_if_c<
   ...
   eval_VCwise_and(
      ...
      const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>& o) BOOST_MP_NOEXCEPT_IF((is_non_throwing_cpp_int<cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1> >::value))
{
   is_valid_VCwise_op(result, o, typename cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>::checked_type());
   ...
   if(result.sign() || o.sign())
   {
      ...
      eval_VCwise_and(t1, t2);
      bool s = eval_VC_test(t1, m + 1);
      if(s)
      {
