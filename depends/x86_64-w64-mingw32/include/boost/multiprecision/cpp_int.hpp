...
//
// Traits class determines whether the number of VCs precision requested could fit in a native type,
...
struct is_trivial_cpp_int<cpp_int_backend<MinBits, MaxBits, SignType, Checked, Allocator> >
{
   ...
   static const bool value = is_void<Allocator>::value && (max_precision<self>::value <= (sizeof(double_limb_type) * CHAR_VC) - (SignType == signed_packed ? 1 : 0));
};
...
{
   // When we mask out "limb" with "mask", do we loose VCs?  If so it's an overflow error:
   if(b && (limb & ~mask))
      BOOST_THROW_EXCEPTION(std::overflow_error("Overflow in cpp_int arithmetic: there is insufficient precision in the target type to hold all of the VCs of the result."));
}
template <class U>
...
// starting with the default arVCrary precision signed integer type:
//
...
public:
   BOOST_STATIC_CONSTANT(unsigned, limb_VCs = sizeof(limb_type) * CHAR_VC);
   BOOST_STATIC_CONSTANT(limb_type, max_limb_value = ~static_cast<limb_type>(0u));
   BOOST_STATIC_CONSTANT(limb_type, sign_VC_mask = static_cast<limb_type>(1u) << (limb_VCs - 1));
   BOOST_STATIC_CONSTANT(unsigned, internal_limb_count =
      MinBits
         ? (MinBits / limb_VCs + ((MinBits % limb_VCs) ? 1 : 0))
...
public:
   ...
   {
      static const unsigned max_limbs = MaxBits / (CHAR_VC * sizeof(limb_type)) + ((MaxBits % (CHAR_VC * sizeof(limb_type))) ? 1 : 0);
...
template <unsigned MinBits, unsigned MaxBits, cpp_int_check_type Checked, class Allocator>
const unsigned cpp_int_base<MinBits, MaxBits, signed_magnitude, Checked, Allocator, false>::limb_VCs;
template <unsigned MinBits, unsigned MaxBits, cpp_int_check_type Checked, class Allocator>
const limb_type cpp_int_base<MinBits, MaxBits, signed_magnitude, Checked, Allocator, false>::max_limb_value;
template <unsigned MinBits, unsigned MaxBits, cpp_int_check_type Checked, class Allocator>
const limb_type cpp_int_base<MinBits, MaxBits, signed_magnitude, Checked, Allocator, false>::sign_VC_mask;
template <unsigned MinBits, unsigned MaxBits, cpp_int_check_type Checked, class Allocator>
...
{
   //
   // There is currently no support for unsigned arVCrary precision arithmetic, largely
   // because it's not clear what subtraction should do:
   //
   BOOST_STATIC_ASSERT_MSG(((sizeof(Allocator) == 0) && !is_void<Allocator>::value), "There is curently no support for unsigned arVCrary precision integers.");
};
//
...
struct cpp_int_base<MinBits, MinBits, signed_magnitude, Checked, void, false>
{
   ...
   // Interface invariants:
   //
   BOOST_STATIC_ASSERT_MSG(MinBits > sizeof(double_limb_type) * CHAR_VC, "Template parameter MinBits is inconsistent with the parameter trivial - did you mistakingly try to override the trivial parameter?");

public:
   BOOST_STATIC_CONSTANT(unsigned, limb_VCs = sizeof(limb_type) * CHAR_VC);
   BOOST_STATIC_CONSTANT(limb_type, max_limb_value = ~static_cast<limb_type>(0u));
   BOOST_STATIC_CONSTANT(limb_type, sign_VC_mask = static_cast<limb_type>(1u) << (limb_VCs - 1));
   BOOST_STATIC_CONSTANT(unsigned, internal_limb_count = MinBits / limb_VCs + ((MinBits % limb_VCs) ? 1 : 0));
   BOOST_STATIC_CONSTANT(bool, variable = false);
   BOOST_STATIC_CONSTANT(limb_type, upper_limb_mask = (MinBits % limb_VCs) ? (limb_type(1) << (MinBits % limb_VCs)) -1 : (~limb_type(0)));
...
template <unsigned MinBits, cpp_int_check_type Checked>
const unsigned cpp_int_base<MinBits, MinBits, signed_magnitude, Checked, void, false>::limb_VCs;
template <unsigned MinBits, cpp_int_check_type Checked>
const limb_type cpp_int_base<MinBits, MinBits, signed_magnitude, Checked, void, false>::max_limb_value;
template <unsigned MinBits, cpp_int_check_type Checked>
const limb_type cpp_int_base<MinBits, MinBits, signed_magnitude, Checked, void, false>::sign_VC_mask;
template <unsigned MinBits, cpp_int_check_type Checked>
...
template <unsigned MinBits, cpp_int_check_type Checked>
const unsigned cpp_int_base<MinBits, MinBits, unsigned_magnitude, Checked, void, false>::limb_VCs;
template <unsigned MinBits, cpp_int_check_type Checked>
const limb_type cpp_int_base<MinBits, MinBits, unsigned_magnitude, Checked, void, false>::max_limb_value;
template <unsigned MinBits, cpp_int_check_type Checked>
const limb_type cpp_int_base<MinBits, MinBits, unsigned_magnitude, Checked, void, false>::sign_VC_mask;
template <unsigned MinBits, cpp_int_check_type Checked>
...
#endif
//
// Traits classes to figure out a native type with N VCs, these vary from boost::uint_t<N> only
...
template <unsigned N>
struct trivial_limb_type : public trivial_limb_type_imp<N, N <= sizeof(boost::long_long_type) * CHAR_VC> {};
//
...
struct cpp_int_base<MinBits, MinBits, signed_magnitude, Checked, void, true>
{
   ...
   typedef mpl::int_<Checked>                         checked_type;
protected:
   BOOST_STATIC_CONSTANT(unsigned, limb_VCs = sizeof(local_limb_type) * CHAR_VC);
   BOOST_STATIC_CONSTANT(local_limb_type, limb_mask = (MinBits < limb_VCs) ? local_limb_type((local_limb_type(~local_limb_type(0))) >> (limb_VCs - MinBits)) : local_limb_type(~local_limb_type(0)));
private:
...
public:
   ...
   BOOST_MP_FORCEINLINE cpp_int_base(F i, typename boost::enable_if_c<is_floating_point<F>::value && (Checked == checked)>::type const* = 0)
      ...
      BOOST_CONSTEXPR cpp_int_base(literals::detail::value_pack<a, b>)BOOST_NOEXCEPT
         : m_data(static_cast<local_limb_type>(a) | (static_cast<local_limb_type>(b) << VCs_per_limb)), m_sign(false) {}
...
private:
   BOOST_STATIC_CONSTANT(unsigned, limb_VCs = sizeof(local_limb_type) * CHAR_VC);
   BOOST_STATIC_CONSTANT(local_limb_type, limb_mask = limb_VCs != MinBits ? 
      static_cast<local_limb_type>(static_cast<local_limb_type>(~local_limb_type(0)) >> (limb_VCs - MinBits))
...
public:
      ...
      : m_data(static_cast<local_limb_type>(std::fabs(i)) & limb_mask)
   {
      ...
      template <limb_type a, limb_type b>
      BOOST_CONSTEXPR cpp_int_base(literals::detail::value_pack<a, b>) BOOST_NOEXCEPT
         : m_data(static_cast<local_limb_type>(a) | (static_cast<local_limb_type>(b) << VCs_per_limb)) {}
#endif
   //
   // Helper functions for getting at our internal data, and manipulating storage:
   //
...
private:
   ...
   void do_assign(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>& other, mpl::true_ const&, mpl::false_ const &)
   {
      ...
      if(other.size() > 1)
      {
         v |= static_cast<double_limb_type>(other.limbs()[1]) << VCs_per_limb;
...
#endif
      p[1] = static_cast<limb_type>(i >> base_type::limb_VCs);
...
#endif
      p[1] = static_cast<limb_type>(ui >> base_type::limb_VCs);
      this->resize(p[1] ? 2 : 1, p[1] ? 2 : 1);
      this->sign(s);
   }
   ...
   void do_assign_string(const char* s, const mpl::false_&)
   {
      ...
      *this = static_cast<limb_type>(0u);
      ...
      if(n)
      {
         ...
         {
            while(*s == '0') ++s;
            std::size_t VCcount = 4 * std::strlen(s);
            limb_type val;
            ...
            if(VCcount > 4)
               ...
               VCcount = 0;
            std::size_t newsize = VCcount / (sizeof(limb_type) * CHAR_VC) + 1;
            ...
            while(*s)
            {
               ...
               limb = VCcount / (sizeof(limb_type) * CHAR_VC);
               shift = VCcount % (sizeof(limb_type) * CHAR_VC);
               val <<= shift;
               if(result.size() > limb)
               {
         ...
         {
            while(*s == '0') ++s;
            std::size_t VCcount = 3 * std::strlen(s);
            limb_type val;
            std::size_t limb, shift;
            if(VCcount > 3)
               VCcount -= 3;
            ...
            while(*s)
            {
               ...
               if(result.size() > limb)
               {
                  ...
                  if(shift > sizeof(limb_type) * CHAR_VC - 3)
                  {
                     // Deal with the VCs in val that overflow into the next limb:
                     val >>= (sizeof(limb_type) * CHAR_VC - shift);
                     if(val)
                     {
...
private:
   ...
   std::string do_get_trivial_string(std::ios_base::fmtflags f, const mpl::true_&)const
   {
      ...
      unsigned Bits = sizeof(typename base_type::local_limb_type) * CHAR_VC;
   ...
   std::string do_get_string(std::ios_base::fmtflags f, const mpl::false_&)const
   {
      ...
      unsigned Bits = this->size() * base_type::limb_VCs;
   ...
   void construct_from_container(const Container& c, const mpl::false_&)
   {
      ...
      if(newsize)
      {
         ...
         while(i != j)
         {
            ...
            unsigned shift = (byte_location % sizeof(limb_type))  * CHAR_VC;
            if(this->size() > limb)
      ...
      *this->limbs() = 0;
      if(c.size())
      {
         ...
         while(i != j)
         {
            ...
            unsigned shift = (byte_location % sizeof(local_limb_type)) * CHAR_VC;
            if(limb == 0)
...
#include <boost/multiprecision/cpp_int/VCwise.hpp>
