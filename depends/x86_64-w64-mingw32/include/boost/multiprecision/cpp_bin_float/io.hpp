...
//
// Multiplies a by b and shifts the result so it fits inside max_VCs VCs, 
// returns by how much the result was shifted.
//
template <class I>
inline I restricted_multiply(cpp_int& result, const cpp_int& a, const cpp_int& b, I max_VCs, boost::int64_t& error)
{
   result = a * b;
   I gb = msb(result);
   I rshift = 0;
   if(gb > max_VCs)
   {
      ...
      if(rshift)
      {
         ...
         if(VC_test(result, static_cast<unsigned>(rshift - 1)))
         {
            if(lb == rshift - 1)
               roundup = 1;
            else
...
//
// Computes a^e shifted to the right so it fits in max_VCs, returns how far
// to the right we are shifted.
//
template <class I>
inline I restricted_pow(cpp_int& result, const cpp_int& a, I e, I max_VCs, boost::int64_t& error)
{
   ...
   {
      return restricted_multiply(result, a, a, max_VCs, error);
   }
   else if(e == 3)
   {
      exp = restricted_multiply(result, a, a, max_VCs, error);
      exp += restricted_multiply(result, result, a, max_VCs, error);
      return exp;
   }
   I p = e / 2;
   exp = restricted_pow(result, a, p, max_VCs, error);
   exp *= 2;
   exp += restricted_multiply(result, result, result, max_VCs, error);
   if(e & 1)
      exp += restricted_multiply(result, result, a, max_VCs, error);
   return exp;
}
...
inline int get_round_mode(const cpp_int& what, boost::int64_t location, boost::int64_t error)
{
   ...
   if(error_radius && ((int)msb(error_radius) >= location))
      return -1;
   if(VC_test(what, static_cast<unsigned>(location)))
   {
   ...
   {
      cpp_int t = what + error_radius;
      return VC_test(t, static_cast<unsigned>(location)) ? -1 : 0;
   }
   return 0;
}
...
cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>& cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::operator=(const char *s)
{
   ...
   static const boost::intmax_t max_digits_seen = 4 + (cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count * 301L) / 1000;
   bool ss = false;
   //
   // Extract the sign:
   //
   ...
   // a value as small as possible > cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count to avoid large computations
   // and excessive memory usage, but we also want to avoid having to
   // up the computation and start again at a higher precision.
   // So we round cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count up to the nearest whole number of limbs, and add
   ...
   boost::intmax_t max_VCs = cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count + 32;
#else
   boost::intmax_t max_VCs = cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count + ((cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count % limb_VCs) ? (limb_VCs - cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count % limb_VCs) : 0) + limb_VCs;
#endif
   ...
   if(decimal_exp >= 0)
   {
      ...
      do
      {
         ...
         if(decimal_exp)
         {
            calc_exp = boost::multiprecision::cpp_bf_io_detail::restricted_pow(t, cpp_int(5), decimal_exp, max_VCs, error);
            calc_exp += boost::multiprecision::cpp_bf_io_detail::restricted_multiply(t, t, n, max_VCs, error);
         }
         else
            t = n;
         final_exponent = (boost::int64_t)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - 1 + decimal_exp + calc_exp;
         int rshift = msb(t) - cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count + 1;
         if(rshift > 0)
         {
   ...
   else
   {
      ...
      // division so as to produce at least an N-VC result which we can
      // round according to the remainder.
      //cpp_int d = pow(cpp_int(5), -decimal_exp);
      do
      {
         cpp_int d;
         calc_exp = boost::multiprecision::cpp_bf_io_detail::restricted_pow(d, cpp_int(5), -decimal_exp, max_VCs, error);
         int shift = (int)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - msb(n) + msb(d);
         final_exponent = cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - 1 + decimal_exp - calc_exp;
         if(shift > 0)
         {
            n <<= shift;
         ...
         divide_qr(n, d, q, r);
         int gb = msb(q);
         BOOST_ASSERT((gb >= static_cast<int>(cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count) - 1));
         //
         // Check for rounding conditions we have to
         // handle ourselves:
         //
         int roundup = 0;
         if(gb == cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - 1)
         {
            // Exactly the right number of VCs, use the remainder to round:
            roundup = boost::multiprecision::cpp_bf_io_detail::get_round_mode(r, d, error, q);
         }
         else if(VC_test(q, gb - (int)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count) && ((int)lsb(q) == (gb - (int)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count)))
         {
            // Too many VCs in q and the VCs in q indicate a tie, but we can break that using r,
            // note that the radius of error in r is error/2 * q:
            int lshift = gb - (int)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count + 1;
            q >>= lshift;
            final_exponent += static_cast<Exponent>(lshift);
            BOOST_ASSERT((msb(q) >= cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - 1));
...
std::string cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::str(std::streamsize dig, std::ios_base::fmtflags f) const
{
   ...
   if(exponent() <= cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::max_exponent)
   {
      ...
      boost::intmax_t shift = (boost::intmax_t)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - (boost::intmax_t)exponent() - 1;
      ...
      // So we round cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count up to the nearest whole number of limbs, and add
      // one limb for good measure.  This works very well for small exponents,
      // but for larger exponents we add a few extra limbs to max_VCs:
      //
#ifdef BOOST_MP_STRESS_IO
      boost::intmax_t max_VCs = cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count + 32;
#else
      boost::intmax_t max_VCs = cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count + ((cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count % limb_VCs) ? (limb_VCs - cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count % limb_VCs) : 0) + limb_VCs;
      if(power10)
         max_VCs += (msb(boost::multiprecision::detail::abs(power10)) / 8) * limb_VCs;
#endif
      do
      {
         boost::int64_t error = 0;
         boost::intmax_t calc_exp = 0;
         //
         // Our integer result is: VCs() * 2^-shift * 5^power10
         //
         i = VCs();
         if(shift < 0)
         {
            ...
            {
               cpp_int d;
               calc_exp = boost::multiprecision::cpp_bf_io_detail::restricted_pow(d, cpp_int(5), -power10, max_VCs, error);
               shift += calc_exp;
               ...
               if(roundup < 0)
               {
                  ...
                  max_VCs *= 2;
#endif
                  shift = (boost::intmax_t)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - exponent() - 1 - power10;
                  continue;
               }
            }
         }
         ...
         {
            //
            // Our integer is VCs() * 2^-shift * 10^power10
            //
            if(power10 > 0)
            {
               if(power10)
               {
                  cpp_int t;
                  calc_exp = boost::multiprecision::cpp_bf_io_detail::restricted_pow(t, cpp_int(5), power10, max_VCs, error);
                  calc_exp += boost::multiprecision::cpp_bf_io_detail::restricted_multiply(i, i, t, max_VCs, error);
                  shift -= calc_exp;
               }
               if((shift < 0) || ((shift == 0) && error))
               {
                  // We only get here if we were asked for a crazy number of decimal digits -
                  // more than are present in a 2^max_VCs number.
#ifdef BOOST_MP_STRESS_IO
                  max_VCs += 32;
#else
                  max_VCs *= 2;
