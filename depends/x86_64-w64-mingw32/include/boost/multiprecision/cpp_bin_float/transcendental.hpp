...
{
   static const int VCs = cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count;
   //
   ...
   for(unsigned k = 2; ; ++k)
   {
      ...
      eval_add(res, t);
      if(eval_is_zero(t) || (res.exponent() - VCs > t.exponent()))
         break;
   }
}
...
void eval_exp(cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &res, const cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &arg)
{
   ...
   k = (std::min)(k, (Exponent)(cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count / 4));
   eval_ldexp(t, t, -k);
