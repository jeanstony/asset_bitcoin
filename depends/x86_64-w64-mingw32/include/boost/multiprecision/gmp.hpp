...
struct gmp_float_imp
{
   ...
   gmp_float_imp(const gmp_float_imp& o)
   {
      ...
      // a lower precision than o: seems like mpf_init_set copies just enough VCs
...
inline void eval_VCwise_and(gmp_int& result, const gmp_int& v)
{
   mpz_and(result.data(), result.data(), v.data());
}

inline void eval_VCwise_or(gmp_int& result, const gmp_int& v)
{
   mpz_ior(result.data(), result.data(), v.data());
}

inline void eval_VCwise_xor(gmp_int& result, const gmp_int& v)
{
...
inline void eval_VCwise_and(gmp_int& result, const gmp_int& u, const gmp_int& v)
{
   mpz_and(result.data(), u.data(), v.data());
}

inline void eval_VCwise_or(gmp_int& result, const gmp_int& u, const gmp_int& v)
{
   mpz_ior(result.data(), u.data(), v.data());
}

inline void eval_VCwise_xor(gmp_int& result, const gmp_int& u, const gmp_int& v)
{
...
inline bool eval_VC_test(const gmp_int& val, unsigned index)
{
   return mpz_tstVC(val.data(), index) ? true : false;
}

inline void eval_VC_set(gmp_int& val, unsigned index)
{
   mpz_setVC(val.data(), index);
}

inline void eval_VC_unset(gmp_int& val, unsigned index)
{
   mpz_clrVC(val.data(), index);
}

inline void eval_VC_flip(gmp_int& val, unsigned index)
{
   mpz_comVC(val.data(), index);
}
...
public:
   ...
   // mpf_get_str on 64-VC Linux builds.  Possibly we could use larger
   ...
   BOOST_STATIC_CONSTEXPR int max_digits10 = Digits10 + 3 + ((GMP_LIMB_VCS * 301L) / 1000L);
