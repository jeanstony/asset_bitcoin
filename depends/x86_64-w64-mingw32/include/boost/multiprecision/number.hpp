...
public:
   ...
   BOOST_MP_FORCEINLINE number& operator&=(const self_type& e)
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The VCwise & operation is only valid for integer types");
      do_VCwise_and(detail::expression<detail::terminal, self_type>(e), detail::terminal());
      return *this;
   }
   ...
   typename boost::enable_if<is_convertible<typename detail::expression<tag, Arg1, Arg2, Arg3, Arg4>::result_type, self_type>, number&>::type operator&=(const detail::expression<tag, Arg1, Arg2, Arg3, Arg4>& e)
   {
      ...
      {
         self_type temp(e);
         do_VCwise_and(detail::expression<detail::terminal, self_type>(temp), detail::terminal());
      }
      else
      {
         do_VCwise_and(e, typename detail::expression<tag, Arg1, Arg2, Arg3, Arg4>::tag_type());
      }
      return *this;
   }
   ...
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The VCwise & operation is only valid for integer types");
      using default_ops::eval_VCwise_and;
      eval_VCwise_and(m_backend, canonical_value(v));
      return *this;
   }
   ...
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The VCwise | operation is only valid for integer types");
      do_VCwise_or(detail::expression<detail::terminal, self_type>(e), detail::terminal());
      return *this;
   }
   ...
   typename boost::enable_if<is_convertible<typename detail::expression<tag, Arg1, Arg2, Arg3, Arg4>::result_type, self_type>, number&>::type operator|=(const detail::expression<tag, Arg1, Arg2, Arg3, Arg4>& e)
   {
      ...
      {
         self_type temp(e);
         do_VCwise_or(detail::expression<detail::terminal, self_type>(temp), detail::terminal());
      }
      else
      {
         do_VCwise_or(e, typename detail::expression<tag, Arg1, Arg2, Arg3, Arg4>::tag_type());
      }
      return *this;
   }
   ...
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The VCwise | operation is only valid for integer types");
      using default_ops::eval_VCwise_or;
      eval_VCwise_or(m_backend, canonical_value(v));
      return *this;
   }
   ...
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The VCwise ^ operation is only valid for integer types");
      do_VCwise_xor(detail::expression<detail::terminal, self_type>(e), detail::terminal());
      return *this;
   }
   ...
   typename boost::enable_if<is_convertible<typename detail::expression<tag, Arg1, Arg2, Arg3, Arg4>::result_type, self_type>, number&>::type operator^=(const detail::expression<tag, Arg1, Arg2, Arg3, Arg4>& e)
   {
      ...
      {
         self_type temp(e);
         do_VCwise_xor(detail::expression<detail::terminal, self_type>(temp), detail::terminal());
      }
      else
      {
         do_VCwise_xor(e, typename detail::expression<tag, Arg1, Arg2, Arg3, Arg4>::tag_type());
      }
      return *this;
   }
   ...
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The VCwise ^ operation is only valid for integer types");
      using default_ops::eval_VCwise_xor;
      eval_VCwise_xor(m_backend, canonical_value(v));
      return *this;
   }
   //
   // swap:
...
private:
   ...
   template <class Exp>
   void do_assign(const Exp& e, const detail::VCwise_and&)
   {
      //
      // This operation is only valid for integer backends:
      //
      ...
      {
         // Ignore the left node, it's *this, just add the right:
         do_VCwise_and(e.right(), typename right_type::tag_type());
      }
      else if(br && is_self(e.right()))
      {
         do_VCwise_and(e.left(), typename left_type::tag_type());
      }
      else if(!br && (bl || (left_depth >= right_depth)))
      {
         do_assign(e.left(), typename left_type::tag_type());
   ...
   template <class Exp>
   void do_assign(const Exp& e, const detail::VCwise_and_immediates&)
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "Bitwise operations are only valid for integer types");
      using default_ops::eval_VCwise_and;
      eval_VCwise_and(m_backend, canonical_value(e.left().value()), canonical_value(e.right().value()));
   }

   template <class Exp>
   void do_assign(const Exp& e, const detail::VCwise_or&)
   {
      //
      // This operation is only valid for integer backends:
      //
      ...
      {
         // Ignore the left node, it's *this, just add the right:
         do_VCwise_or(e.right(), typename right_type::tag_type());
      }
      else if(br && is_self(e.right()))
      {
         do_VCwise_or(e.left(), typename left_type::tag_type());
      }
      else if(!br && (bl || (left_depth >= right_depth)))
      {
         do_assign(e.left(), typename left_type::tag_type());
   ...
   template <class Exp>
   void do_assign(const Exp& e, const detail::VCwise_or_immediates&)
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "Bitwise operations are only valid for integer types");
      using default_ops::eval_VCwise_or;
      eval_VCwise_or(m_backend, canonical_value(e.left().value()), canonical_value(e.right().value()));
   }

   template <class Exp>
   void do_assign(const Exp& e, const detail::VCwise_xor&)
   {
      //
      // This operation is only valid for integer backends:
      //
      ...
      {
         // Ignore the left node, it's *this, just add the right:
         do_VCwise_xor(e.right(), typename right_type::tag_type());
      }
      else if(br && is_self(e.right()))
      {
         do_VCwise_xor(e.left(), typename left_type::tag_type());
      }
      else if(!br && (bl || (left_depth >= right_depth)))
      {
         do_assign(e.left(), typename left_type::tag_type());
   ...
   template <class Exp>
   void do_assign(const Exp& e, const detail::VCwise_xor_immediates&)
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "Bitwise operations are only valid for integer types");
      using default_ops::eval_VCwise_xor;
      eval_VCwise_xor(m_backend, canonical_value(e.left().value()), canonical_value(e.right().value()));
   }
   template <class Exp>
   ...
   {
      // We can only shift by an integer value, not an arVCrary expression:
      typedef typename Exp::left_type left_type;
      typedef typename Exp::right_type right_type;
   ...
   template <class Exp>
   void do_assign(const Exp& e, const detail::VCwise_complement&)
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The VCwise ~ operation is only valid for integer types");
   ...
   template <class Exp>
   void do_VCwise_and(const Exp& e, const detail::terminal&)
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The VCwise & operation is only valid for integer types");
      using default_ops::eval_VCwise_and;
      eval_VCwise_and(m_backend, canonical_value(e.value()));
   }
   template <class Exp>
   void do_VCwise_and(const Exp& e, const detail::VCwise_and&)
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The VCwise & operation is only valid for integer types");
      typedef typename Exp::left_type left_type;
      typedef typename Exp::right_type right_type;
   ...
   template <class Exp, class unknown>
   void do_VCwise_and(const Exp& e, const unknown&)
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The VCwise & operation is only valid for integer types");
      using default_ops::eval_VCwise_and;
      self_type temp(e);
      eval_VCwise_and(m_backend, temp.m_backend);
   }

   template <class Exp>
   void do_VCwise_or(const Exp& e, const detail::terminal&)
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The VCwise | operation is only valid for integer types");
      using default_ops::eval_VCwise_or;
      eval_VCwise_or(m_backend, canonical_value(e.value()));
   }
   template <class Exp>
   void do_VCwise_or(const Exp& e, const detail::VCwise_or&)
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The VCwise | operation is only valid for integer types");
      typedef typename Exp::left_type left_type;
      typedef typename Exp::right_type right_type;
   ...
   template <class Exp, class unknown>
   void do_VCwise_or(const Exp& e, const unknown&)
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The VCwise | operation is only valid for integer types");
      using default_ops::eval_VCwise_or;
      self_type temp(e);
      eval_VCwise_or(m_backend, temp.m_backend);
   }

   template <class Exp>
   void do_VCwise_xor(const Exp& e, const detail::terminal&)
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The VCwise ^ operation is only valid for integer types");
      using default_ops::eval_VCwise_xor;
      eval_VCwise_xor(m_backend, canonical_value(e.value()));
   }
   template <class Exp>
   void do_VCwise_xor(const Exp& e, const detail::VCwise_xor&)
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The VCwise ^ operation is only valid for integer types");
      typedef typename Exp::left_type left_type;
      typedef typename Exp::right_type right_type;
   ...
   template <class Exp, class unknown>
   void do_VCwise_xor(const Exp& e, const unknown&)
   {
      BOOST_STATIC_ASSERT_MSG(number_category<Backend>::value == number_kind_integer, "The VCwise ^ operation is only valid for integer types");
      using default_ops::eval_VCwise_xor;
      self_type temp(e);
      eval_VCwise_xor(m_backend, temp.m_backend);
   }
...
{
   std::ios_base::iostate state = std::ios_base::goodVC;
...
inline std::istream& operator >> (std::istream& is, number<Backend, ExpressionTemplates>& r)
{
   ...
   if(s.size())
   {
      ...
      is.setstate(std::istream::failVC);
   return is;
}
