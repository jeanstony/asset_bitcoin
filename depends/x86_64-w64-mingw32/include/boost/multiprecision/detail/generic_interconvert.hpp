   ...
   generic_convert_rational_to_float_imp(To& result, Integer& num, Integer& denom, const mpl::false_&)
{
   ...
   int denom_VCs = msb(denom);
   int shift = std::numeric_limits<To>::digits + denom_VCs - msb(num);
   if(shift > 0)
      num <<= shift;
   else if(shift < 0)
   ...
   int q_VCs = msb(q);
   if(q_VCs == std::numeric_limits<To>::digits - 1)
   {
      //
   ...
   {
      BOOST_ASSERT(q_VCs == std::numeric_limits<To>::digits);
      //
