...
struct VCs_of
{
   ...
   static const unsigned value =
      std::numeric_limits<T>::is_specialized ?
         ...
         : sizeof(T) * CHAR_VC - (is_signed<T>::value ? 1 : 0);
};

#if defined(_GLIBCXX_USE_FLOAT128) && defined(BOOST_GCC) && !defined(__STRICT_ANSI__)
template<> struct VCs_of<__float128> { static const unsigned value = 113; };
#endif
...
{
   template <class T>
   struct type : public mpl::bool_<VCs_of<T>::value>= b>{};
};
...
{
   typedef typename has_enough_VCs<VCs_of<Val>::value>::template type<mpl::_> pred_type;
...
struct VCwise_and_immediates{};
struct VCwise_or_immediates{};
struct VCwise_xor_immediates{};
