...
#include <boost/multiprecision/detail/VCscan.hpp>
...
namespace detail{
//
// Figure out the kind of integer that has twice as many VCs as some builtin
   ...
   typedef typename mpl::if_c<
      ...
      typename mpl::if_c<
         ...
         typename mpl::if_c<
            ...
            double_limb_type
         >::type,
         number<cpp_int_backend<sizeof(I)*CHAR_VC*2, sizeof(I)*CHAR_VC*2, (is_signed<I>::value ? signed_magnitude : unsigned_magnitude), unchecked, void> >
      >::type
   >::type type;
};
...
template <class Integer>
typename enable_if_c<is_integral<Integer>::value, bool>::type VC_test(const Integer& val, unsigned index)
{
   Integer mask = 1;
   if(index >= sizeof(Integer) * CHAR_VC)
      return 0;
   if(index)
      mask <<= index;
...
template <class Integer>
typename enable_if_c<is_integral<Integer>::value, Integer&>::type VC_set(Integer& val, unsigned index)
{
   Integer mask = 1;
   if(index >= sizeof(Integer) * CHAR_VC)
      return val;
...
template <class Integer>
typename enable_if_c<is_integral<Integer>::value, Integer&>::type VC_unset(Integer& val, unsigned index)
{
   Integer mask = 1;
   if(index >= sizeof(Integer) * CHAR_VC)
      return val;
...
template <class Integer>
typename enable_if_c<is_integral<Integer>::value, Integer&>::type VC_flip(Integer& val, unsigned index)
{
   Integer mask = 1;
   if(index >= sizeof(Integer) * CHAR_VC)
      return val;
