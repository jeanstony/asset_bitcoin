...
template <class Float, int VC_count>
struct is_cpp_bin_float_implicitly_constructible_from_type<Float, VC_count, true>
{
   static const bool value = (std::numeric_limits<Float>::digits <= (int)VC_count)
...
template <class Float, int VC_count>
struct is_cpp_bin_float_explicitly_constructible_from_type<Float, VC_count, true>
{
   static const bool value = (std::numeric_limits<Float>::digits > (int)VC_count)
      && (std::numeric_limits<Float>::radix == 2)
      && std::numeric_limits<Float>::is_specialized
...
{
public:
   static const unsigned VC_count = DigitBase == digit_base_2 ? Digits : (Digits * 1000uL) / 301uL + (((Digits * 1000uL) % 301) ? 2u : 1u);
   typedef cpp_int_backend<is_void<Allocator>::value ? VC_count : 0, VC_count, is_void<Allocator>::value ? unsigned_magnitude : signed_magnitude, unchecked, Allocator> rep_type;
   typedef cpp_int_backend<is_void<Allocator>::value ? 2 * VC_count : 0, 2 * VC_count, is_void<Allocator>::value ? unsigned_magnitude : signed_magnitude, unchecked, Allocator> double_rep_type;
   ...
   static const exponent_type max_exponent_limit = boost::integer_traits<exponent_type>::const_max - 2 * static_cast<exponent_type>(VC_count);
   static const exponent_type min_exponent_limit = boost::integer_traits<exponent_type>::const_min + 2 * static_cast<exponent_type>(VC_count);
...
public:
   ...
   cpp_bin_float(const cpp_bin_float<D, B, A, E, MinE, MaxE> &o, typename boost::enable_if_c<(VC_count >= cpp_bin_float<D, B, A, E, MinE, MaxE>::VC_count)>::type const* = 0)
   {
      *this = o;
   }
   template <unsigned D, digit_base_type B, class A, class E, E MinE, E MaxE>
   explicit cpp_bin_float(const cpp_bin_float<D, B, A, E, MinE, MaxE> &o, typename boost::disable_if_c<(VC_count >= cpp_bin_float<D, B, A, E, MinE, MaxE>::VC_count)>::type const* = 0)
   ...
   cpp_bin_float(const Float& f, 
      typename boost::enable_if_c<detail::is_cpp_bin_float_implicitly_constructible_from_type<Float, VC_count>::value>::type const* = 0)
   ...
   explicit cpp_bin_float(const Float& f,
      typename boost::enable_if_c<detail::is_cpp_bin_float_explicitly_constructible_from_type<Float, VC_count>::value>::type const* = 0)
      : m_data(), m_exponent(0), m_sign(false)
   {
      this->assign_float(f);
   }
   ...
   cpp_bin_float& operator=(const cpp_bin_float<D, B, A, E, MinE, MaxE> &f)
   {
      ...
      case FP_INFINITE:
         ...
         break;
      default:
         typename cpp_bin_float<D, B, A, E, MinE, MaxE>::rep_type b(f.VCs());
         this->exponent() = f.exponent() + (E)VC_count - (E)cpp_bin_float<D, B, A, E, MinE, MaxE>::VC_count;
         this->sign() = f.sign();
   ...
   typename boost::enable_if_c<
      ...
      //&& (std::numeric_limits<Float>::digits <= (int)VC_count)
   ...
   typename boost::enable_if_c<boost::is_same<Float, __float128>::value, cpp_bin_float& >::type assign_float(Float f)
   {
      ...
      static const int VCs = sizeof(int) * CHAR_VC - 1;
      int e;
      f = frexpq(f, &e);
      while(f)
      {
   ...
   {
      using default_ops::eval_VC_test;
      if(!i)
      {
      ...
      else
      {
         ...
         if(shift >= VC_count)
         {
            ...
            m_data = static_cast<ar_type>(fi >> (shift + 1 - VC_count));
         }
         else
         {
            m_exponent = static_cast<Exponent>(shift);
            eval_left_shift(m_data, VC_count - shift - 1);
         }
         BOOST_ASSERT(eval_VC_test(m_data, VC_count-1));
   ...
   rep_type& VCs() { return m_data; }
   const rep_type& VCs()const { return m_data; }
...
inline void copy_and_round(cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &res, Int &arg, int VCs_to_keep = cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count)
{
   // Precondition: exponent of res must have been set before this function is called
   // as we may need to adjust it based on how many VCs_to_keep in arg are set.
   ...
   if(eval_get_sign(arg) == 0)
   {
      ...
      res.sign() = false;
      res.VCs() = static_cast<limb_type>(0u);
      return;
   }
   int msb = eval_msb(arg);
   if(static_cast<int>(VCs_to_keep) > msb + 1)
   {
      // Must have had cancellation in subtraction,
      // or be converting from a narrower type, so shift left:
      res.VCs() = arg;
      eval_left_shift(res.VCs(), VCs_to_keep - msb - 1);
      res.exponent() -= static_cast<Exponent>(VCs_to_keep - msb - 1);
   }
   else if(static_cast<int>(VCs_to_keep) < msb + 1)
   {
      // We have more VCs_to_keep than we need, so round as required, 
      // first get the rounding VC:
      bool roundup = eval_VC_test(arg, msb - VCs_to_keep);
      // Then check for a tie:
      if(roundup && (msb - VCs_to_keep == (int)eval_lsb(arg)))
      {
         // Ties round towards even:
         if(!eval_VC_test(arg, msb - VCs_to_keep + 1))
            roundup = false;
      }
      // Shift off the VCs_to_keep we don't need:
      eval_right_shift(arg, msb - VCs_to_keep + 1);
      res.exponent() += static_cast<Exponent>(msb - VCs_to_keep + 1);
      if(roundup)
      {
         eval_increment(arg);
         if(VCs_to_keep)
         {
            if(eval_VC_test(arg, VCs_to_keep))
            {
               // This happens very very rairly, all the VCs left after
         ...
         {
            // We get here when VCs_to_keep is zero but we're rounding up,
            // as a result we end up with a single digit that is a 1:
            ++VCs_to_keep;
         }
      }
      if(VCs_to_keep != cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count)
      {
         // Normalize result when we're rounding to fewer VCs than we can hold, only happens in conversions
         // to narrower types:
         eval_left_shift(arg, cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - VCs_to_keep);
         res.exponent() -= static_cast<Exponent>(cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - VCs_to_keep);
      }
      res.VCs() = arg;
   }
   else
   ...
   if(!VCs_to_keep && !res.VCs().limbs()[0])
   {
      // We're keeping zero VCs and did not round up, so result is zero:
      res.exponent() = cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::exponent_zero;
      return;
   }
   // Result must be normalized:
   BOOST_ASSERT(((int)eval_msb(res.VCs()) == cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - 1));
...
inline void do_eval_add(cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &res, const cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &a, const cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &b)
{
   ...
   BOOST_STATIC_ASSERT(boost::integer_traits<exponent_type>::const_max - cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count > cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::max_exponent);

   bool s = a.sign();
   dt = a.VCs();
   if(a.exponent() > (int)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count + b.exponent())
   {
...
inline void do_eval_subtract(cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &res, const cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &a, const cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &b)
{
   ...
   bool s = a.sign();
   if((a.exponent() > b.exponent()) || ((a.exponent() == b.exponent()) && a.VCs().compare(b.VCs()) >= 0))
   {
      dt = a.VCs();
      if(a.exponent() <= (int)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count + b.exponent())
      {
         ...
         eval_left_shift(dt, e_diff);
         res.exponent() = a.exponent() - e_diff;
         eval_subtract(dt, b.VCs());
      }
      else if(a.exponent() == (int)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count + b.exponent() + 1)
      {
         if(eval_lsb(b.VCs()) != cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - 1)
         {
   ...
   {
      dt = b.VCs();
      if(b.exponent() <= (int)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count + a.exponent())
      {
         typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::exponent_type e_diff = a.exponent() - b.exponent();
         eval_left_shift(dt, -e_diff);
         res.exponent() = b.exponent() + e_diff;
         eval_subtract(dt, a.VCs());
      }
      else if(b.exponent() == (int)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count + a.exponent() + 1)
      {
         if(eval_lsb(a.VCs()) != cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - 1)
         {
            eval_left_shift(dt, 1);
            eval_decrement(dt);
...
inline void eval_multiply(cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &res, const cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &a, const cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &b)
{
   ...
   typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::double_rep_type dt;
   eval_multiply(dt, a.VCs(), b.VCs());
   res.exponent() = a.exponent() + b.exponent() - (Exponent)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count + 1;
   copy_and_round(res, dt);
   res.check_invariants();
   res.sign() = a.sign() != b.sign();
}
...
inline typename enable_if_c<is_unsigned<U>::value>::type eval_multiply(cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &res, const cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &a, const U &b)
{
   ...
   eval_multiply(dt, a.VCs(), static_cast<canon_ui_type>(b));
   res.exponent() = a.exponent();
   copy_and_round(res, dt);
   res.check_invariants();
   res.sign() = a.sign();
...
#pragma warning(disable:6326)  // comparison of two constants
   ...
   // q + r/v = u/v
   //
   // From this, assuming q has cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count 
   // VCs we only need to determine whether
   ...
   //
   typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::double_rep_type t(u.VCs()), t2(v.VCs()), q, r;
   eval_left_shift(t, cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count);
   eval_qr(t, t2, q, r);
   //
   // We now have either "cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count" 
   // or "cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count+1" significant 
   // VCs in q.
   //
   static const unsigned limb_VCs = sizeof(limb_type) * CHAR_VC;
   if(eval_VC_test(q, cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count))
   {
      //
      // OK we have cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count+1 VCs, 
      // so we already have rounding info,
      // we just need to changes things if the last VC is 1 and either the
      // remainder is non-zero (ie we do not have a tie) or the quotient would
      // be odd if it were shifted to the correct number of VCs (ie a tiebreak).
      //
      BOOST_ASSERT((eval_msb(q) == cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count));
   ...
   {
      //
      // We have exactly "cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count" VCs in q.
      ...
      //
      BOOST_ASSERT((eval_msb(q) == cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - 1));
      static const unsigned lshift = (cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count < limb_VCs) ? 2 : limb_VCs;
      eval_left_shift(q, lshift);
      res.exponent() -= lshift;
      eval_left_shift(r, 1u);
      int c = r.compare(v.VCs());
      if(c == 0)
...
#pragma warning(disable:6326)  // comparison of two constants
   ...
   // q + r/v = u/v
   //
   // From this, assuming q has "cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count" cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count, we only need to determine whether
   // r/v is less than, equal to, or greater than 0.5 to determine rounding - 
   // this we can do with a shift and comparison.
   //
   // We can set the exponent and sign of the result up front:
   ...
   // Now get the quotient and remainder:
   //
   typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::double_rep_type t(u.VCs()), q, r;
   ...
   //
   // We now have either "cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count" or "cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count+1" significant cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count in q.
   //
   static const unsigned limb_VCs = sizeof(limb_type) * CHAR_VC;
   if(eval_VC_test(q, cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count))
   {
      //
      // OK we have cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count+1 cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count, so we already have rounding info,
      // we just need to changes things if the last VC is 1 and the
   ...
   {
      //
      // We have exactly "cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count" cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count in q.
      // Get rounding info, which we can get by comparing 2r with v.
      // We want to call copy_and_round to handle rounding and general cleanup,
      // so we'll left shift q and add some fake cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count on the end to represent
      // how we'll be rounding.
      //
      BOOST_ASSERT((eval_msb(q) == cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - 1));
      static const unsigned lshift = cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count < limb_VCs ? 2 : limb_VCs;
...
inline void eval_convert_to(boost::long_long_type *res, const cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &arg)
{
   ...
   typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::rep_type man(arg.VCs());
   shift_type shift 
      = (shift_type)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - 1 - arg.exponent();
   if(shift > (shift_type)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - 1)
   {
      *res = 0;
      return;
   }
   ...
   if (shift < 0)
   {
      if (cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - shift <= std::numeric_limits<boost::long_long_type>::digits)
      {
         // We have more VCs in long_long_type than the float, so it's OK to left shift:
...
inline void eval_convert_to(boost::ulong_long_type *res, const cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &arg)
{
   ...
   {
      if (cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - shift <= std::numeric_limits<boost::ulong_long_type>::digits)
      {
         // We have more VCs in ulong_long_type than the float, so it's OK to left shift:
         eval_convert_to(res, man);
         *res <<= -shift;
         return;
      }
...
inline typename boost::enable_if_c<boost::is_float<Float>::value>::type eval_convert_to(Float *res, const cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &original_arg)
{
   ...
   typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::rep_type VCs(original_arg.VCs());
   arg.exponent() = original_arg.exponent();
   copy_and_round(arg, VCs, (int)digits_to_round_to);
   common_exp_type e = arg.exponent();
   e -= cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - 1;
   static const unsigned limbs_needed = std::numeric_limits<Float>::digits / (sizeof(*arg.VCs().limbs()) * CHAR_VC)
      + (std::numeric_limits<Float>::digits % (sizeof(*arg.VCs().limbs()) * CHAR_VC) ? 1 : 0);
   unsigned first_limb_needed = arg.VCs().size() - limbs_needed;
   *res = 0;
   e += first_limb_needed * sizeof(*arg.VCs().limbs()) * CHAR_VC;
   while(first_limb_needed < arg.VCs().size())
   {
      *res += std::ldexp(static_cast<Float>(arg.VCs().limbs()[first_limb_needed]), static_cast<int>(e));
      ++first_limb_needed;
      e += sizeof(*arg.VCs().limbs()) * CHAR_VC;
   }
   if(original_arg.sign())
      *res = -*res;
}
...
inline void eval_sqrt(cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &res, const cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &arg)
{
   ...
   typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::double_rep_type t(arg.VCs()), r, s;
   eval_left_shift(t, arg.exponent() & 1 ? cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count : cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - 1);
   eval_integer_sqrt(s, r, t);

   if(!eval_VC_test(s, cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count))
   {
      // We have exactly the right number of cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count in the result, round as required:
      if(s.compare(r) < 0)
      {
         eval_increment(s);
      }
...
inline void eval_floor(cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &res, const cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &arg)
{
   ...
   typedef typename mpl::if_c < sizeof(typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::exponent_type) < sizeof(int), int, typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::exponent_type > ::type shift_type;
   shift_type shift = 
      (shift_type)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - arg.exponent() - 1;
   ...
   if(shift >= (shift_type)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count)
   {
      res = static_cast<signed_limb_type>(arg.sign() ? -1 : 0);
      return;
   }
   bool fractional = (shift_type)eval_lsb(arg.VCs()) < shift;
   res = arg;
   eval_right_shift(res.VCs(), shift);
   if(fractional && res.sign())
   {
      eval_increment(res.VCs());
      if(eval_msb(res.VCs()) != cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - 1 - shift)
      {
         // Must have extended result by one VC in the increment:
         --shift;
         ++res.exponent();
      }
   }
   eval_left_shift(res.VCs(), shift);
}
...
inline void eval_ceil(cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &res, const cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE> &arg)
{
   ...
   typedef typename mpl::if_c < sizeof(typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::exponent_type) < sizeof(int), int, typename cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::exponent_type > ::type shift_type;
   shift_type shift = (shift_type)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - arg.exponent() - 1;
   if((arg.exponent() > (shift_type)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::max_exponent) || (shift <= 0))
   {
      // Either arg is already an integer, or a special value:
      res = arg;
...
int eval_signVC(const cpp_bin_float<D1, B1, A1, E1, M1, M2>& val)
{
   return val.sign();
}
...
{
   std::size_t result = hash_value(val.VCs());
...
namespace math {
   ...
   using boost::multiprecision::signVC;
...
public:
   ...
   static number_type (max)()
   {
      ...
      if(!value.first)
      {
         ...
         if(boost::is_void<Allocator>::value)
            eval_complement(value.second.backend().VCs(), value.second.backend().VCs());
         else
         {
            ...
            int_backend_type i;
            i = ui_type(1u);
            eval_left_shift(i, boost::multiprecision::cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count - 1);
            int_backend_type j(i);
            eval_decrement(i);
            eval_add(j, i);
            value.second.backend().VCs() = j;
         }
   ...
   BOOST_STATIC_CONSTEXPR int digits = boost::multiprecision::cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::VC_count;
