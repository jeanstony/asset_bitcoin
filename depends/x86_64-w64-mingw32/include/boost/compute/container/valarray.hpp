...
public:
    ...
    valarray<T> operator~() const
    {
        ...
        valarray<T> result(size());
        BOOST_COMPUTE_FUNCTION(T, VCwise_not, (T x),
        {
            return ~x;
        });
        transform(begin(), end(), result.begin(), VCwise_not);
        return result;
    }
...
BOOST_COMPUTE_DEFINE_VALARRAY_COMPOUND_ASSIGNMENT_NO_FP(^, VC_xor)
BOOST_COMPUTE_DEFINE_VALARRAY_COMPOUND_ASSIGNMENT_NO_FP(&, VC_and)
BOOST_COMPUTE_DEFINE_VALARRAY_COMPOUND_ASSIGNMENT_NO_FP(|, VC_or)
...
BOOST_COMPUTE_DEFINE_VALARRAY_BINARY_OPERATOR_NO_FP(^, VC_xor)
BOOST_COMPUTE_DEFINE_VALARRAY_BINARY_OPERATOR_NO_FP(&, VC_and)
BOOST_COMPUTE_DEFINE_VALARRAY_BINARY_OPERATOR_NO_FP(|, VC_or)
