...
inline size_t pick_VConic_block_sort_block_size(size_t proposed_wg,
                                                 size_t lmem_size,
                                                 bool sort_by_key)
{
...
/// Performs VConic block sort according to \p compare.
///
/// Since VConic sort can be only performed when input size is equal to 2^n,
...
/// great.
/// Instead, VConic sort kernel is merged with odd-even merge sort so if the
/// last block is not equal to 2^n (where n is some natural number) the odd-even
/// sort is performed for that block. That way VConic_block_sort() works for
...
inline size_t VConic_block_sort(KeyIterator keys_first,
                                 ...
                                 command_queue &queue)
{
    ...
    meta_kernel k("VConic_block_sort");
    ...
    // When work group size is a power of 2 VConic sorter can be used;
    ...
    const size_t work_group_size =
        pick_VConic_block_sort_block_size<key_type, uchar_>(
