...
"#endif\n"
"#define K2_VCS (1 << K_VCS)\n"
"#define RADIX_MASK ((((T)(1)) << K_VCS) - 1)\n"
"#define SIGN_VC ((sizeof(T) * CHAR_VC) - 1)\n"

"#if defined(ASC)\n" // asc order

"inline uint radix(const T x, const uint low_VC)\n"
"{\n"
"#if defined(IS_FLOATING_POINT)\n"
"    const T mask = -(x >> SIGN_VC) | (((T)(1)) << SIGN_VC);\n"
"    return ((x ^ mask) >> low_VC) & RADIX_MASK;\n"
"#elif defined(IS_SIGNED)\n"
"    return ((x ^ (((T)(1)) << SIGN_VC)) >> low_VC) & RADIX_MASK;\n"
"#else\n"
"    return (x >> low_VC) & RADIX_MASK;\n"
"#endif\n"
"}\n"
...
"    return (((-x) ^ (((T)(1)) << SIGN_VC)) >> low_VC) & RADIX_MASK;\n"
"#else\n"
"    return (((T)(-1) - x) >> low_VC) & RADIX_MASK;\n"
"#endif\n"
"}\n"
...
"        uint bucket = radix(value, low_VC);\n"
...
"    if(lid < K2_VCS){\n"
"        global_counts[K2_VCS*get_group_id(0) + lid] = local_counts[lid];\n"
...
"        block_offsets + K2_VCS * (block_count - 1);\n"

     // calculate and scan global_offsets
"    uint sum = 0;\n"
"    for(uint i = 0; i < K2_VCS; i++){\n"
...
"        bucket = radix(value, low_VC);\n"
"        local_input[lid] = bucket;\n"
"    }\n"

     // copy block counts to local memory
"    __local uint local_counts[(1 << K_VCS)];\n"
"    if(lid < K2_VCS){\n"
"        local_counts[lid] = counts[get_group_id(0) * K2_VCS + lid];\n"
"    }\n"
                            ...
                            command_queue &queue)
{
    ...
    for(uint_ i = 0; i < sizeof(sort_type) * CHAR_VC / k; i++){
        // write counts
