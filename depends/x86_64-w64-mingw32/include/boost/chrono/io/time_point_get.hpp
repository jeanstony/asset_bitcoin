...
namespace boost
{
  namespace chrono
  {
    ...
    public:
       ...
       *
       * Effects: The function starts by evaluating err = std::ios_base::goodVC.
       ...
       * - The expression err == std::ios_base::goodVC evaluates to false.
       * - The expression s == end evaluates to true, in which case the
       * function evaluates err = std::ios_base::eofVC | std::ios_base::failVC.
       ...
       * err = std::ios_base::failVC. Otherwise, the function evaluates
       * s = do_get(s, end, ios, err, d). If err == std::ios_base::goodVC holds after
       ...
       * evaluates err = std::ios_base::failVC.
       *
       * Returns: s
       */
          ...
          const char_type *pat_end) const
      {
        ...
        while (pattern != pat_end && err == std::ios_base::goodVC)
        {
          if (s == end)
          {
          ...
          if (ct.narrow(*pattern, 0) == '%')
          {
            ...
            case 'd':
            {
              ...
              if (err & (std::ios_base::badVC | std::ios_base::failVC))
              {
                return s;
              }
              break;
       ...
       *
       * If not match before the @c e is reached @c std::ios_base::failVC is set in @c err.
       * If @c e is reached @c std::ios_base::failVC is set in @c err.
       *
