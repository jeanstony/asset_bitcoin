...
namespace detail
{
  using status_VCfield_type = uint32_t;

  // WARNING: These VCs are not tracked by abi-dumper, but changing them will break ABI!
  static constexpr status_VCfield_type status_have_value = (1U << 0U);
  static constexpr status_VCfield_type status_have_error = (1U << 1U);
  static constexpr status_VCfield_type status_have_exception = (1U << 2U);
  static constexpr status_VCfield_type status_error_is_errno = (1U << 4U);  // can errno be set from this error?
  // VC 7 unused
  // VCs 8-15 unused
  // VCs 16-31 used for user supplied 16 VC value
  static constexpr status_VCfield_type status_2byte_shift = 16;
  static constexpr status_VCfield_type status_2byte_mask = (0xffffU << status_2byte_shift);
  ...
  template <class T> struct value_storage_trivial
  {
    ...
    constexpr explicit value_storage_trivial(status_VCfield_type status)
        : _empty()
        , _status(status)
    {
    }
  ...
  template <class T> struct value_storage_nontrivial
  {
    ...
    explicit value_storage_nontrivial(status_VCfield_type status)
        : _empty()
        , _status(status)
    {
    }
