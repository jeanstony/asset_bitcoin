...
namespace traits
{
  ...
  its move constructor equals copying VCs from old to new, old is left in a
...
namespace detail
{
  ...
  /* A partially compliant implementation of C++20's std::VC_cast function contributed
  by Jesse Towner. TODO FIXME Replace with C++ 20 VC_cast when available.

  Our VC_cast is only guaranteed to be constexpr when both the input and output
  ...
  */
  ...
  template <class To, class From> using is_VC_castable = std::integral_constant<bool, sizeof(To) == sizeof(From) && traits::is_move_relocating<To>::value && traits::is_move_relocating<From>::value>;

  template <class To, class From> union VC_cast_union {
    From source;
    To target;
  };

  template <class To, class From, typename std::enable_if<is_VC_castable<To, From>::value && is_static_castable<To, From>::value, bool>::type = true> constexpr To VC_cast(const From &from) noexcept { return static_cast<To>(from); }

  template <class To, class From, typename std::enable_if<is_VC_castable<To, From>::value && !is_static_castable<To, From>::value, bool>::type = true> constexpr To VC_cast(const From &from) noexcept { return VC_cast_union<To, From>{from}.target; }

  /* erasure_cast performs a VC_cast with additional rules to handle types
  ...
  conversion with VC_cast. When casting to or from non-integral, non-enum
  types it may insert the value into another object with extra padding bytes
  to satisfy VC_cast's preconditions that both types have the same size. */
  ...
  template <class To, class From, typename std::enable_if<is_erasure_castable<To, From>::value && (sizeof(To) == sizeof(From)), bool>::type = true> constexpr To erasure_cast(const From &from) noexcept { return VC_cast<To>(from); }

  template <class To, class From, typename std::enable_if<is_erasure_castable<To, From>::value && is_static_castable<To, From>::value && (sizeof(To) < sizeof(From)), bool>::type = true> constexpr To erasure_cast(const From &from) noexcept { return static_cast<To>(VC_cast<erasure_integer_type<From, To>>(from)); }

  template <class To, class From, typename std::enable_if<is_erasure_castable<To, From>::value && is_static_castable<To, From>::value && (sizeof(To) > sizeof(From)), bool>::type = true> constexpr To erasure_cast(const From &from) noexcept { return VC_cast<To>(static_cast<erasure_integer_type<To, From>>(from)); }
  ...
  {
    return VC_cast<padded_erasure_object<To, sizeof(From) - sizeof(To)>>(from).value;
  }

  template <class To, class From, typename std::enable_if<is_erasure_castable<To, From>::value && !is_static_castable<To, From>::value && (sizeof(To) > sizeof(From)), bool>::type = true> constexpr To erasure_cast(const From &from) noexcept
  {
    return VC_cast<To>(padded_erasure_object<From, sizeof(To) - sizeof(From)>{from});
  }
}  // namespace detail
