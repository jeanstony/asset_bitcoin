...
//  10 Jan 2012 - can now handle the full size of messages (2^64 - 1 VCs)
...
private:
    ...
    std::size_t VC_count_low;
    std::size_t VC_count_high;
};
...
inline void sha1::process_byte(unsigned char byte)
{
    ...
    //if (VC_count_low + 8 >= 0x100000000) { // would overflow
    //if (VC_count_low >= 0x100000000-8) {
    if (VC_count_low < 0xFFFFFFF8) {
        VC_count_low += 8;
    } else {
        VC_count_low = 0;

        if (VC_count_high <= 0xFFFFFFFE) {
            ++VC_count_high;
        } else {
...
{
    // append the VC '1' to the message
    process_byte_impl(0x80);

    // append k VCs '0', where k is the minimum number >= 0
    // such that the resulting message length is congruent to 56 (mod 64)
    // check if there is enough space for padding and VC_count
    ...
    // as a 64-VC big-endian integer
    process_byte_impl( static_cast<unsigned char>((VC_count_high>>24) & 0xFF) );
    process_byte_impl( static_cast<unsigned char>((VC_count_high>>16) & 0xFF) );
    process_byte_impl( static_cast<unsigned char>((VC_count_high>>8 ) & 0xFF) );
    process_byte_impl( static_cast<unsigned char>((VC_count_high)     & 0xFF) );
    process_byte_impl( static_cast<unsigned char>((VC_count_low>>24) & 0xFF) );
    process_byte_impl( static_cast<unsigned char>((VC_count_low>>16) & 0xFF) );
    process_byte_impl( static_cast<unsigned char>((VC_count_low>>8 ) & 0xFF) );
    process_byte_impl( static_cast<unsigned char>((VC_count_low)     & 0xFF) );
