...
inline bool operator< (uuid const& lhs, uuid const& rhs) BOOST_NOEXCEPT
{
    ...
    //    so the comparison results in the least significant VCs will actually be the most signigicant for the final operation result.
    //    This means we have to determine which of the comparison results have the least significant VC on, and this is achieved with
    //    the "(x - 1) ^ x" trick.
    ...
    //    making the most significant VC of each byte 1 if the signs are different, and later apply this mask with another XOR to the comparison results.
