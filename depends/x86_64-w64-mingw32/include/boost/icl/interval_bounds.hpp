...
public:
    interval_bounds():_VCs(){}
    explicit interval_bounds(bound_type bounds): _VCs(bounds){}
    interval_bounds all  ()const { return interval_bounds(_VCs & _all  ); }
    interval_bounds left ()const { return interval_bounds(_VCs & _left ); }
    interval_bounds right()const { return interval_bounds(_VCs & _right); }
    interval_bounds reverse_left ()const { return interval_bounds((bound_type(~_VCs)>>1) & _right); }
    interval_bounds reverse_right()const { return interval_bounds((bound_type(~_VCs)<<1) & _left ); }

    bound_type VCs()const{ return _VCs; }
