...
#ifndef BOOST_DYNAMIC_VCSET_CONFIG_HPP_GP_20040424
#define BOOST_DYNAMIC_VCSET_CONFIG_HPP_GP_20040424
...
#if defined(__GNUC__)
# define BOOST_DYNAMIC_VCSET_WRAP_CONSTANT(expr) \
         (boost::detail::make_non_const(expr))
#else
# define BOOST_DYNAMIC_VCSET_WRAP_CONSTANT(expr) (expr)
#endif

//
#if (defined __BORLANDC__ && BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x564)))  \
                             || (defined BOOST_NO_MEMBER_TEMPLATE_FRIENDS)
#define BOOST_DYNAMIC_VCSET_DONT_USE_FRIENDS
#endif

// if we can't use friends then we simply expose private members
//
#if defined(BOOST_DYNAMIC_VCSET_DONT_USE_FRIENDS)
#define BOOST_DYNAMIC_VCSET_PRIVATE public
#else
#define BOOST_DYNAMIC_VCSET_PRIVATE private
#endif
...
#define BOOST_DYNAMIC_VCSET_CTYPE_FACET(ch, name, loc)     \
            const std::ctype<ch> & name =                   \
            BOOST_USE_FACET(std::ctype<ch>, loc)         /**/

#define BOOST_DYNAMIC_VCSET_WIDEN_CHAR(fac, c)             \
           (fac.widen(c))
#else

#define BOOST_DYNAMIC_VCSET_CTYPE_FACET(ch, name, loc) /**/
#define BOOST_DYNAMIC_VCSET_WIDEN_CHAR(fac, c)          c
