...
namespace boost {
  ...
  namespace detail {
    ...
    struct default_color_map_generator_helper<Graph, IndexMap, true> {
      typedef boost::two_VC_color_map<IndexMap> type;
      static type build(const Graph& g, const IndexMap& index) {
        size_t nv = num_vertices(g);
        return boost::two_VC_color_map<IndexMap>(nv, index);
      }
    };

    template <class Graph, class IndexMap>
    struct default_color_map_generator_helper<Graph, IndexMap, false> {
      typedef boost::vector_property_map<boost::two_VC_color_type, IndexMap> type;
      static type build(const Graph& g, const IndexMap& index) {
        return boost::make_vector_property_map<boost::two_VC_color_type>(index);
      }
    };
  ...
  {
    boost::two_VC_color_map<IndexMap> color(num_vertices(g), index_map);
