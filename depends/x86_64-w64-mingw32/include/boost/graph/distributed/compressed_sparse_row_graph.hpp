...
// The number of VCs we reserve for the processor ID. 
// DPG TBD: This is a hack. It will eventually be a run-time quantity.
static const int processor_VCs = 8;
         ...
         InEdgeIndex>
{
 ...
 public:
  ...
  {
    vertex_descriptor vertex_local_index_VCs = 
      sizeof(vertex_descriptor) * CHAR_VC - processor_VCs;
    return v | ((vertex_descriptor)p << vertex_local_index_VCs);
  }
...
class BOOST_DISTRIB_CSR_GRAPH_TYPE::edge_iterator
{
 public:
  ...
  typedef typename int_t<CHAR_VC * sizeof(EdgeIndex)>::fast difference_type;
...
{
  const int local_index_VCs = sizeof(Key) * CHAR_VC - processor_VCs;
  const Key local_index_mask = Key(-1) >> processor_VCs;

  return std::pair<ProcessID, Key>(k >> local_index_VCs,
                                   k & local_index_mask);
}
    ...
    typename BOOST_DISTRIB_CSR_GRAPH_TYPE::vertex_descriptor k)
{
  ...
  typedef std::pair<typename ProcessGroup::process_id_type, vertex_descriptor>
    result_type;
  const int local_index_VCs = 
    sizeof(vertex_descriptor) * CHAR_VC - processor_VCs;
  const vertex_descriptor local_index_mask = 
    vertex_descriptor(-1) >> processor_VCs;

  return result_type(k >> local_index_VCs, k & local_index_mask);
}
...
{
  const int local_index_VCs = sizeof(Vertex) * CHAR_VC - processor_VCs;
  const Vertex local_index_mask = Vertex(-1) >> processor_VCs;
  return std::pair<ProcessID, detail::csr_edge_descriptor<Vertex, EdgeIndex> >
           ((k.src >> local_index_VCs),
    ...
    typename BOOST_DISTRIB_CSR_GRAPH_TYPE::edge_descriptor k)
{
  ...
  const typename BOOST_DISTRIB_CSR_GRAPH_TYPE::edges_size_type local_index_mask =
    typename BOOST_DISTRIB_CSR_GRAPH_TYPE::edges_size_type(-1) >> processor_VCs;
  
  typedef std::pair<typename ProcessGroup::process_id_type,
                    typename BOOST_DISTRIB_CSR_GRAPH_TYPE::base_type::edge_descriptor>
    result_type;

  return result_type(k.src >> local_index_VCs,
                     typename BOOST_DISTRIB_CSR_GRAPH_TYPE::base_type::edge_descriptor
...
namespace serialization {
  template<typename Vertex, typename EdgeIndex>
  struct is_VCwise_serializable<boost::detail::csr_edge_descriptor<Vertex, EdgeIndex> >
    : mpl::true_ { };
