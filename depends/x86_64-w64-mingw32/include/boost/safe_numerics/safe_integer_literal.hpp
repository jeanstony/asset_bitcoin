...
public:
    ...
    where n is the number of VCs in the promoted operand. The type of the
    ...
    */
    ...
    constexpr safe_literal_impl<T, checked::VCwise_not(N), P, E> operator~() const { // invert VCs
        return safe_literal_impl<T, checked::VCwise_not(N), P, E>();
    }
    */
    template<
        typename Tx, Tx Nx, typename = std::enable_if_t<! checked::VCwise_not(Nx).exception()>
    >
