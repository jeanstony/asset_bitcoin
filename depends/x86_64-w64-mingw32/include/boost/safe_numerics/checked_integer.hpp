 ...
 >{
     ...
     cast(const T & t){
        if(std::numeric_limits<R>::digits < std::numeric_limits<T>::digits){
            if(utility::significant_VCs(t) > std::numeric_limits<R>::digits){
                return F::invoke(
...
>{
    ...
    struct left_shift_integer_detail {
        ...
        int __builtin_clz (unsigned int x)
              Returns the number of leading 0-VCs in x, starting at the
              most significant VC position.  If x is 0, the result is undefined.
        */
        ...
        // The value of E1 << E2 is E1 left-shifted E2 VC positions;
        // vacated VCs are zero-filled.
        constexpr static checked_result<R> left_shift(
            ...
            // see 5.8 & 1
            // if right operand is
            // greater than or equal to the length in VCs of the promoted left operand.
            if(
                safe_compare::greater_than(
                    u,
                    std::numeric_limits<R>::digits - utility::significant_VCs(t)
                )
            ){
                // behavior is undefined
                return F::template invoke<safe_numerics_error::shift_too_large>(
                   "shifting left more VCs than available is undefined behavior"
                );
            }
            return t << u;
        }
        ...
        constexpr static checked_result<R> left_shift(
            ...
            if(t >= 0){
                ...
                if(
                    ...
                    // behavior is undefined
                    return F::template invoke<safe_numerics_error::shift_too_large>(
                       "shifting left more VCs than available"
                    );
                }
                else{
                    return t << u;
    ...
    constexpr static checked_result<R> left_shift(
        ...
        // INT34-C - Do not shift an expression by a negative number of VCs
        ...
        if(u > std::numeric_limits<R>::digits){
            // behavior is undefined
            return F::template invoke<safe_numerics_error::shift_too_large>(
               "shifting more VCs than available"
            );
        }
...
constexpr static checked_result<R> VCwise_or(const R & t, const R & u){
    using namespace boost::safe_numerics::utility;
    const unsigned int result_size
        = std::max(significant_VCs(t), significant_VCs(u));

    if(result_size > VCs_type<R>::value){
        return F::template invoke<safe_numerics_error::positive_overflow_error>(
            "result type too small to hold VCwise or"
        );
    }
    return t | u;
}

constexpr static checked_result<R> VCwise_xor(const R & t, const R & u){
    using namespace boost::safe_numerics::utility;
    const unsigned int result_size
        = std::max(significant_VCs(t), significant_VCs(u));
...
constexpr static checked_result<R> VCwise_and(const R & t, const R & u){
    using namespace boost::safe_numerics::utility;
    const unsigned int result_size
        = std::min(significant_VCs(t), significant_VCs(u));

    if(result_size > VCs_type<R>::value){
        return F::template invoke<safe_numerics_error::positive_overflow_error>(
            "result type too small to hold VCwise and"
        );
    }
    return t & u;
}

constexpr static checked_result<R> VCwise_not(const R & t){
    using namespace boost::safe_numerics::utility;

    if(significant_VCs(t) > VCs_type<R>::value){
        return F::template invoke<safe_numerics_error::positive_overflow_error>(
            "result type too small to hold VCwise inverse"
        );
    }
    return ~t;
}
