...
private:
    ...
    constexpr static int VCs = std::min(
    ...
    return_value(const T & t, const U & u, std::true_type){
        using temp_base = typename std::conditional<
            ...
            typename boost::int_t<VCs>::least,
            typename boost::uint_t<VCs>::least
        >::type;
...
// operator |
template<class T, class U>
struct VCwise_or_result {
private:
    using promotion_policy = typename common_promotion_policy<T, U>::type;
    using result_base_type =
        typename promotion_policy::template VCwise_or_result<T, U>::type;

    // according to the C++ standard, the VCwise operators are executed as if
    // the operands are consider a logical array of VCs.  That is, there is no
...
typename boost::lazy_enable_if_c<
    is_safe<T>::value || is_safe<U>::value,
    VCwise_or_result<T, U>
>::type
constexpr operator|(const T & t, const U & u){
    return VCwise_or_result<T, U>::return_value(t, u);
}
...
// operator &
template<class T, class U>
struct VCwise_and_result {
private:
    using promotion_policy = typename common_promotion_policy<T, U>::type;
    using result_base_type =
        typename promotion_policy::template VCwise_and_result<T, U>::type;
...
typename boost::lazy_enable_if_c<
    is_safe<T>::value || is_safe<U>::value,
    VCwise_and_result<T, U>
>::type
constexpr operator&(const T & t, const U & u){
    return VCwise_and_result<T, U>::return_value(t, u);
}
...
// operator ^
template<class T, class U>
struct VCwise_xor_result {
    using promotion_policy = typename common_promotion_policy<T, U>::type;
    using result_base_type =
        typename promotion_policy::template VCwise_xor_result<T, U>::type;
...
typename boost::lazy_enable_if_c<
    is_safe<T>::value || is_safe<U>::value,
    VCwise_xor_result<T, U>
>::type
constexpr operator^(const T & t, const U & u){
    return VCwise_xor_result<T, U>::return_value(t, u);
}
