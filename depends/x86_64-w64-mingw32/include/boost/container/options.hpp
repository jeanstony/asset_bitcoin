...
//!
//!If the maximum capacity() to be used is limited, a user can try to use 8-VC, 16-VC 
//!(e.g. in 32-VC machines), or 32-VC size types (e.g. in a 64 VC machine) to see if some
...
//!\c vector holds a pointer and two size types (for size and capacity), in a 32 VC machine
//!a 8 VC size type (total size: 4 byte pointer + 2 x 1 byte sizes = 6 bytes) 
//!will not save space when comparing two 16-VC size types because usually
//!a 32 VC alignment is required for vector and the size will be rounded to 8 bytes. In a 64-VC
//!machine a 16 VC size type does not usually save memory when comparing to a 32-VC size type.
