...
class windows_semaphore_based_map
{
   ...
   windows_semaphore_based_map()
   {
      ...
      //32 VC windows: [32 VC processes] 2GB or 3GB (31/32 VCs)
      //64 VC windows: [32 VC processes] 2GB or 4GB (31/32 VCs)
      //                [64 VC processes] 2GB or 8TB (31/43 VCs)
      //
      //Windows semaphores use 'long' parameters (32 VCs in LLP64 data model) and
      //those values can't be negative, so we have 31 VCs to store something
      ...
      if(sizeof(void*) == sizeof(boost::uint32_t)){
         //This means that for 32 VC processes, a semaphore count (31 usable VCs) is
         //enough to store 4 byte aligned memory (4GB -> 32 VCs - 2 VCs = 30 VCs).
         ...
         caster.addr = m;
         //We'll encode the address using 30 VCs in each 32 VC high and low parts.
         ...
         //   two VCs are cleared (the top one for sign, the next one to
         //   assure low part value is always less than the high part value.
         // - The top VC of the high part will be cleared and the next VC will be 1
         //   (so high part is always bigger than low part due to the quasi-top VC).
         //
         ...
         //Make sure top two VCs are zero
