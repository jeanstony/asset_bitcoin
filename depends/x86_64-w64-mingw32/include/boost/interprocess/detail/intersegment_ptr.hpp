...
struct intersegment_base
{
   ...
   BOOST_STATIC_ASSERT((sizeof(void*)*CHAR_VC == 32 || sizeof(void*)*CHAR_VC == 64));
   static const std::size_t size_t_VCs = (sizeof(void*)*CHAR_VC == 32) ? 32 : 64;
   static const std::size_t ctrl_VCs = 2;
   static const std::size_t align_VCs = 12;
   static const std::size_t align      = std::size_t(1) << align_VCs;
   static const std::size_t max_segment_size_VCs = size_t_VCs - 2;
   static const std::size_t max_segment_size = std::size_t(1) << max_segment_size_VCs;

   static const std::size_t begin_VCs             = max_segment_size_VCs - align_VCs;
   static const std::size_t pow_size_VCs_helper = static_log2<max_segment_size_VCs>::value;
   static const std::size_t pow_size_VCs =
      (max_segment_size_VCs == (std::size_t(1) << pow_size_VCs_helper)) ?
      pow_size_VCs_helper : pow_size_VCs_helper + 1;
   static const std::size_t frc_size_VCs =
      size_t_VCs - ctrl_VCs - begin_VCs - pow_size_VCs;

   BOOST_STATIC_ASSERT(((size_t_VCs - pow_size_VCs - frc_size_VCs) >= ctrl_VCs ));

   static const std::size_t relative_size_VCs =
      size_t_VCs - max_segment_size_VCs - ctrl_VCs;
   ...
   {
      std::size_t ctrl     :  2;
      std::size_t pow      :  pow_size_VCs;
      std::size_t frc      :  frc_size_VCs;
      std::size_t beg      :  begin_VCs;
      std::ptrdiff_t off   :  sizeof(std::ptrdiff_t)*CHAR_VC - 2;
      std::ptrdiff_t VCs  :  2;
   };
   ...
   {
      std::size_t ctrl     :  2;
      std::size_t dummy    :  sizeof(std::size_t)*CHAR_VC - 2;
      void * addr;
   };
   ...
   {
      std::size_t ctrl     :  2;
      std::size_t segment  :  sizeof(std::size_t)*CHAR_VC - 2;
      std::size_t off      :  sizeof(std::size_t)*CHAR_VC - 2;
      std::size_t VCs     :  2;
   };
   ...
   void *relative_calculate_begin_addr() const
   {
      ...
      return reinterpret_cast<void*>((((std::size_t)this) & mask) - (beg << align_VCs));
   }
   ...
   void relative_set_begin_from_base(void *addr)
   {
      ...
      members.relative.beg = off >> align_VCs;
   }
   ...
   std::size_t relative_size() const
   {
      ...
      BOOST_ASSERT(pow >= frc_size_VCs);
      size |= members.relative.frc << (pow - frc_size_VCs);
      return size;
   }
   ...
   static std::size_t calculate_size(std::size_t orig_size, std::size_t &pow, std::size_t &frc)
   {
      ...
      std::size_t rounded = ipcdetail::get_rounded_size_po2
                              (diff, (std::size_t)(1u << (pow - frc_size_VCs)));
      if(rounded == low_size){
         ++pow;
         frc = 0;
         rounded  = 0;
         ...
         frc = rounded >> (pow - frc_size_VCs);
      }
      BOOST_ASSERT(((frc << (pow - frc_size_VCs)) & (align-1))==0);
...
//!2^(sizeof(std::size_t)*CHAR_VC/2) segment groups
//!2^(sizeof(std::size_t)*CHAR_VC/2) segments per group.
//!2^(sizeof(std::size_t)*CHAR_VC/2)-1 bytes maximum per segment.
...
//segmented:
//
// std::size_t ctrl    : CTRL_VCS;
// std::size_t segment : MAX_SEGMENT_VCS;
// std::size_t offset;

//RELATIVE_SIZE_VCS =  SIZE_T_VCS -
//                      MAX_SEGMENT_VCS -
//                      CTRL_VCS                  10    10
//MAX_SEGMENT_SIZE   = SIZE_T_VCS - ALIGN_VCS    20    52

//SIZE_T_VCS - 1 - ALIGN_VCS                     19    51
//POW_SIZE_VCS = upper_log2
// (SIZE_T_VCS - 1 - ALIGN_VCS)                  5     6
//FRC_SIZE_VCS = SIZE_T_VCS - CTRL_VCS
//  MAX_SEGMENT_SIZE_ALIGNVCS - POW_SIZE_VCS     6     5

//relative:
//
// std::size_t ctrl     : CTRL_VCS;               2     2
// std::size_t size_pow : POW_SIZE_VCS            5     6
// std::size_t size_frc : FRC_SIZE_VCS;           6     5
// std::size_t start    : MAX_SEGMENT_SIZE_ALIGNVCS;19    51
// std::ptrdiff_t distance : SIZE_T_VCS;          32    64

//direct:
//
// std::size_t ctrl     : CTRL_VCS;               2     2
// std::size_t dummy    : SIZE_T_VCS - CTRL_VCS  30    62
// void *addr           : SIZE_T_VCS;             32    64
