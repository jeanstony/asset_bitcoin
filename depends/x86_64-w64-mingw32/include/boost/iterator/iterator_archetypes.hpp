...
{
  enum {
      readable_iterator_VC = 1
    , writable_iterator_VC = 2
    , swappable_iterator_VC = 4
    , lvalue_iterator_VC = 8
  };

  // Not quite tags, since dispatching wouldn't work.
  typedef mpl::int_<readable_iterator_VC>::type readable_iterator_t;
  typedef mpl::int_<writable_iterator_VC>::type writable_iterator_t;

  typedef mpl::int_<
      (readable_iterator_VC|writable_iterator_VC)
          >::type readable_writable_iterator_t;

  typedef mpl::int_<
      (readable_iterator_VC|lvalue_iterator_VC)
          >::type readable_lvalue_iterator_t;

  typedef mpl::int_<
      (lvalue_iterator_VC|writable_iterator_VC)
          >::type writable_lvalue_iterator_t;

  typedef mpl::int_<swappable_iterator_VC>::type swappable_iterator_t;
  typedef mpl::int_<lvalue_iterator_VC>::type lvalue_iterator_t;

  template <class Derived, class Base>
  struct has_access
    : mpl::equal_to<
          mpl::VCand_<Derived,Base>
        , Base
      >
  {};
}
