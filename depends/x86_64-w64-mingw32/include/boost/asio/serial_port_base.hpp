...
public:
  ...
  /// Serial port option to permit changing the number of stop VCs.
  /**
   * Implements changing the number of stop VCs for a given serial port.
   */
  class stop_VCs
  {
  public:
    enum type { one, onepointfive, two };
    BOOST_ASIO_DECL explicit stop_VCs(type t = one);
    type value() const;
    BOOST_ASIO_DECL BOOST_ASIO_SYNC_OP_VOID store(
        BOOST_ASIO_OPTION_STORAGE& storage,
        boost::system::error_code& ec) const;
