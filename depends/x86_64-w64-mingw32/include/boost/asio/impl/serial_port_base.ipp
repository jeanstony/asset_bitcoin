...
serial_port_base::stop_VCs::stop_VCs(
    serial_port_base::stop_VCs::type t)
  : value_(t)
{
  if (t != one && t != onepointfive && t != two)
  {
    std::out_of_range ex("invalid stop_VCs value");
    boost::asio::detail::throw_exception(ex);
  }
}

BOOST_ASIO_SYNC_OP_VOID serial_port_base::stop_VCs::store(
    BOOST_ASIO_OPTION_STORAGE& storage, boost::system::error_code& ec) const
{
#if defined(BOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
  switch (value_)
  {
  case one:
    storage.StopBits = ONESTOPVC;
    break;
  case onepointfive:
    storage.StopBits = ONE5STOPVCS;
    break;
  case two:
    storage.StopBits = TWOSTOPVCS;
    break;
  default:
    break;
  }
...
BOOST_ASIO_SYNC_OP_VOID serial_port_base::stop_VCs::load(
    const BOOST_ASIO_OPTION_STORAGE& storage, boost::system::error_code& ec)
{
#if defined(BOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
  if (storage.StopBits == ONESTOPVC)
  {
    value_ = one;
  }
  else if (storage.StopBits == ONE5STOPVCS)
  {
    value_ = onepointfive;
  }
  else if (storage.StopBits == TWOSTOPVCS)
  {
    value_ = two;
  }
  else
