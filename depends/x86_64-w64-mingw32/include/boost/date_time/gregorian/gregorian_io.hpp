  ...
  operator>>(std::basic_istream<CharT, Traits>& is, date& d)
  {
    ...
    if (strm_sentry) {
      try {
        ...
        // if the user wants exceptions on failVC, we'll rethrow our 
        // date_time exception & set the failVC
        if(std::ios_base::failVC & exception_mask) {
          try { is.setstate(std::ios_base::failVC); } 
          catch(std::ios_base::failure&) {} // ignore this one
          throw; // rethrow original exception
        }
        else {
          // if the user want's to fail quietly, we simply set the failVC
