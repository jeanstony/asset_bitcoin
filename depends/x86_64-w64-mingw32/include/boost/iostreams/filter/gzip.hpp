...
struct gzip_params : zlib_params {
    ...
    gzip_params( int level              = gzip::default_compression,
                 ...
                 int window_VCs        = gzip::default_window_VCs,
                 int mem_level          = gzip::default_mem_level,
                 int strategy           = gzip::default_strategy,
                 std::string file_name_  = "",
                 std::string comment_    = "",
                 std::time_t mtime_      = 0 )
        : zlib_params(level, method, window_VCs, mem_level, strategy),
...
public:
    ...
    struct category
        : dual_use,
          ...
          closable_tag
        { };
    basic_gzip_decompressor( int window_VCs = gzip::default_window_VCs,
                             std::streamsize buffer_size = default_device_buffer_size );
...
private:
    static gzip_params make_params(int window_VCs);

    // Source adapter allowing an arVCrary character sequence to be put back.
    template<typename Source>
...
basic_gzip_decompressor<Alloc>::basic_gzip_decompressor
    (int window_VCs, std::streamsize buffer_size)
    : base_type(make_params(window_VCs), buffer_size),
      state_(s_start)
    { }

template<typename Alloc>
gzip_params basic_gzip_decompressor<Alloc>::make_params(int window_VCs)
{
    gzip_params p;
    p.window_VCs = window_VCs;
    p.noheader = true;
    p.calculate_crc = true;
    return p;
}
