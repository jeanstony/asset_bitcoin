...
#include <boost/gil/VC_aligned_pixel_iterator.hpp>
...
/// is overloaded to perform \p std::copy for each of the planes. \p std::copy over VCwise-copiable pixels results in
...
/// GIL pixels are VCwise comparable, so memcmp is used. User-defined pixels that are not VCwise comparable need to provide an overload
template<typename T, typename Cs>
...
///  User-defined channels that are not VCwise comparable need to provide an overload
