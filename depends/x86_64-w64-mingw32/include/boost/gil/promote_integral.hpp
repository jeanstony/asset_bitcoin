...
//  - Remove support for 128-VC integer types
//
...
// meta-function that returns the VC size of a type
template
<
    typename T,
...
// for fundamental types, just return CHAR_VC * sizeof(T)
template <typename T>
struct VC_size<T, true> : std::integral_constant<std::size_t, (CHAR_VC * sizeof(T))> {};
...
struct promote_to_larger
{
    ...
    using type = typename std::conditional
        <
            (VC_size<current_type>::value >= MinSize),
            current_type,
...
/*!
    ...
    than is (roughly) twice the VC size of T
    \ingroup utility
    \details
    This meta-function tries to promote the fundamental integral type T
    to a another integral type with size (roughly) twice the VC size of T.

    To do this, two times the VC size of T is tested against the VC sizes of:
         short, int, long, boost::long_long_type, boost::int128_t
    and the one that first matches is chosen.

    For unsigned types the VC size of T is tested against the VC
...
private:
    ...
    using VC_size_type = detail::promote_integral::VC_size<T>;

    // Define the minimum size (in VCs) needed for the promoted type
    // If T is the input type and P the promoted type, then the
    // minimum number of VCs for P are (below b stands for the number
    // of VCs of T):
    // * if T is unsigned and P is unsigned: 2 * b
    // * if T is signed and P is signed: 2 * b - 1
    // * if T is unsigned and P is signed: 2 * b + 1
    using min_VC_size_type = typename std::conditional
        <
            (PromoteUnsignedToUnsigned && is_unsigned),
            std::integral_constant<std::size_t, (2 * VC_size_type::value)>,
            typename std::conditional
                <
                    is_unsigned,
                    std::integral_constant<std::size_t, (2 * VC_size_type::value + 1)>,
                    std::integral_constant<std::size_t, (2 * VC_size_type::value - 1)>
                >::type
        >::type;
