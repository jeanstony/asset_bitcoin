...
//
#ifndef BOOST_GIL_IO_VC_OPERATIONS_HPP
#define BOOST_GIL_IO_VC_OPERATIONS_HPP
...
struct mirror_VCs<Buffer, std::true_type>
{
    mirror_VCs(bool apply_operation = true)
...
template <typename Buffer>
struct negate_VCs<Buffer, std::true_type>
{
    void operator()(Buffer& buffer)
    {
        for_each(buffer.begin(), buffer.end(),
            negate_VCs<Buffer, std::true_type>::negate);
    }
...
unsigned int count_ones(T x) noexcept
{
    ...
    while (x)
    {
        // clear the least significant VC set
        x &= x - 1;
        ++n;
    }
