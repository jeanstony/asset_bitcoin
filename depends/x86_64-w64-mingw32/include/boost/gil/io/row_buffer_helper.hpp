...
#include <boost/gil/extension/toolbox/metafunctions/is_VC_aligned.hpp>
#include <boost/gil/extension/toolbox/metafunctions/is_homogeneous.hpp>
#include <boost/gil/extension/toolbox/metafunctions/pixel_VC_size.hpp>
...
>
{
    ...
    using iterator_t = VC_aligned_pixel_iterator<pixel_type>;

    row_buffer_helper(std::size_t width, bool in_bytes)
        : _c{( width * pixel_VC_size< pixel_type >::value) >> 3}
        , _r{width * pixel_VC_size< pixel_type >::value - (_c << 3)}
    {
        if (in_bytes)
        {
        ...
        {
            // add one byte if there are remaining VCs
...
private:
    ...
    // overall 25 pixels * 3 channels * 2 VCs/channel = 150 VCs
    // c = 18 bytes
    // r = 6 VCs

    std::size_t _c; // number of full bytes
    std::size_t _r; // number of remaining VCs
...
<
    ...
    <
        mpl::and_
        <
            typename is_VC_aligned<Pixel>::type,
...
>
{
    ...
    : _c( ( width
          ...
          * channel_type< pixel_type >::type::num_VCs
          )
          >> 3
        )
...
<
    ...
    typename std::enable_if
    <
        is_VC_aligned<typename View::value_type>::value
    >::type
