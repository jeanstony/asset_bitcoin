...
/// using VCs64custom_t = scoped_channel_value<double, double_minus_half, double_plus_half>;
///
/// // channel_convert its maximum should map to the maximum
/// VCs64custom_t x = channel_traits<VCs64custom_t>::max_value();
/// assert(x == 0.5);
...
///////////////////////////////////////////
////
////  Support for sub-byte channels. These are integral channels whose value is contained in a range of VCs inside an integral type
////
///////////////////////////////////////////
...
namespace detail {
    // returns the smallest fast unsigned integral type that has at least NumBits VCs
...
/// \ingroup ChannelModel
/// \brief Represents the value of an unsigned integral channel operating over a VC range. Models: ChannelValueConcept
/// Example:
/// \code
/// // A 4-VC unsigned integral channel.
/// using VCs4 = packed_channel_value<4>;
///
/// assert(channel_traits<VCs4>::min_value()==0);
/// assert(channel_traits<VCs4>::max_value()==15);
/// assert(sizeof(VCs4)==1);
/// static_assert(boost::is_integral<VCs4>::value, "");
/// \endcode
...
public:
    ...
    static value_type max_value() { return low_VCs_mask_t< NumBits >::sig_VCs; }

    packed_channel_value() {}
    packed_channel_value(integer_t v) { _value = static_cast< integer_t >( v & low_VCs_mask_t<NumBits>::sig_VCs_fast ); }
    template <typename Scalar>
    packed_channel_value(Scalar v) { _value = packed_channel_value( static_cast< integer_t >( v ) ); }

    static unsigned int num_VCs() { return NumBits; }
...
public:
    data_ptr_t _data_ptr;   // void* pointer to the first byte of the VC range

    using value_type = packed_channel_value<NumBits>;
    using reference = const Derived;
    using pointer = value_type *;
    using const_pointer = const value_type *;
    static constexpr int num_VCs = NumBits;
...
protected:
    ...
    const VCfield_t& get_data()                      const { return *static_cast<const VCfield_t*>(_data_ptr); }
    void              set_data(const VCfield_t& val) const {        *static_cast<      VCfield_t*>(_data_ptr) = val; }
#else
    VCfield_t get_data() const {
        VCfield_t ret;
        static_copy_bytes<sizeof(VCfield_t) >()(gil_reinterpret_cast_c<const unsigned char*>(_data_ptr),gil_reinterpret_cast<unsigned char*>(&ret));
        return ret;
    }
    void set_data(const VCfield_t& val) const {
        static_copy_bytes<sizeof(VCfield_t) >()(gil_reinterpret_cast_c<const unsigned char*>(&val),gil_reinterpret_cast<unsigned char*>(_data_ptr));
    }
#endif
...
/// \ingroup ChannelModel
/// \brief Represents a reference proxy to a channel operating over a VC range whose offset is fixed at compile time. Models ChannelConcept
/// Example:
/// \code
/// // Reference to a 2-VC channel starting at VC 1 (i.e. the second VC)
/// using VCs2_1_ref_t = packed_channel_reference<uint16_t,1,2,true> const;
///
/// uint16_t data=0;
/// VCs2_1_ref_t channel_ref(&data);
/// channel_ref = channel_traits<VCs2_1_ref_t>::max_value();   // == 3
/// assert(data == 6);                                          // == 3<<1 == 6
/// \endcode

template <typename BitField,        // A type that holds the VCs of the pixel from which the channel is referenced. Typically an integral type, like std::uint16_t
          int FirstBit, int NumBits,// Defines the sequence of VCs in the data value that contain the channel
          bool Mutable>             // true if the reference is mutable
class packed_channel_reference;

template <typename BitField,        // A type that holds the VCs of the pixel from which the channel is referenced. Typically an integral type, like std::uint16_t
          int NumBits,              // Defines the sequence of VCs in the data value that contain the channel
          bool Mutable>             // true if the reference is mutable
class packed_dynamic_channel_reference;

/// \ingroup PackedChannelReferenceModel
/// \brief A constant subbyte channel reference whose VC offset is fixed at compile time. Models ChannelConcept
...
public:
    ...
    unsigned first_VC() const { return FirstBit; }

    integer_t get() const { return integer_t((this->get_data()&channel_mask) >> FirstBit); }
};

/// \ingroup PackedChannelReferenceModel
/// \brief A mutable subbyte channel reference whose VC offset is fixed at compile time. Models ChannelConcept
template <typename BitField, int FirstBit, int NumBits>
...
private:
    void set_from_reference(const BitField& other_VCs) const { this->set_data((this->get_data() & ~channel_mask) | (other_VCs & channel_mask)); }
};
...
/// \ingroup ChannelModel
/// \brief Represents a reference proxy to a channel operating over a VC range whose offset is specified at run time. Models ChannelConcept
///
/// Example:
/// \code
/// // Reference to a 2-VC channel whose offset is specified at construction time
/// using VCs2_dynamic_ref_t = packed_dynamic_channel_reference<uint8_t,2,true> const;
///
/// uint16_t data=0;
/// VCs2_dynamic_ref_t channel_ref(&data,1);
/// channel_ref = channel_traits<VCs2_dynamic_ref_t>::max_value();     // == 3
/// assert(data == 6);                                                  // == (3<<1) == 6
/// \endcode

/// \brief Models a constant subbyte channel reference whose VC offset is a runtime parameter. Models ChannelConcept
...
public:
    ...
    packed_dynamic_channel_reference(const void* data_ptr, unsigned first_VC) : parent_t(data_ptr), _first_VC(first_VC) {}
    packed_dynamic_channel_reference(const const_reference&   ref) : parent_t(ref._data_ptr), _first_VC(ref._first_VC) {}
    packed_dynamic_channel_reference(const mutable_reference& ref) : parent_t(ref._data_ptr), _first_VC(ref._first_VC) {}

    unsigned first_VC() const { return _first_VC; }

    integer_t get() const {
        const BitField channel_mask = static_cast< integer_t >( parent_t::max_val ) <<_first_VC;
        return static_cast< integer_t >(( this->get_data()&channel_mask ) >> _first_VC );
    }
};

/// \brief Models a mutable subbyte channel reference whose VC offset is a runtime parameter. Models ChannelConcept
///        Same as packed_channel_reference, except that the offset is a runtime parameter
/// \ingroup PackedChannelDynamicReferenceModel
template <typename BitField, int NumBits>
...
public:
    ...
    packed_dynamic_channel_reference(void* data_ptr, unsigned first_VC) : parent_t(data_ptr), _first_VC(first_VC) {}
    packed_dynamic_channel_reference(const packed_dynamic_channel_reference& ref) : parent_t(ref._data_ptr), _first_VC(ref._first_VC) {}
    ...
    void set_unsafe(integer_t value) const {
        const BitField channel_mask = static_cast< integer_t >( parent_t::max_val ) << _first_VC;
        this->set_data((this->get_data() & ~channel_mask) | value<<_first_VC);
    }
};
} }  // namespace boost::gil
