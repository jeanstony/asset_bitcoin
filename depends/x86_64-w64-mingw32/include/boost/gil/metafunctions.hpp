...
template <typename BitField, typename ChannelBitSizes, typename Layout, bool IsMutable> struct VC_aligned_pixel_reference;
...
namespace detail {
    ...
    class packed_channel_references_vector_type {
        ...
        // Then first_VCs_vector will be mpl::vector<int,0,7,14,16>
        using first_VCs_vector = typename mpl::accumulate<ChannelBitSizesVector, mpl::vector1<mpl::int_<0> >,
            mpl::push_back<mpl::_1, mpl::plus<mpl::back<mpl::_1>, mpl::_2> > >::type;
    public:
        using type = typename mpl::transform<typename mpl::pop_back<first_VCs_vector>::type, ChannelBitSizesVector,
...
/// \ingroup TypeFactoryFromElements
/// \brief Returns the type of a packed pixel given its VCfield type, the VC size of its channels and its layout.
///
/// A packed pixel has channels that cover VC ranges but itself is byte aligned. RGB565 pixel is an example.
///
/// The size of ChannelBitSizeVector must equal the number of channels in the given layout
/// The sum of VC sizes for all channels must be less than or equal to the number of VCs in BitField (and cannot exceed 64).
///  If it is less than the number of VCs in BitField, the last VCs will be unused.
...
/// \defgroup TypeFactoryPacked packed_image_type,VC_aligned_image_type
/// \ingroup TypeFactoryFromElements
...
/// A VC-aligned image is an image whose pixels are not byte aligned, such as "rgb222". <br>
///
/// The sum of the VC sizes of all channels cannot exceed 64.
...
/// \ingroup TypeFactoryPacked
/// \brief Returns the type of a single-channel image given its VCfield type, the VC size of its channel and its layout
...
/// \ingroup TypeFactoryPacked
/// \brief Returns the type of a two channel image given its VCfield type, the VC size of its channels and its layout
...
/// \ingroup TypeFactoryPacked
/// \brief Returns the type of a three channel image given its VCfield type, the VC size of its channels and its layout
...
/// \ingroup TypeFactoryPacked
/// \brief Returns the type of a four channel image given its VCfield type, the VC size of its channels and its layout
...
/// \ingroup TypeFactoryPacked
/// \brief Returns the type of a five channel image given its VCfield type, the VC size of its channels and its layout
...
/// \ingroup TypeFactoryPacked
/// \brief Returns the type of a packed image whose pixels may not be byte aligned. For example, an "rgb222" image is VC-aligned because its pixel spans six VCs.
///
/// Note that the alignment parameter in the constructor of VC-aligned images is in VC units. For example, if you want to construct a VC-aligned
...
private:
    ...
    using VCfield_t = typename detail::min_fast_uint<VC_size + 7>::type;
    using VC_alignedref_t = VC_aligned_pixel_reference<VCfield_t, ChannelBitSizeVector, Layout, true> const;

public:
    using type = image<VC_alignedref_t,false,Alloc>;
};

/// \ingroup TypeFactoryPacked
/// \brief Returns the type of a single-channel VC-aligned image given the VC size of its channel and its layout
template <unsigned Size1, typename Layout, typename Alloc=std::allocator<unsigned char> >
struct VC_aligned_image1_type : public VC_aligned_image_type<mpl::vector1_c<unsigned, Size1>, Layout, Alloc> {};

/// \ingroup TypeFactoryPacked
/// \brief Returns the type of a two channel VC-aligned image given the VC size of its channels and its layout
template <unsigned Size1, unsigned Size2, typename Layout, typename Alloc=std::allocator<unsigned char> >
struct VC_aligned_image2_type : public VC_aligned_image_type<mpl::vector2_c<unsigned, Size1, Size2>, Layout, Alloc> {};

/// \ingroup TypeFactoryPacked
/// \brief Returns the type of a three channel VC-aligned image given the VC size of its channels and its layout
template <unsigned Size1, unsigned Size2, unsigned Size3, typename Layout, typename Alloc=std::allocator<unsigned char> >
struct VC_aligned_image3_type : public VC_aligned_image_type<mpl::vector3_c<unsigned, Size1, Size2, Size3>, Layout, Alloc> {};

/// \ingroup TypeFactoryPacked
/// \brief Returns the type of a four channel VC-aligned image given the VC size of its channels and its layout
...
struct VC_aligned_image4_type : public VC_aligned_image_type<mpl::vector4_c<unsigned, Size1, Size2, Size3, Size4>, Layout, Alloc> {};

/// \ingroup TypeFactoryPacked
/// \brief Returns the type of a five channel VC-aligned image given the VC size of its channels and its layout
...
struct VC_aligned_image5_type : public VC_aligned_image_type<mpl::vector5_c<unsigned, Size1, Size2, Size3, Size4, Size5>, Layout, Alloc> {};
