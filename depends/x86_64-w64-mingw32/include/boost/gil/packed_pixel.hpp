...
/// A model of a heterogeneous pixel whose channels are VC ranges.
/// For example 16-VC RGB in '565' format.

/// \defgroup ColorBaseModelPackedPixel packed_pixel
/// \ingroup ColorBaseModel
/// \brief A heterogeneous color base whose elements are reference proxies to channels in a pixel. Models ColorBaseValueConcept. This class is used to model packed pixels, such as 16-VC packed RGB.
...
/// \brief Heterogeneous pixel value whose channel references can be constructed from the pixel VCfield and their index. Models ColorBaseValueConcept, PixelValueConcept, PixelBasedConcept
/// Typical use for this is a model of a packed pixel (like 565 RGB)
template <typename BitField,      // A type that holds the VCs of the pixel. Typically an integral type, like std::uint16_t
...
struct packed_pixel
{
    ...
    packed_pixel(){}
    explicit packed_pixel(const BitField& VCfield) : _VCfield(VCfield) {}

    // Construct from another compatible pixel type
    packed_pixel(const packed_pixel& p) : _VCfield(p._VCfield) {}
    ...
    packed_pixel(int chan0, int chan1, int chan2, int chan3, int chan4) : _VCfield(0) {
    ...
    packed_pixel& operator=(const packed_pixel& p)     { _VCfield=p._VCfield; return *this; }
...
at_c(packed_pixel<P,C,L>& p) {
    return typename kth_element_reference_type<packed_pixel<P,C,L>, K>::type(&p._VCfield);
}

template <int K, typename P, typename C, typename L> inline
typename kth_element_const_reference_type<packed_pixel<P,C,L>, K>::type
at_c(const packed_pixel<P,C,L>& p) {
    return typename kth_element_const_reference_type<packed_pixel<P,C,L>, K>::type(&p._VCfield);
}
