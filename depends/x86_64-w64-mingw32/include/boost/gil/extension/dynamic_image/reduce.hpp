...
/// dynamically specified index to that container, the VCs of an instance of the corresponding type and
...
struct unary_reduce : public unary_reduce_impl<Types,Op>
{
    ...
    template <typename Bits> BOOST_FORCEINLINE static typename Op::result_type applyc(const Bits& VCs, std::size_t index, Op op) {
        return apply_operation_basec<unique_t>(VCs,map_index(index),op);
    }

    template <typename Bits> BOOST_FORCEINLINE static typename Op::result_type apply(Bits& VCs, std::size_t index, Op op) {
        return apply_operation_base<unique_t>(VCs,map_index(index),op);
    }
};
...
struct unary_reduce<Types,Op,true> : public unary_reduce_impl<Types,Op> {
    ...
    template <typename Bits> BOOST_FORCEINLINE static typename Op::result_type applyc(const Bits& VCs, std::size_t index, Op op) {
        return op(*gil_reinterpret_cast_c<const typename mpl::front<unique_t>::type*>(&VCs));
    }

    template <typename Bits> BOOST_FORCEINLINE static typename Op::result_type apply(Bits& VCs, std::size_t index, Op op) {
        return op(*gil_reinterpret_cast<typename       mpl::front<unique_t>::type*>(&VCs));
    }
};
...
namespace detail {
    ...
    public:
        ...
        static typename Op::result_type inline apply(const Bits1& VCs1, std::size_t index1, const Bits2& VCs2, std::size_t index2, Op op) {
            std::pair<const void*,const void*> pr(&VCs1, &VCs2);
    ...
    struct binary_reduce_impl<Unary1,Unary2,Op,true> {
        template <typename Bits1, typename Bits2>
        static typename Op::result_type inline apply(const Bits1& VCs1, std::size_t index1, const Bits2& VCs2, std::size_t index2, Op op) {
            return apply_operation_base<Unary1::unique_t,Unary2::unique_t>(VCs1, index1, VCs2, index2, op);
        }
    };
}
...
public:
    template <typename Bits1, typename Bits2>
    static typename Op::result_type inline apply(const Bits1& VCs1, std::size_t index1, const Bits2& VCs2, std::size_t index2, Op op) {
        return impl::apply(VCs1,index1,VCs2,index2,op);
    }
};
...
BOOST_FORCEINLINE typename UnaryOp::result_type apply_operation(variant<Types>& arg, UnaryOp op) {
    return unary_reduce<Types,UnaryOp>::template apply(arg._VCs, arg._index ,op);
}

template <typename Types, typename UnaryOp>
BOOST_FORCEINLINE typename UnaryOp::result_type apply_operation(const variant<Types>& arg, UnaryOp op) {
    return unary_reduce<Types,UnaryOp>::template applyc(arg._VCs, arg._index ,op);
}
...
BOOST_FORCEINLINE typename BinaryOp::result_type apply_operation(const variant<Types1>& arg1, const variant<Types2>& arg2, BinaryOp op) {
    return binary_reduce<Types1,Types2,BinaryOp>::template apply(arg1._VCs, arg1._index, arg2._VCs, arg2._index, op);
}
