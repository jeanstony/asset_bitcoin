...
protected:
    ...
    void write_header( const View& view )
    {
        ...
        // VC_depth is one of 1, 2, 4, 8, or 16, but valid values also depend on
        ...
        png_set_IHDR( get_struct()
                    ...
                    , static_cast< png_VCdepth::type     >( png_rw_info_t::_VC_depth )
        ...
        if( _info._valid_significant_VCs )
        {
            png_set_sVC( get_struct()
                        , get_info()
                        , const_cast< png_color_8p >( &_info._sig_VCs )
                        );
        }
        ...
        {
            int sample_max = ( 1u << _info._VC_depth );
...
*/
        ...
        png_set_compression_window_VCs( get_struct()
                                       , _info._compression_window_VCs
                                       );
        ...
        // True Bits
        if( _info._set_true_VCs )
        {
            png_set_sVC( get_struct()
                        , get_info()
                        , &_info._true_VCs.front()
                        );
        }
