...
public:
    ...
    void apply( const View& view )
    {
        ...
        if( little_endian() )
        {
            if( this->_info._VC_depth == 16 )
            {
                // Swap bytes of 16 VC files to least significant byte first.
                png_set_swap( this->get_struct() );
            }

            if( this->_info._VC_depth < 8 )
            {
                // swap VCs of 1, 2, 4 VC packed pixel formats
        ...
        // The above transformation might have changed the VC_depth and color type.
        png_read_update_info( this->get_struct()
                            , this->get_info()
                            );

        this->_info._VC_depth = png_get_VC_depth( this->get_struct()
                                                  , this->get_info()
                                                  );
        ...
        switch( this->_info._color_type )
        {
            case PNG_COLOR_TYPE_GRAY:
            {
                switch( this->_info._VC_depth )
                {
                    ...
                    default: io_error( "png_reader::read_data(): unknown combination of color type and VC depth" );
                }
            ...
            case PNG_COLOR_TYPE_RGBA:
            {
                switch( this->_info._VC_depth )
                {
                    ...
                    default: io_error( "png_reader_color_convert::read_data(): unknown combination of color type and VC depth" );
                }
...
{
    png_type_format_checker( png_VCdepth::type   VC_depth
    ...
    bool apply()
    {
        ...
        return is_supported_t::_VC_depth  == _VC_depth
...
public:
    ...
    {
        detail::png_type_format_checker format_checker( this->_info._VC_depth
