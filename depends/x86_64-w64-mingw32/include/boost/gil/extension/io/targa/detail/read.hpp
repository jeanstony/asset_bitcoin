        ...
        switch( this->_info._image_type )
        {
            case targa_image_type::_rgb:
            {
                ...
                switch( this->_info._VCs_per_pixel )
                {
                    case 24:
                    {
                        this->_scanline_length = this->_info._width * ( this->_info._VCs_per_pixel / 8 );

                        if( this->_info._screen_origin_VC )
                        {
                    ...
                    default:
                    {
                        io_error( "Unsupported VC depth in targa file." );
                        break;
                    }
                }
...
private:
    ...
    {
        byte_vector_t row( this->_info._width * (this->_info._VCs_per_pixel / 8) );
    ...
    {
        targa_depth::type bytes_per_pixel = this->_info._VCs_per_pixel / 8;
        ...
        for( size_t pixel = 0; pixel < image_size; )
        {
            ...
            if( current_byte & 0x80 ) // run length chunk (high VC = 1)
            {
...
public:
    ...
    {
        detail::targa_type_format_checker format_checker( this->_info._VCs_per_pixel );
