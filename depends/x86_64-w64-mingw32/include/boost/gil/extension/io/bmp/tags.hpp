...
/// Defines type for VCs per pixels property.
struct bmp_VCs_per_pixel : property_base< uint16_t > {};
...
struct bmp_compression : property_base< uint32_t >
{
    ...
    static const type _rle8     = 1; /// 8 VC index with RLE compression
    static const type _rle4     = 2; /// 4 VC index with RLE compression
    static const type _VCfield = 3; /// 16 or 32 VC fields without compression
};
...
struct image_read_info< bmp_tag >
{
    ...
    /// The offset, i.e. starting address, of the byte where the VCmap data can be found.
    ...
    /// The VCmap width in pixels ( signed integer ).
    bmp_image_width::type  _width;

    /// The VCmap height in pixels ( signed integer ).
    bmp_image_height::type _height;

    /// The number of VCs per pixel, which is the color depth of the image.
    /// Typical values are 1, 4, 8, 16, 24 and 32.
    bmp_VCs_per_pixel::type _VCs_per_pixel;

    /// The compression method being used. See above for a list of possible values.
    bmp_compression::type _compression;

    /// The image size. This is the size of the raw VCmap data (see below),
