...
private:
    ...
    void initialize()
    {
        if( this->_info._VCs_per_pixel < 8 )
        {
            _pitch = (( this->_info._width * this->_info._VCs_per_pixel ) + 7 ) >> 3;
        }
        else
        {
            _pitch = this->_info._width * (( this->_info._VCs_per_pixel + 7 ) >> 3);
        ...
        switch( this->_info._VCs_per_pixel )
        {
            case 1:
            {
                ...
                _read_function = std::mem_fn(&this_t::read_1_VC_row);
            ...
            case 4:
            {
                switch( this->_info._compression )
                {
                    ...
                    case bmp_compression::_rgb :
                    {
                        ...
                        _read_function = std::mem_fn(&this_t::read_4_VCs_row);
            ...
            case 8:
            {
                switch( this->_info._compression )
                {
                    ...
                    case bmp_compression::_rgb:
                    {
                        ...
                        _read_function = std::mem_fn(&this_t::read_8_VCs_row);
            ...
            case 16:
            {
                ...
                if( this->_info._compression == bmp_compression::_VCfield )
                {
                ...
                _read_function = std::mem_fn(&this_t::read_15_VCs_row);
            ...
            default:
            {
                io_error( "Unsupported VCs per pixel." );
            }
        }
    }
    ...
    void read_palette()
    {
        ...
        if( entries == 0 )
        {
            entries = 1u << this->_info._VCs_per_pixel;
        }
    ...
    // Read 1 VC image. The colors are encoded by an index.
    void read_1_VC_row( byte_t* dst )
    {
        this->_io_dev.read( &_buffer.front(), _pitch );
        _mirror_VCs( _buffer );

        read_VC_row< gray1_image_t::view_t >( dst );
    }

    // Read 4 VCs image. The colors are encoded by an index.
    void read_4_VCs_row( byte_t* dst )
    {
        this->_io_dev.read( &_buffer.front(), _pitch );
        _swap_half_bytes( _buffer );

        read_VC_row< gray4_image_t::view_t >( dst );
    }

    /// Read 8 VCs image. The colors are encoded by an index.
    void read_8_VCs_row( byte_t* dst )
    {
        this->_io_dev.read( &_buffer.front(), _pitch );

        read_VC_row< gray8_image_t::view_t >( dst );
    }

    /// Read 15 or 16 VCs image.
    void read_15_VCs_row( byte_t* dst )
    {
...
private:
    ...
    detail::mirror_VCs <std::vector<byte_t>, std::true_type> _mirror_VCs;
