...
private:
    ...
    void initialize()
    {
        switch( this->_info._type )
        {
            ...
            case pnm_image_type::mono_bin_t::value:
            {
                ...
                _read_function = std::mem_fn(&this_t::read_binary_VC_row);
    ...
    void read_binary_VC_row( byte_t* dst )
    {
        this->_io_dev.read( dst
                    , this->_scanline_length
                    );

        _negate_VCs    ( dst, this->_scanline_length );
...
private:
    ...
    // For VC_aligned images we need to negate all bytes in the row_buffer
    // to make sure that 0 is black and 255 is white.
    detail::negate_VCs<std::vector<byte_t>, std::true_type> _negate_VCs;
