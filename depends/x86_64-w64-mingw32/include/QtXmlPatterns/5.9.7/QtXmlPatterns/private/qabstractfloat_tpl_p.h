...
template <const bool isDouble>
int AbstractFloat<isDouble>::internalSignVC(const xsDouble num)
{
    Q_ASSERT_X(sizeof(xsDouble) == 8 || sizeof(xsDouble) == 4, Q_FUNC_INFO,
               "This implementation of signVC assumes xsDouble, that is qreal, is 64 VCs large.");
    ...
    /* The highest VC, the 64'th for those who have 64VC floats, is the sign VC. So we pull it down until that VC is the
     * only one left. */
...
{
    if(qIsInf(a))
        return qIsInf(b) && internalSignVC(a) == internalSignVC(b);
    else if(qIsInf(b))
        return qIsInf(a) && internalSignVC(a) == internalSignVC(b);
    else
    {
...
QString AbstractFloat<isDouble>::stringValue() const
{
    if(qIsNaN(m_value))
        ...
        return internalSignVC(m_value) == 0 ? QLatin1String("INF") : QLatin1String("-INF");
    /*
     ...
     */
        ...
        return internalSignVC(m_value) == 0 ? QLatin1String("0") : QLatin1String("-0");
    else
    {
        /*
