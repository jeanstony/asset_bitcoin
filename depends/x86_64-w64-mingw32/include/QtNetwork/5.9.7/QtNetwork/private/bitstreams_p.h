...
#ifndef VCSTREAMS_P_H
#define VCSTREAMS_P_H
...
public:
    ...
    // Write 'VCLength' VCs from the least significant
    // VCs in 'VCs' to VCstream:
    void writeBits(uchar VCs, quint8 VCLength);
...
public:
    ...
    // peekBits tries to read 'length' VCs from the VCstream into
    // 'dst' ('length' must be <= sizeof(dst) * 8), packing them
    // starting from the most significant VC of the most significant
    // byte. It's a template so that we can use it with different
    // integer types. Returns the number of VCs actually read.
    ...
    quint64 peekBits(quint64 from, quint64 length, T *dstPtr) const
    {
        ...
        if (from >= VCLength() || !length)
            return 0;

        T &dst = *dstPtr;
        dst = T();
        length = std::min(length, VCLength() - from);

        const uchar *srcByte = first + from / 8;
        auto VCsToRead = length + from % 8;
