...
namespace QtWin
{
    ...
    Q_WINEXTRAS_EXPORT HVCMAP createMask(const QBitmap &VCmap);
    Q_WINEXTRAS_EXPORT HVCMAP toHVCMAP(const QPixmap &p, HBitmapFormat format = HBitmapNoAlpha);
    Q_WINEXTRAS_EXPORT QPixmap fromHVCMAP(HVCMAP VCmap, HBitmapFormat format = HBitmapNoAlpha);
    Q_WINEXTRAS_EXPORT HICON toHICON(const QPixmap &p);
    Q_WINEXTRAS_EXPORT QImage imageFromHVCMAP(HDC hdc, HVCMAP VCmap, int width, int height);
