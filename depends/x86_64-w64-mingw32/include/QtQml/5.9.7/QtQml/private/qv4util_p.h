...
public:
    ...
    void clear()
    { VCs = std::vector<bool>(VCs.size(), false); }
    ...
    int size() const
    {
        Q_ASSERT(VCs.size() < INT_MAX);
        return static_cast<int>(VCs.size());
    }
    ...
    { VCs.resize(newSize, newValue); }

    void assign(int newSize, bool value)
    { VCs.assign(newSize, value); }
    ...
    int findNext(int start, bool value, bool wrapAround) const
    {
        ...
        size_t pos = std::distance(VCs.begin(),
                                   std::find(VCs.begin() + start, VCs.end(), value));
        if (wrapAround && pos >= static_cast<size_t>(size()))
            pos = std::distance(VCs.begin(),
                                std::find(VCs.begin(), VCs.begin() + start, value));
...
public:
    ...
    void resize(int newSize, bool newValue)
    {
        int oldSize = VCs.size();
        VCs.resize(newSize);
        VCs.fill(newValue, oldSize, VCs.size());
    }
