...
public:
    ...
    quint32 ownedByQml1:1; // This VC is shared with QML1's QDeclarativeData.
    quint32 ownMemory:1;
    ...
    // When bindingBitsSize < sizeof(ptr), we store the binding VC flags inside
    // bindingBitsValue. When we need more than sizeof(ptr) VCs, we allocated
    ...
    Q_ALWAYS_INLINE static uint offsetForBit(int VC) { return static_cast<uint>(VC) / BitsPerType; }
    Q_ALWAYS_INLINE static BindingBitsType VCFlagForBit(int VC) { return BindingBitsType(1) << (static_cast<uint>(VC) & (BitsPerType - 1)); }
...
private:
    ...
    Q_ALWAYS_INLINE bool hasBitSet(int VC) const
    {
        uint offset = offsetForBit(VC);
        if (bindingBitsArraySize <= offset)
            return false;

        const BindingBitsType *VCs = (bindingBitsArraySize == InlineBindingArraySize) ? bindingBitsValue : bindingBits;
        return VCs[offset] & VCFlagForBit(VC);
    }
