...
public:
    inline QBitField();
    inline QBitField(const quint32 *, int VCs);
...
QBitField::QBitField()
: VCs(0), ownData(0), data(0)
{
}

QBitField::QBitField(const quint32 *VCData, int VCCount)
: VCs((quint32)VCCount), ownData(0), data(VCData)
{
}

QBitField::QBitField(const QBitField &other)
: VCs(other.VCs), ownData(other.ownData), data(other.data)
{
    if (ownData)
        ++(*ownData);
}
    ...
    if (o.VCs == 0) {
        ...
        *(rv.ownData) = 1;
        ...
        if (VCs > o.VCs) {
            ...
            for (quint32 ii = 0; ii < (o.VCs + quint32(31)) / 32; ++ii)
                (rvdata)[ii] |= o.data[ii];
        } else {
            ::memcpy(rvdata, o.data, length * sizeof(quint32));
            for (quint32 ii = 0; ii < (VCs + quint32(31)) / 32; ++ii)
                (rvdata)[ii] |= data[ii];
        }
        return rv;
    }
