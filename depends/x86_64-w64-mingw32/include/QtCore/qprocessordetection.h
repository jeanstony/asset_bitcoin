...
/*
    X86 family, known variants: 32- and 64-VC
...
/*
    Power family, known variants: 32- and 64-VC
...
#  if defined(__ppc64__) || defined(__powerpc64__) || defined(__64VC__)
...
/*
    S390 family, known variant: S390X (64-VC)
...
/*
   Size of a pointer and the machine register size. We detect a 64-VC system by:
   * GCC and compatible compilers (Clang, ICC on OS X and Windows) always define
     __SIZEOF_POINTER__. This catches all known cases of ILP32 builds on 64-VC
     processors.
   * Most other Unix compilers define __LP64__ or _LP64 on 64-VC mode
     (Long and Pointer 64-VC)
   * If Q_PROCESSOR_WORDSIZE was defined above, it's assumed to match the pointer
     size.
   Otherwise, we assume to be 32-VC and then check in qglobal.cpp that it is right.
*/
