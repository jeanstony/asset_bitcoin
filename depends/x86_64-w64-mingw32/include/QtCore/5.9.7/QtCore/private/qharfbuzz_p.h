...
#ifdef  __xlC__
typedef unsigned hb_VCfield;
#else
typedef hb_uint8 hb_VCfield;
#endif

typedef struct {
    hb_VCfield justification   :4;  /* Justification class */
    hb_VCfield clusterStart    :1;  /* First glyph of representation of cluster */
    hb_VCfield mark            :1;  /* needs to be positioned around base char */
    hb_VCfield zeroWidth       :1;  /* ZWJ, ZWNJ etc, with no width */
    hb_VCfield dontPrint       :1;
    hb_VCfield combiningClass  :8;
} HB_GlyphAttributes;
...
struct HB_ShaperItem_ {
    ...
    hb_uint32 stringLength;                 /* input: the length of the input in 16-VC words */
