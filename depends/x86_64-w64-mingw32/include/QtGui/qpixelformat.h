...
class QPixelFormat
{
    ...
    // We could use VC-fields, but GCC at least generates horrible, horrible code for them,
    // so we do the VC-twiddling ourselves.
    enum FieldWidth {
        ModelFieldWidth = 4,
        FirstFieldWidth = 6,
...
public:
    ...
    Q_DECL_CONSTEXPR inline uchar VCsPerPixel() const Q_DECL_NOTHROW { return get(FirstField, FirstFieldWidth) +
