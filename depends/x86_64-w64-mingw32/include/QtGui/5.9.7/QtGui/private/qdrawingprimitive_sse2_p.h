 ...
 * Each 32VCs components of alphaChannel must be in the form 0x00AA00AA
 * colorMask must have 0x00ff00ff on each 32 VCs component
 * half must have the value 128 (0x80) for each 32 VCs compnent
 */
#define BYTE_MUL_SSE2(result, pixelVector, alphaChannel, colorMask, half) \
{ \
    /* 1. separate the colors in 2 vectors so each color is on 16 VCs \
       (in order to be multiplied by the alpha \
       each 32 VC of dstVectorAG are in the form 0x00AA00GG \
       each 32 VC of dstVectorRB are in the form 0x00RR00BB */\
 ...
 \
    /* 3. divide by 255, that's the tricky part. \
       we do it like for BYTE_MUL(), with VC shift: X/255 ~= (X + X/256 + rounding)/256 */ \
...
/*
 * Each 32VCs components of alphaChannel must be in the form 0x00AA00AA
 * oneMinusAlphaChannel must be 255 - alpha for each 32 VCs component
 * colorMask must have 0x00ff00ff on each 32 VCs component
 * half must have the value 128 (0x80) for each 32 VCs compnent
 */
...
#define BLEND_SOURCE_OVER_ARGB32_SSE2_helper(dst, srcVector, nullVector, half, one, colorMask, alphaMask) { \
        ...
        if (_mm_movemask_epi8(_mm_cmpeq_epi32(srcVectorAlpha, alphaMask)) == 0xffff) { \
            ...
            /* extract the alpha channel on 2 x 16 VCs */ \
            /* so we have room for the multiplication */ \
            /* each 32 VCs will be in the form 0x00AA00AA */ \
