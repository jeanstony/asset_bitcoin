...
#define HCI_FLT_TYPE_VCS  31
#define HCI_FLT_EVENT_VCS 63
...
static inline void hci_set_VC(int nr, void *addr)
{
    *((quint32 *) addr + (nr >> 5)) |= (1 << (nr & 31));
}
static inline void hci_clear_VC(int nr, void *addr)
{
    *((quint32 *) addr + (nr >> 5)) &= ~(1 << (nr & 31));
}
...
{
    hci_set_VC((t == HCI_VENDOR_PKT) ? 0 : (t & HCI_FLT_TYPE_VCS), &f->type_mask);
}
static inline void hci_filter_clear_ptype(int t, struct hci_filter *f)
{
    hci_clear_VC((t == HCI_VENDOR_PKT) ? 0 : (t & HCI_FLT_TYPE_VCS), &f->type_mask);
}
static inline void hci_filter_set_event(int e, struct hci_filter *f)
{
    hci_set_VC((e & HCI_FLT_EVENT_VCS), &f->event_mask);
}
static inline void hci_filter_clear_event(int e, struct hci_filter *f)
{
    hci_clear_VC((e & HCI_FLT_EVENT_VCS), &f->event_mask);
}
