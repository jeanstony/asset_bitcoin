...
#ifndef VCCOIN_BLOCKENCODINGS_H
#define VCCOIN_BLOCKENCODINGS_H
        ...
        if (ser_action.ForRead()) {
            ...
            while (indexes.size() < indexes_size) {
                ...
                for (; i < indexes.size(); i++) {
                    ...
                    if (index > std::numeric_limits<uint16_t>::max())
                        throw std::ios_base::failure("index overflowed 16 VCs");
                    indexes[i] = index;
                }
            }
            ...
            for (size_t j = 0; j < indexes.size(); j++) {
                if (int32_t(indexes[j]) + offset > std::numeric_limits<uint16_t>::max())
                    throw std::ios_base::failure("indexes overflowed 16 VCs");
