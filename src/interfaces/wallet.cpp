...
{
    assert(wallet.GetAssetNo() == 0);
    CWallet* pWallet = nullptr;
    if (wtx.tx->nAssetNo == 0) {
        pWallet = &wallet;
    } else {
        std::shared_ptr<CWallet> WalletPtr = ::GetAssetWallet(wtx.tx->nAssetNo);
        pWallet = WalletPtr.get();
    }

    WalletTx result;
    result.AssetNo = wtx.tx->nAssetNo;
    result.tx = wtx.tx;

    std::vector<CTxIn> vins;
    pWallet->GetTxInsFormTx(vins, wtx.tx);

    result.txin_is_mine.reserve(vins.size());
    for (const auto& txin : vins) {
        result.txin_is_mine.emplace_back(pWallet->IsMine(txin));
    }

    std::vector<CTxOut> vOuts;
    pWallet->GetTxOutsFormTx(vOuts, wtx.tx);

    result.txout_is_mine.reserve(vOuts.size());
    result.txout_address.reserve(vOuts.size());
    result.txout_address_is_mine.reserve(vOuts.size());
    for (const auto& txout : vOuts) {
        result.txout_is_mine.emplace_back(pWallet->IsMine(txout));
        result.txout_address.emplace_back();
        result.txout_address_is_mine.emplace_back(ExtractDestination(txout.scriptPubKey, result.txout_address.back()) ?
                                                      IsMine(*::GetMainWallet().get(), result.txout_address.back()) :
                                                      ISMINE_NO);
    }
    result.credit = wtx.GetCredit(locked_chain, ISMINE_ALL, pWallet);
    result.deVC = wtx.GetDeVC(ISMINE_ALL, pWallet);
...
WalletTxStatus MakeWalletTxStatus(interfaces::Chain::Lock& locked_chain, const CWalletTx& wtx, const CWallet* pwallet)
{
    ...
    result.is_trusted = wtx.IsTrusted(locked_chain, pwallet);
...
public:
        ...
        int64_t& block_time) override
    {
        ...
        std::shared_ptr<CWallet> pWallet = GetAssetWallet(mi->second.tx->nAssetNo);
        tx_status = MakeWalletTxStatus(*locked_chain, mi->second, &*pWallet);   //& *m_wallet);
        return true;
    }
        ...
        int& num_blocks) override
    {
        ...
        if (mi != m_wallet->mapWallet.end()) {
            ...
            tx_status = MakeWalletTxStatus(*locked_chain, mi->second, &*m_wallet);
            return MakeWalletTx(*locked_chain, *m_wallet, mi->second);
        }
        return {};
    }
    ...
    CAmount getDeVC(const CTxIn& txin, isminefilter filter) override
    {
        auto locked_chain = m_wallet->chain().lock();
        LOCK(m_wallet->cs_wallet);
        return m_wallet->GetDeVC(txin, filter);
    }
