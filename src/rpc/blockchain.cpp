...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("VCs", strprintf("%08x", blockindex->nBits));
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("VCs", strprintf("%08x", block.nBits));
    result.pushKV("difficulty", GetDifficulty(blockindex));
    result.pushKV("chainwork", blockindex->nChainWork.GetHex());
    result.pushKV("nTx", (uint64_t)blockindex->nTx);
...
static std::string EntryDescriptionString()
{
    return "    \"vsize\" : n,            (numeric) virtual transaction size as defined in BIP 141. This is different from actual serialized size for witness transactions as witness data is discounted.\n"
           "    \"size\" : n,             (numeric) (DEPRECATED) same as vsize. Only returned if VCcoind is started with -deprecatedrpc=size\n"
...
struct CCoinsStats {
    ...
    uint64_t nDiskSize;
    std::map<const int, CAmount> nTotalAmount;
...
static void ApplyStats(CCoinsStats& stats, CHashWriter& ss, const uint256& hash, const std::map<uint32_t, Coin>& outputs)
{
    ...
    std::shared_ptr<CWallet> walletptr = ::GetMainWallet();
    CWallet* pWallet = walletptr.get();

    LOCK(pWallet->cs_wallet);
    auto cur = pWallet->mapWallet.find(hash);
    assert(cur != pWallet->mapWallet.end());
    int AssetNo = cur->second.tx->nAssetNo;
    ...
    for (const auto& output : outputs) {
        ...
        if (stats.nTotalAmount.find(AssetNo) != stats.nTotalAmount.end()) {
            stats.nTotalAmount[AssetNo] += output.second.out.nValue;
        } else {
            stats.nTotalAmount.insert(std::make_pair(AssetNo, output.second.out.nValue));
        }
...
static UniValue gettxoutsetinfo(const JSONRPCRequest& request)
{
    ...
    if (GetUTXOStats(pcoinsdbview.get(), stats)) {
        ...
        UniValue amountinfo(UniValue::VARR);
        for (auto cur : stats.nTotalAmount) {
            CoinAsset ca;
            CoinAssetManager::Instance().GetAsset(cur.first, ca);

            UniValue amount(UniValue::VOBJ);
            amount.pushKV("asset .No", (int)cur.first);
            amount.pushKV("amount", ValueFromAmount(cur.second, ca.coin));
            amountinfo.push_back(amount);
        }
        ret.pushKV("total_amount", amountinfo);
    } else {
...
UniValue gettxout(const JSONRPCRequest& request)
{
    ...
    int AssetNo;
    {
        ...
        auto cur = pwallet->mapWallet.find(hash);
        assert(cur != pwallet->mapWallet.end());
        AssetNo = cur->second.tx->nAssetNo;
    }
    ...
    ret.pushKV("value", ValueFromAmount(coin.out.nValue, ca.coin));
    UniValue o(UniValue::VOBJ);
...
static UniValue BIP9SoftForkDesc(const Consensus::Params& consensusParams, Consensus::DeploymentPos id)
{
    ...
    if (ThresholdState::STARTED == thresholdState) {
        rv.pushKV("VC", consensusParams.vDeployments[id].VC);
    }
...
UniValue getblockchaininfo(const JSONRPCRequest& request)
{
    RPCHelpMan{
        ...
        RPCResult{
            ...
            "        \"VC\": xx,              (numeric) the VC (0-28) in the block version field used to signal this softfork (only for \"started\" status)\n"
            "        \"startTime\": xx,        (numeric) the minimum median time past of a block at which the VC gains its meaning\n"
            "        \"timeout\": xx,          (numeric) the median time past of a block at which the deployment is considered failed if not yet locked in\n"
            "        \"since\": xx,            (numeric) height of the first block to which the status applies\n"
            "        \"statistics\": {         (object) numeric statistics about BIP9 signalling for a softfork (only for \"started\" status)\n"
            "           \"period\": xx,        (numeric) the length in blocks of the BIP9 signalling period \n"
            "           \"threshold\": xx,     (numeric) the number of blocks with the version VC set required to activate the feature \n"
            "           \"elapsed\": xx,       (numeric) the number of blocks elapsed since the beginning of the current period \n"
            "           \"count\": xx,         (numeric) the number of blocks with the version VC set in the current period \n"
    ...
    for (int pos = Consensus::DEPLOYMENT_CSV; pos != Consensus::MAX_VERSION_VCS_DEPLOYMENTS; ++pos) {
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    { "blockchain",         "getblockchaininfo",      &getblockchaininfo,      {} },                                        // ok
    { "blockchain",         "getchaintxstats",        &getchaintxstats,        {"nblocks", "blockhash"} },                  // ok
    { "blockchain",         "getblockstats",          &getblockstats,          {"hash_or_height", "stats"} },               // ok
    { "blockchain",         "getbestblockhash",       &getbestblockhash,       {} },                                        // ok
    { "blockchain",         "getblockcount",          &getblockcount,          {} },                                        // ok
    { "blockchain",         "getblock",               &getblock,               {"blockhash","verbosity|verbose"} },         // ok
    { "blockchain",         "getblockhash",           &getblockhash,           {"height"} },                                // ok
    { "blockchain",         "getblockheader",         &getblockheader,         {"blockhash","verbose"} },                   // ok
    { "blockchain",         "getchaintips",           &getchaintips,           {} },                                        // ok
    { "blockchain",         "getdifficulty",          &getdifficulty,          {} },                                        // ok
    { "blockchain",         "getmempoolancestors",    &getmempoolancestors,    {"txid","verbose"} },                        // ok
    { "blockchain",         "getmempooldescendants",  &getmempooldescendants,  {"txid","verbose"} },                        // ok
    { "blockchain",         "getmempoolentry",        &getmempoolentry,        {"txid"} },                                  // ok
    { "blockchain",         "getmempoolinfo",         &getmempoolinfo,         {} },                                        // fixed me
    { "blockchain",         "getrawmempool",          &getrawmempool,          {"verbose"} },                               // ok
    { "blockchain",         "gettxout",               &gettxout,               {"txid","n","include_mempool"} },            // ok
    { "blockchain",         "gettxoutsetinfo",        &gettxoutsetinfo,        {} },                                        // ok
    { "blockchain",         "pruneblockchain",        &pruneblockchain,        {"height"} },                                // ok
    { "blockchain",         "savemempool",            &savemempool,            {} },                                        // ok
    { "blockchain",         "verifychain",            &verifychain,            {"checklevel","nblocks"} },                  // ok

    { "blockchain",         "preciousblock",          &preciousblock,          {"blockhash"} },                             // ok
    { "blockchain",         "scantxoutset",           &scantxoutset,           {"action", "scanobjects"} },                 // fixed me
    { "blockchain",         "getblockfilter",         &getblockfilter,         {"blockhash", "filtertype"} },               

    /* Not shown in help */
    { "hidden",             "invalidateblock",        &invalidateblock,        {"blockhash"} },                             // ok
    { "hidden",             "reconsiderblock",        &reconsiderblock,        {"blockhash"} },                             // ok
    { "hidden",             "waitfornewblock",        &waitfornewblock,        {"timeout"} },                               // ok
    { "hidden",             "waitforblock",           &waitforblock,           {"blockhash","timeout"} },                   // ok
    { "hidden",             "waitforblockheight",     &waitforblockheight,     {"height","timeout"} },                      // ok
    { "hidden",             "syncwithvalidationinterfacequeue", &syncwithvalidationinterfacequeue, {} },                    // ok
};
// clang-format on
