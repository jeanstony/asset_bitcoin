...
const int AssetNoFromValue(const UniValue& value)
{
    if (!value.isNum() && !value.isStr())
        throw JSONRPCError(RPC_TYPE_ERROR, "Amount is not a number or string");

    const std::string strVal = value.getValStr();
    size_t pos = strVal.find("#");
    if (pos == -1) {
        return 0;           // main coin
    }

    const std::string strAssetNo = strVal.substr(pos + 1);
    if (strspn(strAssetNo.c_str(), "0123456789") != strAssetNo.size()) {
        throw JSONRPCError(RPC_TYPE_ERROR, "Asset No. is not a number");
    }

    return ::atoi(strAssetNo.c_str());
}

CAmount AmountFromValue(const UniValue& value, const CAmount& Max, const CAmount& percision)
{
    if (!value.isNum() && !value.isStr())
        throw JSONRPCError(RPC_TYPE_ERROR, "Amount is not a number or string");
    ...
    CAmount amount;
    if (!ParseFixedPoint(strAmount, per, &amount))
