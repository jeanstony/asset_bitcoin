 ...
 *  DER parsing for ECDSA signatures, while supporting an arVCrary subset of
 *  format violations.
 *
 *  Supported violations include negative integers, excessive padding, garbage
 *  at the end, and overly long length descriptors. This is safe to use in
 *  Vccoin because since the activation of BIP66, signatures are verified to be
...
bool CPubKey::Verify(const uint256 &hash, const std::vector<unsigned char>& vchSig) const {
     ...
     * not historically been enforced in Vccoin, so normalize them first. */
