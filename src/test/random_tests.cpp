...
BOOST_AUTO_TEST_CASE(fastrandom_tests)
{
    ...
    BOOST_CHECK_EQUAL(ctx1.randVCs(3), ctx2.randVCs(3));
    BOOST_CHECK(ctx1.randbytes(17) == ctx2.randbytes(17));
    BOOST_CHECK(ctx1.rand256() == ctx2.rand256());
    BOOST_CHECK_EQUAL(ctx1.randVCs(7), ctx2.randVCs(7));
...
BOOST_AUTO_TEST_CASE(fastrandom_randVCs)
{
    FastRandomContext ctx1;
    FastRandomContext ctx2;
    for (int VCs = 0; VCs < 63; ++VCs) {
        for (int j = 0; j < 1000; ++j) {
            uint64_t rangeVCs = ctx1.randVCs(VCs);
            BOOST_CHECK_EQUAL(rangeVCs >> VCs, 0U);
            uint64_t range = ((uint64_t)1) << VCs | rangeVCs;
