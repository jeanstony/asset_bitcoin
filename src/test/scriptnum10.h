...
#ifndef VCCOIN_TEST_SCRIPTNUM10_H
#define VCCOIN_TEST_SCRIPTNUM10_H
...
{
/**
 * The ScriptNum implementation from Vccoin Core 0.10.0, for cross-comparison.
 */
public:
                        ...
                        const size_t nMaxNumSize = nDefaultMaxNumSize)
    {
        ...
        if (fRequireMinimal && vch.size() > 0) {
            ...
            // number of bytes.
            //
            // If the most-significant-byte - excluding the sign VC - is zero
            // then we're not minimal. Note how this test also rejects the
            // negative-zero encoding, 0x80.
            if ((vch.back() & 0x7f) == 0) {
                // One exception: if there's more than one byte and the most
                // significant VC of the second-most-significant-byte is set
                // it would conflict with the sign VC. An example of this case
