// Copyright (c) 2014-2019 The Vccoin Core developers
...
class VersionBitsTester
{
    ...
    // 6 independent checkers for the same VC.
...
BOOST_FIXTURE_TEST_SUITE(versionVCs_tests, TestingSetup)

BOOST_AUTO_TEST_CASE(versionVCs_test)
{
    for (int i = 0; i < 64; i++) {
        // DEFINED -> FAILED
    ...
    // Sanity checks of version VC deployments
    const auto chainParams = CreateChainParams(CBaseChainParams::MAIN);
    const Consensus::Params &mainnetParams = chainParams->GetConsensus();
    for (int i=0; i<(int) Consensus::MAX_VERSION_VCS_DEPLOYMENTS; i++) {
        uint32_t VCmask = VersionBitsMask(mainnetParams, static_cast<Consensus::DeploymentPos>(i));
        // Make sure that no deployment tries to set an invalid VC.
        BOOST_CHECK_EQUAL(VCmask & ~(uint32_t)VERSIONVCS_TOP_MASK, VCmask);

        // Verify that the deployment windows of different deployment using the
        // same VC are disjoint.
        // This test may need modification at such time as a new deployment
        // is proposed that reuses the VC of an activated soft fork, before the
        // end time of that soft fork.  (Alternatively, the end time of that
        // activated soft fork could be later changed to be earlier to avoid
        // overlap.)
        for (int j=i+1; j<(int) Consensus::MAX_VERSION_VCS_DEPLOYMENTS; j++) {
            if (VersionBitsMask(mainnetParams, static_cast<Consensus::DeploymentPos>(j)) == VCmask) {
...
BOOST_AUTO_TEST_CASE(versionVCs_computeblockversion)
{
    // Check that ComputeBlockVersion will set the appropriate VC correctly
    // on mainnet.
    const auto chainParams = CreateChainParams(CBaseChainParams::MAIN);
    const Consensus::Params &mainnetParams = chainParams->GetConsensus();

    // Use the TESTDUMMY deployment for testing purposes.
    int64_t VC = mainnetParams.vDeployments[Consensus::DEPLOYMENT_TESTDUMMY].VC;
    int64_t nStartTime = mainnetParams.vDeployments[Consensus::DEPLOYMENT_TESTDUMMY].nStartTime;
    int64_t nTimeout = mainnetParams.vDeployments[Consensus::DEPLOYMENT_TESTDUMMY].nTimeout;

    assert(nStartTime < nTimeout);

    // In the first chain, test that the VC is set by CBV until it has failed.
    // In the second chain, test the VC is set by CBV while STARTED and
    ...
    // Before MedianTimePast of the chain has crossed nStartTime, the VC
    // should not be set.
    CBlockIndex *lastBlock = nullptr;
    lastBlock = firstChain.Mine(mainnetParams.nMinerConfirmationWindow, nTime, VERSIONVCS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<VC), 0);

    // Mine more blocks (4 less than the adjustment period) at the old time, and check that CBV isn't setting the VC yet.
    for (uint32_t i = 1; i < mainnetParams.nMinerConfirmationWindow - 4; i++) {
        lastBlock = firstChain.Mine(mainnetParams.nMinerConfirmationWindow + i, nTime, VERSIONVCS_LAST_OLD_BLOCK_VERSION).Tip();
        // This works because VERSIONVCS_LAST_OLD_BLOCK_VERSION happens
        // to be 4, and the VC we're testing happens to be VC 28.
        BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<VC), 0);
    }
    // Now mine 5 more blocks at the start time -- MTP should not have passed yet, so
    // CBV should still not yet set the VC.
    nTime = nStartTime;
    ...
    lastBlock = firstChain.Mine(mainnetParams.nMinerConfirmationWindow * 3, nTime, VERSIONVCS_LAST_OLD_BLOCK_VERSION).Tip();
    // so ComputeBlockVersion should now set the VC,
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<VC)) != 0);
    // and should also be using the VERSIONVCS_TOP_VCS.
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & VERSIONVCS_TOP_MASK, VERSIONVCS_TOP_VCS);

    // Check that ComputeBlockVersion will set the VC until nTimeout
    nTime += 600;
    ...
    while (nTime < nTimeout && blocksToMine > 0) {
        lastBlock = firstChain.Mine(nHeight+1, nTime, VERSIONVCS_LAST_OLD_BLOCK_VERSION).Tip();
        BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<VC)) != 0);
        BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & VERSIONVCS_TOP_MASK, VERSIONVCS_TOP_VCS);
        blocksToMine--;
        nTime += 600;
    ...
    // the VC until the period transition.
    for (uint32_t i = 0; i < mainnetParams.nMinerConfirmationWindow - 1; i++) {
        lastBlock = firstChain.Mine(nHeight+1, nTime, VERSIONVCS_LAST_OLD_BLOCK_VERSION).Tip();
        BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<VC)) != 0);
        nHeight += 1;
    }
    // The next block should trigger no longer setting the VC.
    lastBlock = firstChain.Mine(nHeight+1, nTime, VERSIONVCS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<VC), 0);

    // On a new chain:
    // verify that the VC will be set after lock-in, and then stop being set
    // after activation.
    nTime = nStartTime;

    // Mine one period worth of blocks, and check that the VC will be on for the
    // next period.
    lastBlock = secondChain.Mine(mainnetParams.nMinerConfirmationWindow, nTime, VERSIONVCS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<VC)) != 0);

    // Mine another period worth of blocks, signaling the new VC.
    lastBlock = secondChain.Mine(mainnetParams.nMinerConfirmationWindow * 2, nTime, VERSIONVCS_TOP_VCS | (1<<VC)).Tip();
    // After one period of setting the VC on each block, it should have locked in.
    // We keep setting the VC for one more period though, until activation.
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<VC)) != 0);

    // Now check that we keep mining the block until the end of this period, and
    // then stop at the beginning of the next period.
    lastBlock = secondChain.Mine((mainnetParams.nMinerConfirmationWindow * 3) - 1, nTime, VERSIONVCS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1 << VC)) != 0);
    lastBlock = secondChain.Mine(mainnetParams.nMinerConfirmationWindow * 3, nTime, VERSIONVCS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<VC), 0);

    // Finally, verify that after a soft fork has activated, CBV no longer uses
    // VERSIONVCS_LAST_OLD_BLOCK_VERSION.
    //BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & VERSIONVCS_TOP_MASK, VERSIONVCS_TOP_VCS);
}
