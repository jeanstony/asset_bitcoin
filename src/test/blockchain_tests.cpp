...
static CBlockIndex* CreateBlockIndexWithNVCs(uint32_t nVCs)
{
    CBlockIndex* block_index = new CBlockIndex();
    block_index->nHeight = 46367;
    block_index->nTime = 1269211443;
    block_index->nBits = nVCs;
    return block_index;
}
...
/* Given a BlockIndex with the provided nVCs,
 * verify that the expected difficulty results.
 */
static void TestDifficulty(uint32_t nVCs, double expected_difficulty)
{
    CBlockIndex* block_index = CreateBlockIndexWithNVCs(nVCs);
