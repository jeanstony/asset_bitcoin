// Copyright (c) 2015-2019 The Vccoin Core developers
...
static void MerkleComputation(const std::vector<uint256>& leaves, uint256* proot, bool* pmutated, uint32_t branchpos, std::vector<uint256>* pbranch) {
    ...
    while (count < leaves.size()) {
        ...
        count++;
        int level;
        // For each of the lower VCs in count that are 0, do 1 step. Each
    ...
    int level = 0;
    // As long as VC number level in count is zero, skip it. It means there
    ...
    while (count != (((uint32_t)1) << level)) {
        ...
        // We combine it with itself (Vccoin's special rule for odd levels in
