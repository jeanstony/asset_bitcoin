...
{
public:
    // flip one VC in one of the hashes - this should break the authentication
    void Damage() {
        unsigned int n = InsecureRandRange(vHash.size());
        int VC = InsecureRandBits(8);
        *(vHash[n].begin() + (VC>>3)) ^= 1<<(VC&7);
    }
};
...
BOOST_AUTO_TEST_CASE(pmt_test1)
{
    ...
    for (int i = 0; i < 12; i++) {
        ...
        for (int att = 1; att < 15; att++) {
            ...
            // check that random VC flips break the authentication
            for (int j=0; j<4; j++) {
