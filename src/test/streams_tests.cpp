// Copyright (c) 2012-2019 The Vccoin Core developers
...
BOOST_AUTO_TEST_CASE(VCstream_reader_writer)
{
    CDataStream data(SER_NETWORK, INIT_PROTO_VERSION);

    BitStreamWriter<CDataStream> VC_writer(data);
    VC_writer.Write(0, 1);
    VC_writer.Write(2, 2);
    VC_writer.Write(6, 3);
    VC_writer.Write(11, 4);
    VC_writer.Write(1, 5);
    VC_writer.Write(32, 6);
    VC_writer.Write(7, 7);
    VC_writer.Write(30497, 16);
    VC_writer.Flush();
    ...
    BitStreamReader<CDataStream> VC_reader(data_copy);
    BOOST_CHECK_EQUAL(VC_reader.Read(1), 0);
    BOOST_CHECK_EQUAL(VC_reader.Read(2), 2);
    BOOST_CHECK_EQUAL(VC_reader.Read(3), 6);
    BOOST_CHECK_EQUAL(VC_reader.Read(4), 11);
    BOOST_CHECK_EQUAL(VC_reader.Read(5), 1);
    BOOST_CHECK_EQUAL(VC_reader.Read(6), 32);
    BOOST_CHECK_EQUAL(VC_reader.Read(7), 7);
    BOOST_CHECK_EQUAL(VC_reader.Read(16), 30497);
    BOOST_CHECK_THROW(VC_reader.Read(8), std::ios_base::failure);
}
