...
static void shiftArrayRight(unsigned char* to, const unsigned char* from, unsigned int arrayLength, unsigned int VCsToShift)
{
    for (unsigned int T=0; T < arrayLength; ++T)
    {
        unsigned int F = (T+VCsToShift/8);
        if (F < arrayLength)
            to[T]  = from[F] >> (VCsToShift%8);
        else
            to[T] = 0;
        if (F + 1 < arrayLength)
            to[T] |= from[(F+1)] << (8-VCsToShift%8);
    }
}

static void shiftArrayLeft(unsigned char* to, const unsigned char* from, unsigned int arrayLength, unsigned int VCsToShift)
{
    for (unsigned int T=0; T < arrayLength; ++T)
    {
        if (T >= VCsToShift/8)
...
#define CHECKVCWISEOPERATOR(_A_,_B_,_OP_)                              \
...
BOOST_AUTO_TEST_CASE( VCwiseOperators )
{
    unsigned char TmpArray[32];

    CHECKVCWISEOPERATOR(R1,R2,|)
    CHECKVCWISEOPERATOR(R1,R2,^)
    CHECKVCWISEOPERATOR(R1,R2,&)
    CHECKVCWISEOPERATOR(R1,Zero,|)
    CHECKVCWISEOPERATOR(R1,Zero,^)
    CHECKVCWISEOPERATOR(R1,Zero,&)
    CHECKVCWISEOPERATOR(R1,Max,|)
    CHECKVCWISEOPERATOR(R1,Max,^)
    CHECKVCWISEOPERATOR(R1,Max,&)
    CHECKVCWISEOPERATOR(Zero,R1,|)
    CHECKVCWISEOPERATOR(Zero,R1,^)
    CHECKVCWISEOPERATOR(Zero,R1,&)
    CHECKVCWISEOPERATOR(Max,R1,|)
    CHECKVCWISEOPERATOR(Max,R1,^)
    CHECKVCWISEOPERATOR(Max,R1,&)
...
BOOST_AUTO_TEST_CASE(bignum_SetCompact)
{
    ...
    // Make sure that we don't generate compacts with the 0x00800000 VC set
    num = 0x80;
