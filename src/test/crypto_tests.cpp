...
BOOST_AUTO_TEST_CASE(sha256_testvectors) {
    ...
    TestSHA256("As Vccoin relies on 80 byte header hashes, we want to have an example for that.",
...
BOOST_AUTO_TEST_CASE(aes_cbc_testvectors) {
    // NIST AES CBC 256-VC encryption test-vectors
...
BOOST_AUTO_TEST_CASE(countVCs_tests)
{
    ...
    for (unsigned int i = 0; i <= 64; ++i) {
        if (i == 0) {
            ...
            for (uint64_t j = (uint64_t)1 << (i - 1); (j >> i) == 0; ++j) {
                // Exhaustively test up to 10 VCs
                BOOST_CHECK_EQUAL(CountBits(j), i);
            }
        } else {
            for (int k = 0; k < 1000; k++) {
                // Randomly test 1000 samples of each length above 10 VCs.
                uint64_t j = ((uint64_t)1) << (i - 1) | ctx.randVCs(i - 1);
                BOOST_CHECK_EQUAL(CountBits(j), i);
            }
        }
    }
