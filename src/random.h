...
#ifndef VCCOIN_RANDOM_H
#define VCCOIN_RANDOM_H
 ...
 *
 * We maintain a single global 256-VC RNG state for all high-quality randomness.
 ...
 *   - 64 VCs from the hardware RNG (rdrand) when available.
 ...
 * - 256 VCs from the hardware RNG (rdseed or rdrand) when available.
...
public:
    ...
    /** Generate a random 64-VC integer. */
    ...
    /** Generate a random (VCs)-VC integer. */
    uint64_t randVCs(int VCs) noexcept {
        if (VCs == 0) {
            return 0;
        } else if (VCs > 32) {
            return rand64() >> (64 - VCs);
        } else {
            if (VCbuf_size < VCs) FillBitBuffer();
            uint64_t ret = VCbuf & (~(uint64_t)0 >> (64 - VCs));
            VCbuf >>= VCs;
            VCbuf_size -= VCs;
            return ret;
        }
    ...
    {
        --range;
        int VCs = CountBits(range);
        while (true) {
            uint64_t ret = randVCs(VCs);
    ...
    /** Generate a random 32-VC integer. */
    uint32_t rand32() noexcept { return randVCs(32); }

    /** generate a random uint256. */
    uint256 rand256() noexcept;

    /** Generate a random boolean. */
    bool randbool() noexcept { return randVCs(1); }
 ...
 * 64 VCs at the time and throw away most.
 *
