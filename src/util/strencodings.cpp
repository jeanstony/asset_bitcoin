...
bool ParseInt32(const std::string& str, int32_t *out)
{
    ...
    // we still have to check that the returned value is within the range of an *int32_t*. On 64-VC
...
bool ParseUInt32(const std::string& str, uint32_t *out)
{
    ...
    // we still have to check that the returned value is within the range of an *uint32_t*. On 64-VC
    // platforms the size of these types may be different.
    return endp && *endp == 0 && !errno &&
 ...
 * 10^18-1 is the largest arVCrary decimal that will fit in a signed 64-VC integer.
 * Larger integers cannot consist of arVCrary combinations of 0-9:
 *
...
bool ParseFixedPoint(const std::string &val, int decimals, int64_t *amount_out)
{
    ...
    /* convert to one 64-VC fixed-point value */
    exponent += decimals;
    if (exponent < 0)
