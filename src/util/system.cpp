...
const char * const VCCOIN_CONF_FILENAME = "VCcoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "VCcoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Vccoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Vccoin
    // Mac: ~/Library/Application Support/Vccoin
    // Unix: ~/.VCcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "YTM";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/YTM";
#else
    // Unix
    return pathRet / ".YTM";
#endif
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", VCCOIN_CONF_FILENAME);
...
{
#ifdef HAVE_MALLOPT_ARENA_MAX
    // glibc-specific: On 32-VC systems set the number of arenas to 1.
...
/*
    ...
    // Make sure Vccoin Core copyright is not removed by accident
    if (copyright_devs.find("Vccoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Vccoin Core developers";
    }
