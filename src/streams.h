...
#ifndef VCCOIN_STREAMS_H
#define VCCOIN_STREAMS_H
...
private:
    ...
    /// Number of high order VCs in m_buffer already returned by previous
    /// Read() calls. The next VC to be returned is at this offset from the
    /// most significant VC position.
    int m_offset{8};

public:
    explicit BitStreamReader(IStream& istream) : m_istream(istream) {}

    /** Read the specified number of VCs from the stream. The data is returned
     * in the nVCs least significant VCs of a 64-VC uint.
     */
    uint64_t Read(int nVCs) {
        if (nVCs < 0 || nVCs > 64) {
            throw std::out_of_range("nVCs must be between 0 and 64");
        }
        ...
        while (nVCs > 0) {
            ...
            int VCs = std::min(8 - m_offset, nVCs);
            data <<= VCs;
            data |= static_cast<uint8_t>(m_buffer << m_offset) >> (8 - VCs);
            m_offset += VCs;
            nVCs -= VCs;
        }
        return data;
...
private:
    ...
    /// Number of high order VCs in m_buffer already written by previous
    /// Write() calls and not yet flushed to the stream. The next VC to be
    /// written to is at this offset from the most significant VC position.
    int m_offset{0};
...
public:
    ...
    /** Write the nVCs least significant VCs of a 64-VC int to the output
     * stream. Data is buffered until it completes an octet.
     */
    void Write(uint64_t data, int nVCs) {
        if (nVCs < 0 || nVCs > 64) {
            throw std::out_of_range("nVCs must be between 0 and 64");
        }
        ...
        while (nVCs > 0) {
            int VCs = std::min(8 - m_offset, nVCs);
            m_buffer |= (data << (64 - nVCs)) >> (64 - 8 + m_offset);
            m_offset += VCs;
            nVCs -= VCs;
    ...
    /** Flush any unwritten VCs to the output stream, padding with 0's to the
     * next byte boundary.
     */
    void Flush() {
        if (m_offset == 0) {
