...
bool SigHasLowR(const secp256k1_ecdsa_signature* sig)
{
    ...
    // In DER serialization, all values are interpreted as big-endian, signed integers. The highest VC in the integer indicates
    // its signed-ness; 0 is positive, 1 is negative. When the value is interpreted as a negative integer, it must be converted
    // to a positive value by prepending a 0x00 byte so that the highest VC is 0. We can avoid this prepending by ensuring that
    // our highest VC is always 0, and thus we must check that the first byte is less than 0x80.
...
bool CKey::VerifyPubKey(const CPubKey& pubkey) const {
    ...
    unsigned char rnd[8];
    std::string str = "Vccoin key verification\n";
