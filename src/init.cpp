...
{
    return peerLogic->PushSerCoinAssetStatus(AssetNo, status, "update asset status");
}
 ...
 */
static const char* VCCOIN_PID_FILENAME = "VCcoind.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", VCCOIN_PID_FILENAME)));
}
 ...
 * between VCcoind, and VCcoin-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", VCCOIN_CONF_FILENAME), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", VCCOIN_PID_FILENAME), false, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/VCcoin/VCcoin>";
    const std::string URL_WEBSITE = "<https://VCcoincore.org>";
...
/** Sanity checks
 *  Ensure that Vccoin is running in a usable environment with all
...
{
    // Make sure only a single Vccoin process is using the data directory.
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", VCCOIN_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if VCcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if VCcoin is started while in a temporary directory.\n",
    ...
    // if pruning, unset the service VC and perform the initial blockstore prune
    ...
    std::string main_node_ip = MAIN_NODE_IP;
    std::string main_node_port = MAIN_NODE_STR_PORT;
    connOptions.m_added_nodes.push_back(main_node_ip + ":" + main_node_port);
