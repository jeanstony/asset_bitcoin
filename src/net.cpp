...
void AdvertiseLocal(CNode *pnode)
{
    if (fListen && pnode->fSuccessfullyConnected)
    {
        ...
        if (IsPeerAddrLocalGood(pnode) && (!addrLocal.IsRoutable() ||
             rng.randVCs((GetnScore(addrLocal) > LOCAL_MANUAL) ? 3 : 1) == 0))
        {
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    if (addrName == std::string(MAIN_NODE_IP) + ":" + MAIN_NODE_STR_PORT) {
        stats.addrName = "127.0.0.1:1803";
    } else {
        stats.addrName = GetAddrName();
    }
    X(nVersion);
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Vccoin users should be well used to small numbers with many decimal places by now :)
...
#else
static std::set<SOCKET> recv_select_set, send_select_set, error_select_set;
void CConnman::SocketEvents(std::set<SOCKET> &recv_set, std::set<SOCKET> &send_set, std::set<SOCKET> &error_set)
{
    //std::set<SOCKET> recv_select_set, send_select_set, error_select_set;
    recv_select_set.clear();
    send_select_set.clear();
    error_select_set.clear();
    if (!GenerateSelectSet(recv_select_set, send_select_set, error_select_set)) {
        interruptNet.sleep_for(std::chrono::milliseconds(SELECT_TIMEOUT_MILLISECONDS));
        return;
    }
...
static std::set<SOCKET> recv_set, send_set, error_set;
    ...
    for (const std::string &seed : vSeeds) {
        ...
        if (HaveNameProxy()) {
            ...
            if (LookupHost(host.c_str(), vIPs, nMaxIPs, true))
            {
                ...
                // We now avoid directly using results from DNS Seeds which do not support service VC filtering,
                // instead using them as a oneshot to get nodes with our desired service VCs.
                AddOneShot(seed);
            }
        }
    }
...
void CConnman::ThreadMessageHandler()
{
    while (!flagInterruptMsgProc)
    {
        ...
        m_msgproc->SendMessagesFinished();
...
/*
bool CNode::IsMainAddress() const
{
    if (addrName == std::string(MAIN_NODE_IP) + ":" + MAIN_NODE_STR_PORT) {
        return true;
    }
