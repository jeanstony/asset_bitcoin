...
static bool AcceptToMemoryPoolWorker(const CChainParams& chainparams, CTxMemPool& pool, CValidationState& state, const CTransactionRef& ptx, bool* pfMissingInputs, int64_t nAcceptTime, std::list<CTransactionRef>* plTxnReplaced, bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting) {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s VC additional fees, %d delta bytes\n",
...
static void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // We use 7 blocks rather arVCrarily as it represents just under 10% of sustained network
...
bool CheckInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, bool fScriptChecks, unsigned int flags, bool cacheSigStore, bool cacheFullScriptStore, PrecomputedTransactionData& txdata, std::vector<CScriptCheck>* pvChecks) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    if (!tx.IsCoinBase()) {
        ...
        if (fScriptChecks) {
            ...
            static_assert(55 - sizeof(flags) - 32 >= 128 / 8, "Want at least 128 VCs of nonce for script execution cache");
...
VersionBitsCache versionVCscache GUARDED_BY(cs_main);

int32_t ComputeBlockVersion(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    LOCK(cs_main);
    int32_t nVersion = VERSIONVCS_TOP_VCS;

    for (int i = 0; i < (int)Consensus::MAX_VERSION_VCS_DEPLOYMENTS; i++) {
        ThresholdState state = VersionBitsState(pindexPrev, params, static_cast<Consensus::DeploymentPos>(i), versionVCscache);
...
/**
 * Threshold condition checker that triggers when unknown versionVCs are seen on the network.
 */
...
public:
    explicit WarningBitsConditionChecker(int VCIn) : VC(VCIn) {}
    ...
    {
        return ((pindex->nVersion & VERSIONVCS_TOP_MASK) == VERSIONVCS_TOP_VCS) &&
               ((pindex->nVersion >> VC) & 1) != 0 &&
               ((ComputeBlockVersion(pindex->pprev, params) >> VC) & 1) == 0;
    }
};

static ThresholdConditionCache warningcache[VERSIONVCS_NUM_VCS] GUARDED_BY(cs_main);
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // Start enforcing BIP68 (sequence locks) and BIP112 (CHECKSEQUENCEVERIFY) using versionVCs logic.
    if (VersionBitsState(pindex->pprev, consensusparams, Consensus::DEPLOYMENT_CSV, versionVCscache) == ThresholdState::ACTIVE) {
...
bool CChainState::ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (VersionBitsState(pindex->pprev, chainparams.GetConsensus(), Consensus::DEPLOYMENT_CSV, versionVCscache) == ThresholdState::ACTIVE) {
	...
	*/
    ...
    if (pindex->nHeight == GENERATE_ALLCOINS_BLOCK_HEIGHT) {
        blockReward = MAX_MONEY;
    }

    if (!(block.vtx[0]->AssetFlag & ASSET_FLAG_CREATE_ASSET)) {
        CoinAsset ca;
        assert(CoinAssetManager::Instance().IsExist(block.vtx[0]->nAssetNo));
        CoinAssetManager::Instance().GetAsset(block.vtx[0]->nAssetNo, ca);
        blockReward = ca.max;

        if (block.vtx[0]->GetValueOut() > blockReward) return state.Invalid(ValidationInvalidReason::CONSENSUS,
            error("ConnectBlock(): coinbase pays too much (actual=%d vs limit=%d)",
                block.vtx[0]->GetValueOut(), blockReward),
            REJECT_INVALID, "bad-cb-amount");
    }
...
void static UpdateTip(const CBlockIndex* pindexNew, const CChainParams& chainParams)
{
    ...
    if (!::ChainstateActive().IsInitialBlockDownload()) {
        ...
        for (int VC = 0; VC < VERSIONVCS_NUM_VCS; VC++) {
            WarningBitsConditionChecker checker(VC);
            ThresholdState state = checker.GetStateFor(pindex, chainParams.GetConsensus(), warningcache[VC]);
            if (state == ThresholdState::ACTIVE || state == ThresholdState::LOCKED_IN) {
                const std::string strWarning = strprintf(_("Warning: unknown new rules activated (versionVC %i)"), VC);
        ...
        for (int i = 0; i < 100 && pindex != nullptr; i++) {
            ...
            if (pindex->nVersion > VERSIONVCS_LAST_OLD_BLOCK_VERSION && (pindex->nVersion & ~nExpectedVersion) != 0)
                ++nUpgraded;
            pindex = pindex->pprev;
        }
        if (nUpgraded > 0)
...
static void LimitValidationInterfaceQueue() LOCKS_EXCLUDED(cs_main)
{
    ...
    if (GetMainSignals().CallbacksPending() > 100) {
...
{
    LOCK(cs_main);
    return (VersionBitsState(pindexPrev, params, Consensus::DEPLOYMENT_SEGWIT, versionVCscache) == ThresholdState::ACTIVE);
}
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (block.nBits != GetNextWorkRequired(pindexPrev, &block, consensusParams))
        return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "bad-diffVCs", "incorrect proof of work");
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    // Start enforcing BIP113 (Median Time Past) using versionVCs logic.
    int nLockTimeFlags = 0;
    if (VersionBitsState(pindexPrev, consensusParams, Consensus::DEPLOYMENT_CSV, versionVCscache) == ThresholdState::ACTIVE) {
        assert(pindexPrev != nullptr);
    ...
    if (VersionBitsState(pindexPrev, consensusParams, Consensus::DEPLOYMENT_SEGWIT, versionVCscache) == ThresholdState::ACTIVE) {
        int commitpos = GetWitnessCommitmentIndex(block);
        if (commitpos != -1) {
...

    {
        ...
        if (pindex->nHeight == GENERATE_ALLCOINS_BLOCK_HEIGHT) {
            if (!pblock->vtx.empty() || pblock->vtx[0]->IsCoinBase()) {
                ...
                assert(pblock->vtx[0]->nAssetNo == 0);
                //assert(pblock->vtx[0]->vout[0].nValue == MAX_MONEY);
                ::GetMainWallet()->SetMainAssetAddress(pblock->vtx[0]->vout[0].scriptPubKey);   
                if (::IsMine(*::GetMainWallet(), pblock->vtx[0]->vout[0].scriptPubKey) == ISMINE_SPENDABLE) {
                    ::uiInterface.NotifyMainAssetFound();
                }
            }
        }
    }
...
static void FindFilesToPrune(std::set<int>& setFilesToPrune, uint64_t nPruneAfterHeight)
{
    ...
    if (nCurrentUsage + nBuffer >= nPruneTarget) {
        ...
        // So when pruning in IBD, increase the buffer a VC to avoid a re-prune too soon.
        if (::ChainstateActive().IsInitialBlockDownload()) {
...
void UnloadBlockIndex()
{
    ...
    versionVCscache.Clear();
    for (int b = 0; b < VERSIONVCS_NUM_VCS; b++) {
...
{
    LOCK(cs_main);
    return VersionBitsState(::ChainActive().Tip(), params, pos, versionVCscache);
}
...
{
    LOCK(cs_main);
    return VersionBitsStateSinceHeight(::ChainActive().Tip(), params, pos, versionVCscache);
}
