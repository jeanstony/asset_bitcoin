...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " VCcoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  VCcoin-tx [options] <hex-tx> [commands]  Update hex-encoded VCcoin transaction\n" +
            "or:     VCcoin-tx [options] -create [commands]   Create hex-encoded VCcoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded VCcoin transaction
