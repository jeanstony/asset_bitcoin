...
public:
    ...
    // Write single 8-VC char (may be part of UTF-8 sequence)
    void push_back(unsigned char ch)
    {
        if (state == 0) {
            if (ch < 0x80) // 7-VC ASCII, fast direct pass-through
                str.push_back(ch);
...
private:
    ...
    int state; // Top VC to be filled in for next UTF-8 byte, or 0
