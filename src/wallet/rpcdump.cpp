...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    RPCHelpMan{
        ...
        "Note: Use \"getwalletinfo\" to query the scanning progress.\n",
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The Vccoin address (or hex-encoded script)"},
...
UniValue importprunedfunds(const JSONRPCRequest& request)
{
    ...
    if (pwallet->GetAssetNo() != 0) {
        ...
        if (pMainWallet->IsMine(*wtx.tx)) {
            pMainWallet->AddToWallet(wtx, false);
        }
    }
...
UniValue removeprunedfunds(const JSONRPCRequest& request)
{
    ...
    if (AssetNo != 0) {
        std::shared_ptr<CWallet> const wallet = ::GetAssetWallet(AssetNo);
        CWallet* const pwallet = wallet.get();
        if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
            return NullUniValue;
        }
...
UniValue importwallet(const JSONRPCRequest& request)
{
    ...
    */
    ...
    file.open(request.params[0].get_str(), std::ios::in);
    if (!file.is_open()) {
    ...
    std::map<const int, std::pair<const int, const int>> asset_pos_map;
    long size = 0;
    file >> size;
    if (size <= 0) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        int offset = -1, endpos = -1;
        file >> offset >> endpos;
        if (offset < 0 || endpos < 0 || endpos < offset) {
            throw JSONRPCError(RPC_WALLET_ERROR, "Error adding some keys/scripts to wallet");
        }
        asset_pos_map.insert(std::make_pair(assetno, std::make_pair(offset, endpos)));
    }

    int cur_pos = file.tellg();
    file.close();
    file.open(request.params[0].get_str(), std::ios::in | std::ios::binary);
    file.seekg(cur_pos, file.beg);

    std::vector<std::pair<const int, fs::path>> asset_file_map;
    for (auto i : asset_pos_map) {
        std::stringstream ext;
        ext << ".tmp." << i.first;
        fs::path tmpfilepath = request.params[0].get_str();
        tmpfilepath.replace_extension(ext.str());

        fsbridge::ofstream ofile;
        ofile.open(tmpfilepath, std::ios::out | std::ios::binary);
        if (!ofile.is_open())
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Cannot open wallet dump file");

        int fsize = i.second.second - i.second.first;
        char* pBuf = new char[fsize];
        file.read(pBuf, fsize);
        if (file.gcount() != fsize) {
            delete[] pBuf;
            throw JSONRPCError(RPC_WALLET_ERROR, "Error readding the wallet data");
        }
        ofile.write(pBuf, fsize);
        delete[] pBuf;
        ofile.close();

        asset_file_map.push_back(std::make_pair(i.first, tmpfilepath));
    }
    file.close();

    int64_t nTimeBegin = 0;
    bool fGood = true;
    for (auto i : asset_file_map) {
        std::shared_ptr<CWallet> wallet_ptr = ::GetAssetWallet(i.first);
...

        {
            ...
            fsbridge::ifstream file;
            file.open(i.second, std::ios::in | std::ios::ate);
            if (!file.is_open()) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Cannot open wallet dump file");
            }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    RPCHelpMan{
        ...
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The VCcoin address for the private key"},
        },
        RPCResult{
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    RPCHelpMan{
        ...
        {
            {"filename", RPCArg::Type::STR, RPCArg::Optional::NO, "The filename with path (either absolute or relative to VCcoind)"},
        },
        RPCResult{
            "{                           (json object)\n"
    ...
    /* Prevent arVCrary files from being overwritten. There have been reports
     * that users have overwritten wallet files this way:
     * https://github.com/VCcoin/VCcoin/issues/9934
    ...
    for (auto wallet_ptr : Wallets) {
        ...
        std::stringstream ext;
        ext << ".tmp." << pwallet->GetAssetNo();
        fs::path tmpfilepath = filepath;
        tmpfilepath.replace_extension(ext.str());

        fsbridge::ofstream file;
        file.open(tmpfilepath);
        if (!file.is_open())
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Cannot open wallet dump file");
        ...
        asset_file_map.push_back(std::make_pair(pwallet->GetAssetNo(), tmpfilepath));
    }
    ...
    file << "# head" << std::endl;
    file << (long)asset_file_map.size() << std::endl;
    int offset = 0;
    for (auto i : asset_file_map) {
        file << (long)i.first << std::endl;
        file << (long)offset << std::endl;
        file << (long)(offset + fs::file_size(i.second)) << std::endl;
        offset = fs::file_size(i.second);
    }

    for (auto i : asset_file_map) {
        fsbridge::ifstream tmpfile;
        tmpfile.open(i.second);
        while (!tmpfile.eof()) {
            char buf[512] = {0};
            tmpfile.read(buf, 512);
            int rdcount = tmpfile.gcount();
            file.write(buf, rdcount);
        }
        tmpfile.close();
        fs::remove(i.second);
    }
