// Copyright (c) 2016-2019 The Vccoin Core developers
...
// queue, which doesn't exist for the VCcoin-wallet. Define our own
// deleter here.
...
static std::shared_ptr<CWallet> CreateWallet(const std::string& name, const fs::path& path)
{
    ...
    std::shared_ptr<CWallet> wallet_instance(new CWallet(nullptr /* chain */, WalletLocation(name), WalletDatabase::Create(path, 0), 0), WalletToolReleaseWallet);
    bool first_run = true;
