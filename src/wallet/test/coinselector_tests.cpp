// Copyright (c) 2017-2019 The Vccoin Core developers
...
static void add_coin(const CAmount& nValue, int nAge = 6*24, bool fIsFromMe = false, int nInput=0)
{
    ...
    tx.vout[nInput].nValue = nValue;
    if (fIsFromMe) {
        // IsFromMe() returns (GetDeVC() > 0), and GetDeVC() is 0 if vin.empty(),
        // so stop vin being empty, and cache a non-zero DeVC to fake out IsFromMe()
        tx.vin.resize(1);
    }
    std::unique_ptr<CWalletTx> wtx = MakeUnique<CWalletTx>(&testWallet, MakeTransactionRef(std::move(tx)));
    if (fIsFromMe)
    {
        wtx->m_amounts[CWalletTx::DEVC].Set(ISMINE_SPENDABLE, 1);
    }
...
inline std::vector<OutputGroup>& GroupCoins(const std::vector<COutput>& coins)
{
    static std::vector<OutputGroup> static_groups;
    static_groups.clear();
    for (auto& coin : coins) static_groups.emplace_back(coin.GetInputCoin(), coin.nDepth, coin.tx->m_amounts[CWalletTx::DEVC].m_cached[ISMINE_SPENDABLE] && coin.tx->m_amounts[CWalletTx::DEVC].m_value[ISMINE_SPENDABLE] == 1 /* HACK: we can't figure out the is_me flag so we use the conditions defined above; perhaps set safe to false for !fIsFromMe in add_coin() */, 0, 0);
    return static_groups;
}
