...
struct ValidationInterfaceConnections {
    ...
    boost::signals2::scoped_connection AddCoinAsset;
    boost::signals2::scoped_connection RemoveCoinAsset;
};
...
struct MainSignalsInstance {
    ...
    boost::signals2::signal<void (const CoinAsset&)> AddCoinAsset;
    boost::signals2::signal<void (const CoinAsset&)> RemoveCoinAsset;
...
void RegisterValidationInterface(CValidationInterface* pwalletIn) {
    ...
    conns.AddCoinAsset = g_signals.m_internals->AddCoinAsset.connect(std::bind(&CValidationInterface::AddCoinAsset, pwalletIn, std::placeholders::_1));
    conns.RemoveCoinAsset = g_signals.m_internals->RemoveCoinAsset.connect(std::bind(&CValidationInterface::RemoveCoinAsset, pwalletIn, std::placeholders::_1));
}
...
void CMainSignals::AddCoinAsset(const CoinAsset& ca)
{
    m_internals->m_schedulerClient.AddToProcessQueue([ca, this] {
        m_internals->AddCoinAsset(ca);
    });
}

void CMainSignals::RemoveCoinAsset(const CoinAsset& ca) 
{
    m_internals->m_schedulerClient.AddToProcessQueue([ca, this] {
        m_internals->RemoveCoinAsset(ca);
    });
}
