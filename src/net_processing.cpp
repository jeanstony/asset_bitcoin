...
static void PushNodeVersion(CNode* pnode, CConnman* connman, int64_t nTime)
{
    ...
    connman->PushMessage(pnode, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::VERSION, PROTOCOL_VERSION, (uint64_t)nLocalNodeServices, nTime, addrYou, addrMe, nonce, strSubVersion, nNodeStartingHeight, ::g_relay_txes));
...
static bool MaybePunishNode(NodeId nodeid, const CValidationState& state, bool via_compact_block, const std::string& message = "")
{
    ...
    case ValidationInvalidReason::BLOCK_MISSING_PREV: {
        // TODO: Handle this much more gracefully (10 DoS points is super arVCrary)
        LOCK(cs_main);
...
void PeerLogicValidation::AddCoinAsset(const CoinAsset& ca)
{
    std::shared_ptr<const CSerCoinAsset> psca = std::make_shared<const CSerCoinAsset>(ca);
    const CNetMsgMaker msgMaker(PROTOCOL_VERSION);

    LOCK(cs_main);

    connman->ForEachNode([this, &psca, &msgMaker](CNode* pnode) {
        AssertLockHeld(cs_main);

        if (pnode->nVersion < INVALID_CB_NO_BAN_VERSION || pnode->fDisconnect)
            return;

        LogPrint(BCLog::NET, "sending coinasset %s to peer=%d\n", "PeerLogicValidation::AddCoinAsset",
            psca->ToString().c_str(), pnode->GetId());
        pnode->PushSendAssetID(psca->GetAssetNo());
        connman->PushMessage(pnode, msgMaker.Make(NetMsgType::CRTCOINASSET, *psca));
    });
}
...
int static GetBlockAssetIDs(const CBlock* pBlk, std::vector<int>& AssetIDs)
{
    for (int i = 0; i < pBlk->vtx.size(); i++) {
        if (pBlk->vtx[i]->nAssetNo != 0) {
            AssetIDs.push_back(pBlk->vtx[i]->nAssetNo);
        }
    }
...
int static SendAssetsInfo2Peer(const CBlock* pBlk, CNode* pto, CConnman* connman)
{
    if (pBlk == NULL || pto == NULL) {
        return 0;
    }

    std::vector<int> AssetIDs;
    GetBlockAssetIDs(pBlk, AssetIDs);

    const CNetMsgMaker msgMaker(pto->GetSendVersion());
    for (auto AssetID : AssetIDs) {
        if (!pto->PushSendAssetID(AssetID)) {
            continue;
        }

        CoinAsset ca;
        CoinAssetManager::Instance().GetAsset(AssetID, ca);
        CSerCoinAsset sca(ca);

        connman->PushMessage(pto, msgMaker.Make(NetMsgType::CRTCOINASSET, sca));
    }
...
inline void static SendBlockTransactions(const CBlock& block, const BlockTransactionsRequest& req, CNode* pfrom, CConnman* connman)
{
    ...
    for (size_t i = 0; i < req.indexes.size(); i++) {
        ...
        const int AssetNo = block.vtx[req.indexes[i]]->nAssetNo;
        assert(CoinAssetManager::Instance().IsExist(AssetNo));

        if (AssetNo != 0) {
            CoinAsset ca;
            CoinAssetManager::Instance().GetAsset(AssetNo, ca);
            CSerCoinAsset SerCa(ca);
            pfrom->PushSerCoinAsset(SerCa);
        }
    }
    ...
    while (!done && !orphan_work_set.empty()) {
        ...
        if (AcceptToMemoryPool(mempool, orphan_state, porphanTx, &fMissingInputs2, &removed_txn, false /* bypass_limits */, 0 /* nAbsurdFee */)) {
            ...
            if (!orphanTx.HasWitness() && orphan_state.GetReason() != ValidationInvalidReason::TX_WITNESS_MUTATED) {
                ...
                // See https://github.com/VCcoin/VCcoin/issues/8279 for details.
                assert(recentRejects);
...
bool SendNotifyMessageIfAssetLocked(CNode* pfrom, CConnman* connman, const int AssetNo)
{
    std::shared_ptr<CWallet> pWallet = ::GetMainWallet();
    if (AssetNo == 0 || pWallet == nullptr || !pWallet->IsMainWallet()) {
        return false;
    }
    ...
    CSerCoinAssetStatus s;
    s.SetAssetNo(AssetNo, (int)ca.status, "CoinAssetDisable");

    connman->PushMessage(pfrom, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::COINASSETSTATUS, s));
    return true;
}
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    if (strCommand == NetMsgType::CMPCTBLOCK) {
...

        {
            ...
            // We want to be a VC conservative just to be extra careful about DoS
    ...
    if (strCommand == NetMsgType::BLOCKTXN) {
        ...
        bool fBlockRead = false;
        {
            ...
            if (status == READ_STATUS_INVALID) {
                ...
                for (auto txf : pblock->vtx) {
                    if (txf->nAssetNo != 0) {
                        SendNotifyMessageIfAssetLocked(pfrom, connman, txf->nAssetNo);
                    }
                }
    ...
    if (strCommand == NetMsgType::CRTCOINASSET) {
        LOCK(cs_main);

        CSerCoinAsset sca;
        vRecv >> sca;
        LogPrint(BCLog::NET, sca.ToString().c_str());

        const CoinAsset ca = sca;
        if (CoinAssetManager::Instance().IsExist(ca.no)) {
            return true;
        }
    ...
    if (strCommand == NetMsgType::GETCOINASSETSTATUS) {
        int AssetNo;
        vRecv >> AssetNo;
        LogPrint(BCLog::NET, "recv command GETCOINASSETSTATUS: %d", AssetNo);

        std::shared_ptr<CWallet> pWallet = ::GetMainWallet();
        if (pWallet != nullptr) {
            LOCK(pWallet->cs_wallet);
            if (pWallet->IsMainWallet()) {
                CoinAsset ca;
                CoinAssetManager::Instance().GetAsset(AssetNo, ca);
                CSerCoinAssetStatus cas;
                cas.SetAssetNo(ca.no, ca.status, "");

                connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::COINASSETSTATUS, cas));
            }
        }
    }

    if (strCommand == NetMsgType::COINASSETSTATUS) {
        //assert(pfrom->IsMainAddress());

        CSerCoinAssetStatus cas;
        vRecv >> cas;
        LogPrint(BCLog::NET, "recv command COINASSETSTATUS: %s", cas.ToString());

        unsigned int oldStatus = CoinAssetManager::Instance().UpdateCoinAssetStatus(cas.GetAssetNo(), cas.GetAssetStatus());
        if (oldStatus != (unsigned int)cas.GetAssetStatus()) {
            CoinAssetManager::Instance().WriteToDB();
            // notify UI
            ::uiInterface.NotifyAssetStatusChanged(cas.GetAssetNo());
        }

        /*
        if (pfrom->IsMainAddress()) {
            // broadcast the asset's status
            connman->ForEachNode([&cas, pfrom, &connman, &msgMaker](CNode* pnode) {
                AssertLockHeld(cs_main);
                if (pnode != pfrom) {
                    connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::COINASSETSTATUS, cas));
                }
...
int PeerLogicValidation::QueryCoinAssetsStatus(CNode* pto, const std::vector<int>& QueryAssets)
{
    AssertLockHeld(cs_main);
    const CNetMsgMaker msgMaker(pto->GetSendVersion());

    for (auto no : QueryAssets) {
        connman->PushMessage(pto, msgMaker.Make(NetMsgType::GETCOINASSETSTATUS, no));
    }
...
bool PeerLogicValidation::SendMessagesFinished()
{
    {
        LOCK(cs_vCoinAsset);
        cScaStatus.clear();
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        // start coinasset sync
        std::vector<CSerCoinAsset> scas;
        pto->GetSerCoinAssets(scas);
        if (scas.size() > 0) {
            for (auto s : scas) {
                connman->PushMessage(pto, msgMaker.Make(NetMsgType::CRTCOINASSET, s));
            }
        }

        bool SyncAllAssetsStatus = pto->ResetSyncAssetsFlag();
        if (SyncAllAssetsStatus) {
            do {
                std::shared_ptr<CWallet> pWallet = ::GetMainWallet();
                if (pWallet == nullptr || !pWallet->IsMainWallet()) {
                    break;
                }
                ...
                for (CoinAsset& ca : cas) {
                    if (ca.no == 0) {
                        continue;
                    }

                    CSerCoinAssetStatus s;
                    s.SetAssetNo(ca.no, (int)ca.status, "CoinAssetDisable");

                    connman->PushMessage(pto, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::COINASSETSTATUS, s));
                }
            } while (false);
        } else {
            // start asset status sync
            std::vector<CSerCoinAssetStatus> scas_stats;
            this->GetSerCoinAssetStatus(scas_stats);
            for (auto s : scas_stats) {
                connman->PushMessage(pto, msgMaker.Make(NetMsgType::COINASSETSTATUS, s));
            }
        }
