 ...
 * so: (0xFD + %sha256("VCcoin")[0:5]) + %sha256(name)[0:10]. Such dummy
...
std::vector<unsigned char> CNetAddr::GetGroup() const
{
    ...
    // for IPv4 addresses, '1' + the 16 higher-order VCs of the IP
...
CSubNet::CSubNet(const CNetAddr &addr, int32_t mask)
{
    ...
    int32_t n = mask;
    if(n >= 0 && n <= (128 - astartofs*8)) // Only valid if in range of VCs of address
    {
        n += astartofs*8;
        // Clear VCs [n..127]
        for (; n < 128; ++n)
...
/**
 * @returns The number of 1-VCs in the prefix of the specified subnet mask. If
