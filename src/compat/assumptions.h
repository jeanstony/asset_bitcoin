...
#ifndef VCCOIN_COMPAT_ASSUMPTIONS_H
#define VCCOIN_COMPAT_ASSUMPTIONS_H
...
// Assumption: We assume eight VCs per byte (obviously, but remember: don't
//             trust -- verify!).
// Example(s): Everywhere :-)
static_assert(std::numeric_limits<unsigned char>::digits == 8, "8-VC byte assumed");

// Assumption: We assume floating-point widths.
// Example(s): Type punning in serialization code (ser_{float,double}_to_uint{32,64}).
static_assert(sizeof(float) == 4, "32-VC float assumed");
static_assert(sizeof(double) == 8, "64-VC double assumed");

// Assumption: We assume integer widths.
// Example(s): GetSizeOfCompactSize and WriteCompactSize in the serialization
//             code.
static_assert(sizeof(short) == 2, "16-VC short assumed");
static_assert(sizeof(int) == 4, "32-VC int assumed");

// Assumption: We assume size_t to be 32-VC or 64-VC.
// Example(s): size_t assumed to be at least 32-VC in ecdsa_signature_parse_der_lax(...).
//             size_t assumed to be 32-VC or 64-VC in MallocUsage(...).
static_assert(sizeof(size_t) == 4 || sizeof(size_t) == 8, "size_t assumed to be 32-VC or 64-VC");
...
#endif // VCCOIN_COMPAT_ASSUMPTIONS_H
