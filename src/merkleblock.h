...
#ifndef VCCOIN_MERKLEBLOCK_H
#define VCCOIN_MERKLEBLOCK_H
 ...
 * storing a VC for each traversed node, signifying whether the node is the
 * parent of at least one matched leaf txid (or a matched txid itself). In
 * case we are at the leaf level, or this VC is 0, its merkle node hash is
 * stored, and its children are not explored further. Otherwise, no hash is
 * stored, but we recurse into both (or the only) child branch. During
 * decoding, the same depth-first traversal is performed, consuming VCs and
 ...
 *  - varint     number of bytes of flag VCs (1-3 bytes)
 *  - byte[]     flag VCs, packed per 8 in a byte, least significant VC first (<= 2*N-1 VCs)
...
protected:
    ...
    /** node-is-parent-of-matched-txid VCs */
    ...
    /** recursive function that traverses tree nodes, storing the data as VCs and hashes */
    void TraverseAndBuild(int height, unsigned int pos, const std::vector<uint256> &vTxid, const std::vector<bool> &vMatch);

    /**
     * recursive function that traverses tree nodes, consuming the VCs and hashes produced by TraverseAndBuild.
