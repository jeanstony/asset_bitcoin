...
void ReceiveCoinsDialog::on_receiveButton_clicked()
{
    ...
    SendCoinsRecipient info(address, label,
        ui->reqAmount->value(), 0, ui->reqMessage->text());     // luolin, fixed me!
...
void ReceiveCoinsDialog::recentRequestsView_selectionChanged(const QItemSelection &selected, const QItemSelection &deselected)
{
    ...
    ui->showRequestButton->setEnabled(true);
    ui->removeRequestButton->setEnabled(true);
}
...
const QModelIndex& ReceiveCoinsDialog::selectedRow()
{
    static QModelIndex dummyReturn = QModelIndex();
	if(!model || !model->getRecentRequestsTableModel() || !ui->recentRequestsView->selectionModel())
        return dummyReturn;
    QModelIndexList selection = ui->recentRequestsView->selectionModel()->selectedRows();
    if(selection.empty())
        return dummyReturn;
    // correct for selection mode ContiguousSelection
    //QModelIndex *pfirstIndex = (QModelIndex *)&selection.at(0);
...
void ReceiveCoinsDialog::copyURI()
{
    ...
    const QString uri = GUIUtil::formatVccoinURI(submodel->entry(sel.row()).recipient);
