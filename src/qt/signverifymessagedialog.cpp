...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    CTxDestination destination = DecodeDestination(GUIUtil::toStdString(ui->addressIn_SM->text()));
    ...
    ss << strMessageMagic;
    ss << GUIUtil::toStdString(ui->messageIn_SM->document()->toPlainText());
...
{
    CTxDestination destination = DecodeDestination(GUIUtil::toStdString(ui->addressIn_VM->text()));
    if (!IsValidDestination(destination)) {
    ...
    bool fInvalid = false;
    std::vector<unsigned char> vchSig = DecodeBase64(GUIUtil::toStdString(ui->signatureIn_VM->text()).c_str(), &fInvalid);
    ...
    CHashWriter ss(SER_GETHASH, 0);
    ss << strMessageMagic;
    ss << GUIUtil::toStdString(ui->messageIn_VM->document()->toPlainText());
