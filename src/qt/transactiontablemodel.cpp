...
{
    columns << QString() << QString() << tr("Date") << tr("Type") << tr("Label") << tr("Amount"); //VccoinUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
{
    columns[Amount] = VccoinUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
{
    uint256 updated;
    updated.SetHex(GUIUtil::toStdString(hash));
...
QString TransactionTableModel::formatTxAmount(const TransactionRecord* wtx, bool showUnconfirmed, VccoinUnits::SeparatorStyle separators) const
{
    QString str = VccoinUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->credit + wtx->deVC, false, separators);
    if (showUnconfirmed) {
        if (!wtx->status.countsForBalance) {
            str = QString("[") + str + QString("]");
        }
    }
    return QString(str) + " " + VccoinUnits::shortName(walletModel->getOptionsModel()->getDisplayUnit());
}

QString TransactionTableModel::formatTxFee(const TransactionRecord* wtx, bool showUnconfirmed, VccoinUnits::SeparatorStyle separators) const
{
    VccoinUnits::PushAsset();
    VccoinUnits::SetActiveCoinAsset();

    QString str = VccoinUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->fee, false, separators);
    if (showUnconfirmed) {
        if (!wtx->status.countsForBalance) {
            str = QString("[") + str + QString("]");
        }
    ...
    QString ret = QString(str) + " " + VccoinUnits::shortName(walletModel->getOptionsModel()->getDisplayUnit());
...
QVariant TransactionTableModel::data(const QModelIndex& index, int role) const
{
    ...
    VccoinUnits::SetActiveCoinAsset(rec->AssetNo);
    ...
    case Qt::DisplayRole:
        ...
        case Amount:
            return formatTxAmount(rec, true, VccoinUnits::separatorAlways);
        case Fee:
            return formatTxFee(rec, true, VccoinUnits::separatorAlways);
        }
        break;
    case Qt::EditRole:
        ...
        case Amount:
            return qint64(rec->credit + rec->deVC);
        case Fee:
            return qint64(rec->fee);
        }
        break;
    case Qt::ToolTipRole:
    ...
    case Qt::ForegroundRole:
        ...
        if (index.column() == Amount && (rec->credit + rec->deVC) < 0) {
    ...
    case TxPlainTextRole: {
        ...
        details.append(formatTxAmount(rec, false, VccoinUnits::separatorNever));
        return details;
    }
    case ConfirmedRole:
    ...
    case FormattedAmountRole:
        ...
        return formatTxAmount(rec, false, VccoinUnits::separatorNever);
    case StatusRole:
