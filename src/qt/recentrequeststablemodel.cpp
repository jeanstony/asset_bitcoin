    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return VccoinUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, VccoinUnits::separatorNever);
            else
                return VccoinUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+VccoinUnits::shortName(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
...
bool RecentRequestsTableModel::removeRows(int row, int count, const QModelIndex &parent)
{
    ...
    if(count > 0 && row >= 0 && (row+count) <= list.size())
    {
        for (int i = 0; i < count; ++i)
        {
            ...
            if (!walletModel->saveReceiveRequest(GUIUtil::toStdString(rec->recipient.address), rec->id, ""))
                return false;
        }
...
void RecentRequestsTableModel::addNewRequest(const SendCoinsRecipient &recipient)
{
    ...
    if (!walletModel->saveReceiveRequest(GUIUtil::toStdString(recipient.address), newEntry.id, ss.str()))
        return;
