...
const int VCCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString VCCOIN_IPC_PREFIX("VCcoin:");
#ifdef ENABLE_BIP70
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the VCcoin: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(VCCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // VCcoin: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseVccoinURI(arg, &r) && !r.address.isEmpty())
            {
                auto tempChainParams = CreateChainParams(CBaseChainParams::MAIN);

                if (IsValidDestinationString(GUIUtil::toStdString(r.address), *tempChainParams)) {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(VCCOIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(VCCOIN_IPC_CONNECT_TIMEOUT);
...
#endif
{
    ...
    // on Mac: sent when you click VCcoin: links
...
//
// OSX-specific way of handling VCcoin: URIs and PaymentRequest mime types.
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("VCcoin://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'VCcoin://' is not a valid URI. Use 'VCcoin:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(VCCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // VCcoin: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseVccoinURI(s, &recipient))
            {
                if (!IsValidDestinationString(GUIUtil::toStdString(recipient.address))) {
#ifndef ENABLE_BIP70
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in VCcoin: URIs
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom VCcoin addresses are not supported
        ...
        // Vccoin amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, optionsModel->node().getDustRelayFee())) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(VccoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
...
void PaymentServer::fetchPaymentACK(WalletModel* walletModel, const SendCoinsRecipient& recipient, QByteArray transaction)
{
    ...
    if (walletModel->wallet().getNewDestination(change_type, "", dest)) {
        ...
        std::string label = GUIUtil::toStdString(tr("Refund from %1").arg(recipient.authenticatedMerchant));
