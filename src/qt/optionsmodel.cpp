...
void OptionsModel::Init(bool resetSettings)
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", VccoinUnits::VC);
    ...
    if (!m_node.softSetArg("-dbcache", GUIUtil::toStdString(settings.value("nDatabaseCache").toString())))
        addOverriddenOption("-dbcache");

    if (!settings.contains("nThreadsScriptVerif"))
        settings.setValue("nThreadsScriptVerif", DEFAULT_SCRIPTCHECK_THREADS);
    if (!m_node.softSetArg("-par", GUIUtil::toStdString(settings.value("nThreadsScriptVerif").toString())))
    ...
    if (settings.value("fUseProxy").toBool() && !m_node.softSetArg("-proxy", GUIUtil::toStdString(settings.value("addrProxy").toString())))
    ...
    if (settings.value("fUseSeparateProxyTor").toBool() && !m_node.softSetArg("-onion", GUIUtil::toStdString(settings.value("addrSeparateProxyTor").toString())))
    ...
    if (!m_node.softSetArg("-lang", GUIUtil::toStdString(settings.value("language").toString())))
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/VCcoin/VCcoin/pull/8273
