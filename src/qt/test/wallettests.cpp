...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<VccoinAmountField*>("payAmount")->setValue(amount);
...
//
//     src/qt/test/test_VCcoin-qt -platform xcb      # Linux
//     src/qt/test/test_VCcoin-qt -platform windows  # Windows
//     src/qt/test/test_VCcoin-qt -platform cocoa    # macOS
void TestGUI()
{
    ...
    QString balanceComparison = VccoinUnits::formatWithUnit(unit, balance, false, VccoinUnits::separatorAlways);
    ...
    // Amount input
    VccoinAmountField* amountInput = receiveCoinsDialog.findChild<VccoinAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QVERIFY(paymentTextList.at(1).indexOf(QString("URI: VCcoin:")) != -1);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'test_VCcoin-qt -platform cocoa' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI();
