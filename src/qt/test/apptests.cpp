...
void TestRpcCommand(RPCConsole* console)
{
    ...
    UniValue value;
    value.read(GUIUtil::toStdString(output.right(output.size() - output.lastIndexOf(QChar::ObjectReplacementCharacter) - 1)));
    QCOMPARE(value["chain"].get_str(), std::string("regtest"));
}
} // namespace

//! Entry point for VccoinApplication tests.
void AppTests::appTests()
{
    ...
    connect(&m_app, &VccoinApplication::windowShown, this, &AppTests::guiTests);
...
//! Entry point for VccoinGUI tests.
void AppTests::guiTests(VccoinGUI* window)
{
    HandleCallback callback{"guiTests", *this};
    connect(window, &VccoinGUI::consoleShown, this, &AppTests::consoleTests);
