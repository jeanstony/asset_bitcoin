...
void RPCExecutor::request(const QString &command, const WalletModel* wallet_model)
{
    ...
    {
        std::string result;
        std::string executableCommand = GUIUtil::toStdString(command) + "\n";
...
void RPCConsole::on_lineEdit_returnPressed()
{
    ...
    if(!cmd.isEmpty())
    {
        ...
        try {
            std::string dummy;
            if (!RPCParseCommandLine(nullptr, dummy, GUIUtil::toStdString(cmd), false, &strFilteredCmd)) {
...
void RPCConsole::unbanSelectedNode()
{
    ...
    for(int i = 0; i < nodes.count(); i++)
    {
        ...
        LookupSubNet(GUIUtil::toStdString(strNode).c_str(), possibleSubnet);
