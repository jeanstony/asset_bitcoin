...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        CTxDestination curAddress = DecodeDestination(GUIUtil::toStdString(rec->address));
        if(index.column() == Label)
        {
            ...
            walletModel->wallet().setAddressBook(curAddress, GUIUtil::toStdString(value.toString()), strPurpose);
        } else if(index.column() == Address) {
            CTxDestination newAddress = DecodeDestination(GUIUtil::toStdString(value.toString()));
            ...
            else if(rec->type == AddressTableEntry::Sending)
            {
                ...
                walletModel->wallet().setAddressBook(newAddress, GUIUtil::toStdString(value.toString()), strPurpose);
            }
        }
        return true;
    }
...
{
    // Update address book model from Vccoin core
    priv->updateEntry(address, label, isMine, purpose, status);
}

QString AddressTableModel::addRow(const QString &type, const QString &label, const QString &address, const OutputType address_type)
{
    std::string strLabel = GUIUtil::toStdString(label);
    std::string strAddress = GUIUtil::toStdString(address);
...
bool AddressTableModel::removeRows(int row, int count, const QModelIndex &parent)
{
    ...
    walletModel->wallet().delAddressBook(DecodeDestination(GUIUtil::toStdString(rec->address)));
    return true;
}
...
bool AddressTableModel::getAddressData(const QString &address,
    ...
    CTxDestination destination = DecodeDestination(GUIUtil::toStdString(address));
