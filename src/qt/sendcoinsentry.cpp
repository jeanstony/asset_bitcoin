    ...
    platformStyle(_platformStyle)
{
    ...
    // normal VCcoin address field
    GUIUtil::setupAddressWidget(ui->payTo, this);
    // just a label for displaying VCcoin address(es)
    ui->payTo_is->setFont(GUIUtil::fixedPitchFont());

    // Connect signals
    connect(ui->payAmount, &VccoinAmountField::valueChanged, this, &SendCoinsEntry::payAmountChanged);
...
void SendCoinsEntry::UpdateAssetCoin(void)
{
    interfaces::WalletBalances nouse_balances;
    ui->payAmount->setBalance(nouse_balances);
}
...
void SendCoinsEntry::setModel(WalletModel *_model)
{
    ...
    connect(_model, &WalletModel::balanceChanged, ui->payAmount, &VccoinAmountField::setBalance);
...
SendCoinsRecipient SendCoinsEntry::getValue()
{
    ...
    recipient.AssetNo = ui->payAmount->getCurrentAssetNo();
...
void SendCoinsEntry::getAmount(CAmount &amount, int &AssetNo) const
{
    AssetNo = ui->payAmount->getCurrentAssetNo();
    amount = ui->payAmount->value();
}
