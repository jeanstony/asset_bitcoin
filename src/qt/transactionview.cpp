...
void TransactionView::changedAmount()
{
    ...
    if (VccoinUnits::parse(model->getOptionsModel()->getDisplayUnit(), amountWidget->text(), &amount_parsed)) {
...
void TransactionView::exportClicked()
{
    ...
    writer.addColumn(VccoinUnits::getAmountColumnTitle(model->getOptionsModel()->getDisplayUnit()), 0, TransactionTableModel::FormattedAmountRole);
...
void TransactionView::contextualMenu(const QPoint &point)
{
    ...
    uint256 hash;
    hash.SetHex(GUIUtil::toStdString(selection.at(0).data(TransactionTableModel::TxHashRole).toString()));
    //abandonAction->setEnabled(false);       //model->wallet().transactionCanBeAbandoned(hash));
    //bumpFeeAction->setEnabled(false);       //model->wallet().transactionCanBeBumped(hash));
...
void TransactionView::abandonTx()
{
    ...
    hash.SetHex(GUIUtil::toStdString(hashQStr));
