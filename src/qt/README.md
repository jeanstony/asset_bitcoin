This directory contains the VccoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Vccoin UI.
...
ClientModel is used by the main application `VCcoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/VCcoin/VCcoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a VCcoin:... link from a browser).
...
* UI elements like VccoinAmountField, which inherit from QWidget.
* `VCcoinstrings.cpp`: automatically generated
* `VCcoinunits.(h/cpp)`: BTC / mBTC / etc handling
* `callback.h`
...
* don't change `local/VCcoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
8. Select the default "Desktop" kit and select "Clang (x86 64VC in /usr/bin)" as compiler
9. Select LLDB as debugger (you might need to set the path to your installation)
10. Start debugging with Qt Creator (you might need to the executable to "VCcoin-qt" under "Run", which is where you can also add command line arguments)
