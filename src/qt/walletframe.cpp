...
WalletFrame::WalletFrame(PlatformStyle *_platformStyle, VccoinGUI *_gui) :
    QFrame(_gui),
    gui(_gui),
...
void WalletFrame::addWallet(WalletModel *walletModel)
{
    ...
    connect(this, &WalletFrame::NotifyModalOverlayActived, walletView, &WalletView::NotifyModalOverlayActived);
}
...
void WalletFrame::ModalOverlayActived(bool IsActive)
{
    if(platformStyle){
        platformStyle->SetModaloverlayViewActive(IsActive);
    }
    Q_EMIT NotifyModalOverlayActived(IsActive);
}
