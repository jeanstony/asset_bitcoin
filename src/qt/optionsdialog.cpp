    ...
    mapper(nullptr)
{
    ...
    ui->VCcoinAtStartup->setVisible(false);
    ui->verticalLayout_Main->removeWidget(ui->VCcoinAtStartup);
    ...
    ui->VCcoinAtStartup->setToolTip(ui->VCcoinAtStartup->toolTip().arg(PACKAGE_NAME));
    ui->VCcoinAtStartup->setText(ui->VCcoinAtStartup->text().arg(PACKAGE_NAME));

    ui->openVccoinConfButton->setToolTip(ui->openVccoinConfButton->toolTip().arg(PACKAGE_NAME));
    ...
    ui->unit->setModel(new VccoinUnits(this));
...
{
    /* Main */
    mapper->addMapping(ui->VCcoinAtStartup, OptionsModel::StartAtStartup);
...
void OptionsDialog::on_openVccoinConfButton_clicked()
{
...
void OptionsDialog::updateDefaultProxyNets()
{
    ...
    (strProxy == GUIUtil::toStdString(strDefaultProxyGUI)) ? ui->proxyReachIPv4->setChecked(true) : ui->proxyReachIPv4->setChecked(false);

    model->node().getProxy(NET_IPV6, proxy);
    strProxy = proxy.proxy.ToStringIP() + ":" + proxy.proxy.ToStringPort();
    strDefaultProxyGUI = ui->proxyIp->text() + ":" + ui->proxyPort->text();
    (strProxy == GUIUtil::toStdString(strDefaultProxyGUI)) ? ui->proxyReachIPv6->setChecked(true) : ui->proxyReachIPv6->setChecked(false);

    model->node().getProxy(NET_ONION, proxy);
    strProxy = proxy.proxy.ToStringIP() + ":" + proxy.proxy.ToStringPort();
    strDefaultProxyGUI = ui->proxyIp->text() + ":" + ui->proxyPort->text();
    (strProxy == GUIUtil::toStdString(strDefaultProxyGUI)) ? ui->proxyReachTor->setChecked(true) : ui->proxyReachTor->setChecked(false);
}
...
QValidator::State ProxyAddressValidator::validate(QString &input, int &pos) const
{
    Q_UNUSED(pos);
    // Validate the proxy
    CService serv(LookupNumeric(GUIUtil::toStdString(input).c_str(), DEFAULT_GUI_PROXY_PORT));
