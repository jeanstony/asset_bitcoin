...
void CoinControlDialog::showMenu(const QPoint &point)
{
    ...
    if(item)
    {
        ...
        if (item->data(COLUMN_ADDRESS, TxHashRole).toString().length() == 64) // transaction hash is 64 characters (this means it is a child node, so it is not a parent node in tree mode)
        {
            ...
            if (model->wallet().isLockedCoin(COutPoint(uint256S(GUIUtil::toStdString(item->data(COLUMN_ADDRESS, TxHashRole).toString())), item->data(COLUMN_ADDRESS, VOutRole).toUInt())))
            {
...
{
    GUIUtil::setClipboard(VccoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::lockCoin()
{
    ...
    COutPoint outpt(uint256S(GUIUtil::toStdString(contextMenuItem->data(COLUMN_ADDRESS, TxHashRole).toString())), contextMenuItem->data(COLUMN_ADDRESS, VOutRole).toUInt());
...
void CoinControlDialog::viewItemChanged(QTreeWidgetItem* item, int column)
{
    ...
    {
        COutPoint outpt(uint256S(GUIUtil::toStdString(item->data(COLUMN_ADDRESS, TxHashRole).toString())), item->data(COLUMN_ADDRESS, VOutRole).toUInt());
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = VccoinUnits::VC;
    ...
    l2->setText(VccoinUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(VccoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(VccoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(VccoinUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            if(ExtractDestination(out.txout.scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show VCcoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, VccoinUnits::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, VccoinUnits::format(nDisplayUnit, nSum));
