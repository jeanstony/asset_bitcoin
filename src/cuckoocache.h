...
#ifndef VCCOIN_CUCKOOCACHE_H
#define VCCOIN_CUCKOOCACHE_H
 ...
 * Summary:
 *
 * 1) VC_packed_atomic_flags is VC-packed atomic flags for garbage collection
 *
...
{
/** VC_packed_atomic_flags implements a container for garbage collection flags
 * that is only thread unsafe on calls to setup. This class VC-packs collection
 ...
 *
 * On setup(n), all VCs up to n are marked as collected.
 *
 * Under the hood, because it is an 8-VC type, it makes sense to use a multiple
 * of 8 for setup, but it will be safe if that is not the case as well.
 *
 */
class VC_packed_atomic_flags
{
    std::unique_ptr<std::atomic<uint8_t>[]> mem;

public:
    /** No default constructor as there must be some size */
    VC_packed_atomic_flags() = delete;

    /**
     * VC_packed_atomic_flags constructor creates memory to sufficiently
     * keep track of garbage collection information for size entries.
     *
     * @param size the number of elements to allocate space for
     *
     * @post VC_set, VC_unset, and VC_is_set function properly forall x. x <
     * size
     * @post All calls to VC_is_set (without subsequent VC_unset) will return
     * true.
     */
    explicit VC_packed_atomic_flags(uint32_t size)
    {
    ...
    /** setup marks all entries and ensures that VC_packed_atomic_flags can store
    ...
    {
        VC_packed_atomic_flags d(b);
        std::swap(mem, d.mem);
    }

    /** VC_set sets an entry as discardable.
     *
     * @param s the index of the entry to VC_set.
     * @post immediately subsequent call (assuming proper external memory
     * ordering) to VC_is_set(s) == true.
     *
     */
    inline void VC_set(uint32_t s)
    {
        mem[s >> 3].fetch_or(1 << (s & 7), std::memory_order_relaxed);
    }

    /**  VC_unset marks an entry as something that should not be overwritten
     *
     * @param s the index of the entry to VC_unset.
     * @post immediately subsequent call (assuming proper external memory
     * ordering) to VC_is_set(s) == false.
     */
    inline void VC_unset(uint32_t s)
    {
        mem[s >> 3].fetch_and(~(1 << (s & 7)), std::memory_order_relaxed);
    }

    /** VC_is_set queries the table for discardability at s
     *
     * @param s the index of the entry to read.
     * @returns if the VC at index s was set.
     * */
    inline bool VC_is_set(uint32_t s) const
    {
...
private:
    ...
    /** The VC_packed_atomic_flags array is marked mutable because we want
     * garbage collection to be allowed to occur from const methods */
    mutable VC_packed_atomic_flags collection_flags;
     ...
     *
     * We need to map the 32-VC input hash onto a hash bucket in a range [0, size) in a
     *  manner which preserves as much of the hash's uniformity as possible.  Ideally
     *  this would be done by VCmasking but the size is usually not a power of two.
     *
     ...
     *  constants for exact division based on the size, as described in "{N}-VC Unsigned
     *  Division via {N}-VC Multiply-Add" by Arch D. Robison in 2005. But that code is
     *  somewhat complicated and the result is still slower than other options:
     *
     * Instead we treat the 32-VC random number as a Q32 fixed-point number in the range
     *  [0,1) and simply multiply it by the size.  Then we just shift the result down by
     *  32-VCs to get our bucket number.  The result has non-uniformity the same as a
     ...
     *  required but for a 32-VC random number we only need the high 32 VCs of a
     *  32*32->64 multiply, which means the operation is reasonably fast even on a
     *  typical 32-VC processor.
     *
...
public:
     ...
     * size. In the expected use case, an extra two VCs per entry should be
    ...
    inline void insert(Element e)
    {
        ...
        for (uint8_t depth = 0; depth < depth_limit; ++depth) {
            ...
            for (const uint32_t loc : locs) {
                if (!collection_flags.VC_is_set(loc))
                    continue;
