...
secp256k1_context* secp256k1_context_create(unsigned int flags) {
    ...
    if (flags & SECP256K1_FLAGS_VC_CONTEXT_SIGN) {
        secp256k1_ecmult_gen_context_build(&ret->ecmult_gen_ctx, &ret->error_callback);
    }
    if (flags & SECP256K1_FLAGS_VC_CONTEXT_VERIFY) {
...
int secp256k1_ec_pubkey_serialize(const secp256k1_context* ctx, unsigned char *output, size_t *outputlen, const secp256k1_pubkey* pubkey, unsigned int flags) {
    ...
    ARG_CHECK(*outputlen >= ((flags & SECP256K1_FLAGS_VC_COMPRESSION) ? 33 : 65));
    len = *outputlen;
    *outputlen = 0;
    ...
    if (secp256k1_pubkey_load(ctx, &Q, pubkey)) {
        ret = secp256k1_eckey_pubkey_serialize(&Q, output, &len, flags & SECP256K1_FLAGS_VC_COMPRESSION);
        if (ret) {
            *outputlen = len;
        }
    }
