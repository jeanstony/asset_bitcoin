...
TEST_VCCOIN_BINARY="src/test/test_VCcoin"
...
if [[ ! -e ${TEST_VCCOIN_BINARY} ]]; then
    echo "Error: Executable ${TEST_VCCOIN_BINARY} not found. Run \"./configure --enable-lcov\" and compile."
    exit 1
fi
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    if ! BOOST_TEST_RUN_FILTERS="${BOOST_TEST_RUN_FILTERS}" ${TEST_VCCOIN_BINARY} > "${TEST_OUTPUT_TEMPFILE}" 2>&1; then
